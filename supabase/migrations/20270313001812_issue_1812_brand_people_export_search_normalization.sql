-- Issue #1812: PostgreSQL standard-conforming strings treat '\\s+' as two
-- literal backslashes followed by s+, so #1770 did not collapse interior
-- whitespace. Use an explicit POSIX character class without changing any
-- export authorization, filter, audit, job, worker, or provider-hook contract.
