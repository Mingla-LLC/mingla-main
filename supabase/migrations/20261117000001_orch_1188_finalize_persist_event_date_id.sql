-- ORCH-1188 FIX 3b [finalize persists the booked occurrence onto the order]
-- ===================================================================
-- biz_ticket_checkout_finalize creates the `orders` row but never recorded WHICH
-- occurrence the buyer booked, even though the buyer's selected occurrence lives
-- on the session at `ticket_checkout_sessions.metadata->>'event_date_id'`
-- (written by ticket-checkout-create). The consumer calendar therefore fell back
-- to the event's is_master (often past) date → wrong date + wrong Archives split.
--
-- 20261117000000 added the nullable `orders.event_date_id` column. This migration
-- re-declares biz_ticket_checkout_finalize(uuid, text, text, text, text, text,
-- text, boolean) VERBATIM from the current live definition (ORCH-1006,
-- 20260802000002_orch_1006_finalize_copy_pricing_breakdown.sql), changing ONLY
-- the order INSERT to ALSO write `event_date_id` from the session ROWTYPE
-- (`(v_session.metadata->>'event_date_id')::uuid`, NULL-safe — NULL for
-- single-date events / sessions with no selected occurrence). No signature,
-- return type, security context, search_path, grants, idempotent early-return,
-- compare-and-correct, installment logic, ticket-minting, chat, or notification
-- logic is altered. ADDITIVE-ONLY on the payment-critical, idempotent finalize.
--
-- ⚠️ ORCHESTRATOR: verify this re-emit matches the LIVE function body before
-- applying (clobber risk). Source of truth re-emitted from:
--   supabase/migrations/20260802000002_orch_1006_finalize_copy_pricing_breakdown.sql
-- (confirmed the last migration that does CREATE OR REPLACE FUNCTION
-- public.biz_ticket_checkout_finalize; META-ORCH-1076 / 1148 / 1174 reference but
-- do NOT redefine it — 8-param overload, unchanged).
--
-- Collision-checked: 20261117000001 (strictly greater than 20261117000000).
-- Idempotent / safe to re-run: CREATE OR REPLACE + idempotent GRANT.
...
DO $$
BEGIN
  -- Confirm the function exists with exactly one overload at 8 params.
  IF (SELECT COUNT(*) FROM pg_proc WHERE proname = 'biz_ticket_checkout_finalize' AND pronargs = 8) <> 1 THEN
    RAISE EXCEPTION 'ORCH-1188 self-verify: expected exactly 1 biz_ticket_checkout_finalize overload with 8 params';
  END IF;
END$$;
