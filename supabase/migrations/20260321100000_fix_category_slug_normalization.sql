-- ═══════════════════════════════════════════════════════════════════════════════
-- Fix: Category slug normalization in query_pool_cards
-- ═══════════════════════════════════════════════════════════════════════════════
--
-- ROOT CAUSE: card_pool.categories stores slugs (e.g., 'nature_views'),
-- but callers send display names (e.g., 'Nature & Views'). The array
-- overlap operator (&&) does exact-match, so zero cards ever matched.
--
-- Also fixes: v_hidden_categories used 'Groceries' (display name) instead
-- of 'groceries' (slug), so groceries-only cards leaked through.
--
-- Strategy: Normalize p_categories to slug format at function entry using
-- a strict CASE mapping. Unknown values are dropped (ELSE NULL).
-- ═══════════════════════════════════════════════════════════════════════════════
