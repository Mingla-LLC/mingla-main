-- ═══════════════════════════════════════════════════════════════════════════════
-- Pass 4b: School Exclusions + Flowers Fix
-- ═══════════════════════════════════════════════════════════════════════════════

-- P4-05: Exclude school-related place types from ALL categories.
-- Schools, academies, and educational institutions should never appear in a dating/experiences app.

INSERT INTO category_type_exclusions (category_slug, excluded_type)
SELECT cs.slug, et.type
FROM (VALUES
  ('nature'),('first_meet'),('picnic_park'),('drink'),('casual_eats'),
  ('fine_dining'),('watch'),('live_performance'),('creative_arts'),
  ('play'),('wellness'),('flowers'),('groceries')
) AS cs(slug)
CROSS JOIN (VALUES
...
-- P4-06: Exclude retail/grocery types from flowers category.
-- Flowers should only surface florist-type places.

INSERT INTO category_type_exclusions (category_slug, excluded_type)
VALUES
  ('flowers', 'grocery_store'),
  ('flowers', 'supermarket'),
  ('flowers', 'department_store'),
  ('flowers', 'garden_center'),
  ('flowers', 'home_improvement_store'),
  ('flowers', 'convenience_store')
ON CONFLICT DO NOTHING;

-- Update query_pool_cards to include school types in global exclusion.
