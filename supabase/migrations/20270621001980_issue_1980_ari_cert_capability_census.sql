-- Issue #1980 — re-pin Ari certification capability census after marketing/growth tools.
-- #1980 adds update_campaign_draft, delete_campaign_draft, get_growth_tool_report
-- (+3 registered tools) and three ledger capability rows (132 → 135).
-- Restores Pass-5 begin_run / finalize_run count gates to the live ledger size.
-- MONOTONIC VERSION 20270621001980 (after 20270620003157 on disk).
-- Do NOT apply here — orchestrator owns production apply.

BEGIN;

-- Seed certification requirements for the three #1980 registered tools.
...
BEGIN
  ...
  END IF;
  IF v_capability_count <> 135 THEN RAISE EXCEPTION 'ari_cert_missing_capabilities:%', v_capability_count; END IF;
...
COMMENT ON FUNCTION public.ari_cert_begin_run(text, jsonb, text, jsonb, jsonb) IS
  'Issue #2060 Pass-5 post-#2830: stamps private.ari_cert_requirements_set_digest_v1() of the live 135-row requirement set.';
COMMENT ON FUNCTION public.ari_cert_finalize_run(uuid) IS
  'Issue #2060 Pass-5 post-#2830: finalizes only when v_run.requirements_digest equals a fresh private.ari_cert_requirements_set_digest_v1().';
