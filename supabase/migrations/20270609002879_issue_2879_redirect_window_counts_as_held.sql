-- ===========================================================================
-- #2879 B/C step 1 — a buyer on the payment provider's page HOLDS their
-- tickets. Today they hold nothing.
--
-- THE INCONSISTENCY, IN THE EXISTING CODE. `awaiting_web_redirect` is the
-- state a session sits in while the buyer is on Stripe's or Paystack's hosted
-- page. The capacity guard ALREADY treats it as a live in-flight state — its
-- stale-session cleanup lists it alongside pending_free / requires_payment /
-- processing_payment and expires it past `expires_at`. But the HELD count,
-- three lines further down the same function, leaves it out.
--
-- So the same status is "live" for expiry and "not holding anything" for
-- capacity. A buyer can be beaten to their own ticket while paying for it,
-- and the result is a charge that `biz_ticket_checkout_finalize` then has to
-- reverse — the paid_reversal_pending path. Charging someone and refunding
-- them is a materially worse outcome than telling them no.
--
-- ONE OWNER FOR CAPACITY (#2462). The predicate is copied in three places and
-- they must move together, or the number a buyer is shown, the number the
-- server enforces, and the number the counter shadows stop agreeing:
--     issue_2491_derived_held                        (the counter's mirror)
--     issue_1930_ticket_checkout_create_session_base (the guard, v_reserved)
--     pg_direct_event_checkout_bundle                (the public `remaining`)
--
-- SAFE DIRECTION. This can only ever count MORE as held, so `remaining` gets
-- more conservative and the guard refuses earlier. It cannot cause an oversell;
-- it can only prevent one.
--
-- SMALL TODAY, LOAD-BEARING UNDER LOAD. Zero sessions are in this state right
-- now and only six have ever been. It matters when many people are paying at
-- once — which is precisely the moment #2879's waiting room promises "your
-- tickets are held", a promise that cannot be true across this window until
-- the window counts.
--
-- The guard's pre-existing four-status EXPIRY list is deliberately untouched.
-- ===========================================================================
...
-- ── probe ──────────────────────────────────────────────────────────────────
DO $probe$
DECLARE h text; g text; b text; v_types int; v_bad int; v_r jsonb;
  k_held CONSTANT text := '''pending_free'', ''requires_payment'', ''processing_payment'', ''awaiting_web_redirect''';
BEGIN
  h := pg_get_functiondef('public.issue_2491_derived_held'::regproc);
  ...
  -- 1. ALL THREE moved. Two out of three is the #2462 defect returning in a
  --    new place: the advertised number and the enforced number disagreeing.
  IF position(k_held IN h) = 0 THEN RAISE EXCEPTION 'PROBE FAIL: counter mirror still on three statuses'; END IF;
  IF position(k_held IN g) = 0 THEN RAISE EXCEPTION 'PROBE FAIL: capacity guard still on three statuses'; END IF;
  IF position(k_held IN b) = 0 THEN RAISE EXCEPTION 'PROBE FAIL: public remaining still on three statuses'; END IF;

  -- 2. and NONE of them kept a stale three-status copy alongside the new one
  IF position('''processing_payment'')' IN h) > 0
     OR position('''processing_payment'')' IN b) > 0 THEN
    RAISE EXCEPTION 'PROBE FAIL: a three-status held list survives somewhere';
  END IF;

  -- 3. the guard's SEPARATE expiry list is untouched — it already named all
  --    four and is not this migration's business
  IF position('''pending_free'',''requires_payment'',''processing_payment'',''awaiting_web_redirect''' IN g) = 0 THEN
    RAISE EXCEPTION 'PROBE FAIL: the guard''s pre-existing expiry list was altered';
  END IF;

  -- 4. the sold counter is untouched and still agrees, denominator reported
  ...
  IF v_bad <> 0 THEN
    RAISE EXCEPTION 'PROBE FAIL: % of % ticket types disagree on sold', v_bad, v_types;
  END IF;
  RAISE NOTICE 'issue #2879: 0 of % ticket types drift on sold', v_types;

  -- 5. the reconciler is clean AFTER widening the held definition. held_count
  --    is maintained by triggers that recompute from issue_2491_derived_held,
  --    so changing that function changes what they compute — this asserts the
  --    stored counters were refreshed to match, not left behind.
  PERFORM public.issue_2491_refresh_counters(tt.id) FROM public.ticket_types tt WHERE tt.deleted_at IS NULL;
  v_r := public.issue_2491_reconcile_ticket_type_counters(false);
  IF (v_r->>'driftCount')::int <> 0 THEN
    RAISE EXCEPTION 'PROBE FAIL: counters drift after widening the held definition: %', v_r;
  END IF;
END $probe$;
