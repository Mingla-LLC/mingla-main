...
-- #3439 timing rework: exact latest inner owner from 20270702003313,
-- with only locked cover reconciliation / poster persistence added. The latest
-- wrapper from 20270422001972 delegates without a competing poster write.
-- CREATE OR REPLACE retains existing signatures, ownership and ACLs.
...
BEGIN
  PERFORM public.business_assert_event_visibility(p_draft_payload#>'{theme,business_draft,requestedVisibility}');
  -- #3439: the locked publish owner resolves, validates and persists the
  -- URL/type/poster together. A second write from stale input would either
  -- erase the poster or roll the safe publish back with persist_mismatch.
