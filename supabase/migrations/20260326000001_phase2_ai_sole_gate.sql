-- ═══════════════════════════════════════════════════════════════════════════════
-- Phase 2: AI Sole Quality Gate
-- Strips all type exclusion NOT EXISTS blocks from query_pool_cards.
-- Flips AI default from true to false (unvalidated = hidden).
--
-- ⚠️  DEPLOY ONLY AFTER ALL EXISTING CARDS HAVE BEEN VALIDATED VIA ai-validate-cards.
-- ⚠️  Otherwise unvalidated cards will be hidden from users.
-- ═══════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  filtered AS (
    ...
    WHERE cp.is_active = true
      ...
      -- Phase 2: AI is the sole quality gate. Unvalidated cards are hidden (default false).
      -- Deploy ONLY after all existing cards have been validated via ai-validate-cards.
      AND COALESCE(cp.ai_override, cp.ai_approved, false) = true
  ),
  deduped AS (
