-- Issue #2060 Pass-5 — re-pin certification digest after #2830.
-- #2830 (20270609002830) redefined ari_cert_begin_run / ari_cert_finalize_run with
-- hardcoded literals for the 132-capability set. This migration restores the Pass-5
-- contract: both halves derive requirements_digest from
-- private.ari_cert_requirements_set_digest_v1() of the live requirement table.
-- MONOTONIC VERSION 20270610002060 (after 20270609002830 on disk).
...
COMMENT ON FUNCTION public.ari_cert_begin_run(text, jsonb, text, jsonb, jsonb) IS
  'Issue #2060 Pass-5 post-#2830: stamps private.ari_cert_requirements_set_digest_v1() of the live 132-row requirement set.';
