...
--
-- This migration deliberately does NOT create or imitate the #1972 generic
-- agent pending-action receipt. rsvp_domain_operation_receipts is the narrower
-- domain replay boundary required for RSVP mutations; the two layers compose
-- after #1972 merges.
...
BEGIN
  ...
  WITH page AS (
    ...
    LEFT JOIN LATERAL(SELECT financial_state FROM public.source_refunds s
      WHERE s.source_type='rsvp_contribution' AND s.source_id=c.id ORDER BY s.created_at DESC LIMIT 1) sr ON true
...
CREATE FUNCTION public.biz_prepare_rsvp_contribution_refund(
