...
-- Add session_id to boards table if it doesn't exist
ALTER TABLE public.boards ADD COLUMN IF NOT EXISTS session_id UUID NULL;
...
-- Note: We don't add FK to auth.users as it's managed by Supabase