...
BEGIN
  ...
  END IF;
  -- Readiness true is unreachable this release (the operator RPC refuses
  -- unconditionally), and the identity-bound contract is frozen. The DISTINCT reason
  -- class matters: it is what makes the readiness check above independently falsifiable.
  -- Deleting that check does not leave the denial intact — it changes the observable
  -- reason from `private_access_not_ready` to `private_access_release_frozen`, which
  -- SC-46 asserts on exactly.
  RAISE EXCEPTION 'private_access_release_frozen';
END $function$;
...
-- The explicit `security_invoker = false` is RE-STATED here. The landed view carried it
-- from 20270116000869:969, and a bare CREATE OR REPLACE VIEW drops the reloption —
-- behaviourally identical today (unset defaults to false) which is why byte-identity
-- could not see it, but it silently removes the explicit guarantee.
...
-- 6.11 — business_publish_event_draft — the legacy-Private-draft PUBLISH BLOCK.
-- Latest definer resolved at implementation time:
-- 20270323001919_issue_1919_provider_neutral_paid_readiness.sql:35
--
-- This is the ONE released object whose behaviour deliberately changes at readiness
-- false, and it changes ONLY on the `private` branch. Amendment 1 §3 requires it by
-- name: "For requestedVisibility='private' ... require both readiness functions true",
-- and "the server independently returns typed private_access_not_ready". Amendment 3
-- §5 released item 6 names "the legacy-Private-draft publish block" as RELEASED.
--
-- Every non-private publish path is bit-identical, which is proven by execution rather
-- than asserted. It is reached from the client through
-- issue_1719_publish_event_with_poster, which delegates here, so the block covers the
-- real publish path and not merely a helper nothing calls.
