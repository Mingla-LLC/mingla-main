-- ORCH-0950 [Trip capacity + dashboard coherence - expanded scope]
--
-- Expanded direction: trip capacity, dates, and destination text are canonical
-- SQL columns. JSONB business_trip is retained for forward-compatible metadata
-- but no longer owns capacity/start/end/destination text.
...
-- 6. Add canonical trip destination text column.
...
-- 7. Backfill destination_text from legacy JSONB where it still exists.
UPDATE public.events
...
DO $$
DECLARE
  v_missing_destination_count int;
BEGIN
  SELECT count(*) INTO v_missing_destination_count
  FROM public.events
  WHERE event_type = 'trip'
    AND deleted_at IS NULL
  ...
  RAISE NOTICE 'ORCH-0950 expanded destination backfill: % scheduled/live trip row(s) still have destination_text NULL and require operator re-entry if destination was historically wiped.', v_missing_destination_count;
END;
$$;

-- 8. Strip canonicalized destination/date keys from legacy JSONB.
UPDATE public.events
SET
  theme = jsonb_set(
    theme,
...
BEGIN
  ...
  IF v_residue_count > 0 THEN
    RAISE EXCEPTION 'ORCH-0950 expanded strip failed: % trip rows still have canonicalized business_trip destination/date keys', v_residue_count;
  END IF;
END;
$$;

-- 9. Per-tier ticket-sold RPC for planner dashboard tier cards.
...
BEGIN
  SELECT EXISTS (
    ...
    WHERE table_schema = 'public'
      ...
      AND column_name = 'destination_text'
  ) INTO v_exists;
  IF NOT v_exists THEN
    RAISE EXCEPTION 'ORCH-0950 expanded verification failed: events.destination_text column missing';
  END IF;

  SELECT EXISTS (
    SELECT 1
    FROM pg_proc
    WHERE proname = 'biz_trip_tickets_sold_by_tier'
      AND pronamespace = 'public'::regnamespace
      AND pronargs = 1
  ) INTO v_exists;
  IF NOT v_exists THEN
    RAISE EXCEPTION 'ORCH-0950 expanded verification failed: biz_trip_tickets_sold_by_tier(uuid) missing';
  END IF;
  ...
  IF v_update_source IS NULL
     ...
     OR position('p_patch := p_patch #- ''{theme,business_trip,capacity}''' in v_update_source) = 0
     OR position('UPDATE public.event_dates' in v_update_source) = 0
     OR position('SET destination_text' in v_update_source) = 0
     OR position('jsonb_set' in v_update_source) = 0
     OR position('''{business_trip}''' in v_update_source) = 0 THEN
    RAISE EXCEPTION 'ORCH-0950 expanded verification failed: biz_update_live_trip canonical reroute/deep merge missing';
  END IF;
  ...
  IF v_publish_source IS NULL
     ...
     OR position('mingla.business_publish_event_draft' in v_publish_source) = 0
     OR position('destination_text = v_destination_text' in v_publish_source) = 0
     OR position('#- ''{business_trip,destinationLocationText}''' in v_publish_source) = 0
     OR position('#- ''{business_trip,startAt}''' in v_publish_source) = 0 THEN
    RAISE EXCEPTION 'ORCH-0950 expanded verification failed: business_publish_trip_draft canonical destination/date strip missing';
  END IF;

  RAISE NOTICE 'ORCH-0950 expanded migration complete: trip capacity/dates/destination canonicalized';
END $$;
