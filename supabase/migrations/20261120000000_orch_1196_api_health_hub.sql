-- ORCH-1196 — Admin API-Health Hub + Email Alerts.
--
...
-- satisfies I-PROPOSED-1196-SERVICE-KEY-CANONICAL with no pseudo-row to filter.
--
...
-- (Invariant I-PROPOSED-1196-SERVICE-KEY-CANONICAL — real services only.)
...
DO $$ BEGIN
  ...
  IF (SELECT count(*) FROM public.api_health_services) <> 25 THEN
    RAISE EXCEPTION 'ORCH-1196 verify: expected 25 monitored services (no pseudo-rows)';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.api_health_meta WHERE key='last_digest_at') THEN
    RAISE EXCEPTION 'ORCH-1196 verify: api_health_meta last_digest_at row missing';
  END IF;
END $$;
