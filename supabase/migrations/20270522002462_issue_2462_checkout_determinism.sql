-- ===========================================================================
-- issue #2462 — FREE TICKET CHECKOUT IS NOT DETERMINISTIC (Batch 1, server half)
--
-- Three defects, all proven by execution against production inside aborted
-- transactions. Full evidence + probe transcripts on the issue.
--
--  §A  THE ORGANISER'S PURCHASE RULES REACH THE CART.
--      `pg_direct_event_checkout_bundle` is the FIRST reader consulted by both
--      getPublicEventBySlug and getPublicEventById, and it has never returned
--      min_purchase_qty / max_purchase_qty / allow_transfers. The client mapper
--      `directBundleTicketToStub` therefore hardcodes `1 / null / true` — it has
--      nothing to map. The quantity stepper clamps to
--      `min(remaining, maxPurchaseQty ?? Infinity)`, so a null cap means NO cap:
--      on We Go Again Exhibition the stepper offered up to 229 on a ticket type
--      the organiser capped at 1. The server then refused with
--      `ticket_quantity_above_max`, which the free-rail mapper renders as
--      "We could not reserve your free ticket. Nothing was reserved — please try
--      again." Retrying can never succeed. Verified by probe:
--        2 x "Day 2 Admission" (max_purchase_qty=1) -> RAISED ticket_quantity_above_max
--
--  §B  ONE OWNER FOR CAPACITY.
--      The `remaining` this reader publishes counted only issued tickets; the
--      capacity guard also counts in-flight holds. Verified by probe: 5 holds
--      moved the guard by 5 and moved the published number by 0 (229 -> 229).
--      Under load the guest reads "N available" and is refused as sold out.
--
--  §C  DETERMINISTIC LOCK ORDER.
--      Pass 1 of the create-session base locks ticket_types FOR UPDATE in the
--      order the client sent the lines; the cart appends in buyer tap order.
--      Opposite orders + concurrency = deadlock, killed txn, and the same
--      "Nothing was reserved" sentence. Verified by probe: lines sent
--      [Day 2, Day 1] were processed [Day 2, Day 1].
--
--  §D  The supporting index the capacity count has never had.
--
-- NO BEHAVIOUR OUTSIDE THESE FOUR POINTS CHANGES. Both functions are re-emitted
-- from their #2160 definer text with only the marked edits.
-- ===========================================================================

-- ---------------------------------------------------------------------------
-- §D — the index. `EXPLAIN (ANALYZE, BUFFERS)` on the capacity count showed
-- `Seq Scan on tickets` — a full scan of the PLATFORM-WIDE tickets table, run
-- while holding the ticket_type row lock, once per reservation. Cost is O(total
-- tickets), so the serialized critical section GROWS as the platform grows;
-- measured at 2.89 ms/reservation with 203 rows in the table.
-- CONCURRENTLY is deliberately NOT used: this migration runs in a transaction.
-- ---------------------------------------------------------------------------
CREATE INDEX IF NOT EXISTS idx_tickets_ticket_type_status
...
$$;
-- ---------------------------------------------------------------------------
-- POST-MIGRATION PROBES — read-only, and they RAISE rather than warn, so a
-- silently-degraded deploy cannot report success (the #2113 lesson).
-- ---------------------------------------------------------------------------
DO $probe$
DECLARE v_def text; v_missing text := '';
BEGIN
  IF to_regclass('public.idx_tickets_ticket_type_status') IS NULL THEN
    RAISE EXCEPTION 'issue #2462 probe: idx_tickets_ticket_type_status missing post-migration';
  END IF;

  SELECT pg_get_functiondef(p.oid) INTO v_def
    FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = 'pg_direct_event_checkout_bundle';
  IF position('''minPurchaseQty''' IN v_def) = 0 THEN v_missing := v_missing || 'minPurchaseQty '; END IF;
  IF position('''maxPurchaseQty''' IN v_def) = 0 THEN v_missing := v_missing || 'maxPurchaseQty '; END IF;
  IF position('''allowTransfers''' IN v_def) = 0 THEN v_missing := v_missing || 'allowTransfers '; END IF;
  IF position('processing_payment' IN v_def) = 0 THEN v_missing := v_missing || 'inflight-holds-in-remaining '; END IF;
  IF v_missing <> '' THEN
    RAISE EXCEPTION 'issue #2462 probe: bundle reader is missing: %', v_missing;
  END IF;
  ...
  IF position('ORDER BY tt.id' IN v_def) = 0 THEN
    RAISE EXCEPTION 'issue #2462 probe: canonical lock pre-pass absent — deadlock fix not applied';
  END IF;
END
$probe$;
