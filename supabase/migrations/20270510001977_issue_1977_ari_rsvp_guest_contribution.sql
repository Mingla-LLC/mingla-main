...
-- #1977 forward-integrates the current certification contract. The historical
-- #2060/#1973 migrations remain immutable; this migration replaces the
-- duplicate guest-approval requirement with the two distinct RSVP capabilities
-- now exposed by the canonical registry and promotes contribution settings to
-- its implemented write mode.
...
BEGIN
  ...
  IF v_count <> 118 THEN
    RAISE EXCEPTION 'issue_1977_expected_118_certification_requirements:%', v_count;
  END IF;
  IF EXISTS (
    SELECT 1
    FROM (VALUES
...
-- #1977 forward-replaces the current certifier finalizer explicitly. Keeping
-- the full static definition makes every DDL effect auditable by the repository
-- SQL gates while preserving #1973's historical migration snapshot.
...
BEGIN
  ...
  END IF;
  IF v_capability_count <> 118 THEN RAISE EXCEPTION 'ari_cert_missing_capabilities:%', v_capability_count; END IF;
  IF v_run.requirements_digest <> 'bac1588dd5d65fd2accdbaebfc7168fd2d682b41c9a253f98e1b3afd97d3dab6' THEN
