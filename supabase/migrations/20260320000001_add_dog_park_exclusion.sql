-- Migration: add_dog_park_exclusion
-- Adds dog_park to the globally excluded place types in query_pool_cards.
-- Must match GLOBAL_EXCLUDED_PLACE_TYPES in categoryPlaceTypes.ts: ['gym', 'fitness_center', 'dog_park']

-- ═══════════════════════════════════════════════════════════════════════════════
-- Part 1: Deactivate existing dog_park cards in the pool
-- ═══════════════════════════════════════════════════════════════════════════════
...
WHERE is_active = true
  ...
  AND EXISTS (
    ...
    WHERE pp.id = card_pool.place_pool_id
      AND pp.types && ARRAY['dog_park']
  );

-- ═══════════════════════════════════════════════════════════════════════════════
-- Part 2: Rewrite query_pool_cards with dog_park added to exclusion list
-- ═══════════════════════════════════════════════════════════════════════════════
