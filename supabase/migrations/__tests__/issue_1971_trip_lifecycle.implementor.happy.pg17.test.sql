...
-- hypothetical here; the fails-on-revert run for the destination theme-authoring
-- fix passed against reverted code until this file was corrected.
--
...
BEGIN
  ...
  IF (g#>>'{event,status}') IS DISTINCT FROM 'draft' THEN RAISE EXCEPTION 'A-02 created trip is not a draft'; END IF;
  IF (g#>>'{event,event_type}') IS DISTINCT FROM 'trip' THEN RAISE EXCEPTION 'A-03 created row is not a trip'; END IF;
  IF (g#>>'{event,title}') IS DISTINCT FROM 'Lagos December' THEN RAISE EXCEPTION 'A-04 seed title was dropped'; END IF;
  IF (g#>>'{event,timezone}') IS DISTINCT FROM 'Africa/Lagos' THEN RAISE EXCEPTION 'A-05 seed timezone was dropped'; END IF;
  -- The brand's currency is derived server-side and never invented.
  IF (g#>>'{event,currency}') IS DISTINCT FROM 'GBP' THEN RAISE EXCEPTION 'A-06 brand currency was not derived, got %', g#>>'{event,currency}'; END IF;
  IF jsonb_array_length(g->'tiers') IS DISTINCT FROM 1 THEN RAISE EXCEPTION 'A-07 placeholder tier missing (%)', jsonb_array_length(g->'tiers'); END IF;
  IF (g#>>'{tiers,0,ticket_type,name}') IS DISTINCT FROM 'Standard' THEN RAISE EXCEPTION 'A-08 placeholder ticket missing'; END IF;
...
BEGIN
  ...
  IF g IS NULL THEN RAISE EXCEPTION 'B-00 the draft graph patch returned nothing'; END IF;
  IF jsonb_array_length(g->'days') IS DISTINCT FROM 2 THEN RAISE EXCEPTION 'B-01 days did not round-trip'; END IF;
  IF (g#>>'{days,0,media,0,url}') IS DISTINCT FROM 'https://cdn.example/1.jpg' THEN
    RAISE EXCEPTION 'B-02 ORCH-1119 per-day media was dropped';
  END IF;
  IF jsonb_array_length(g->'inclusions') IS DISTINCT FROM 2 THEN RAISE EXCEPTION 'B-03 inclusions did not round-trip'; END IF;
  IF (g#>>'{event,description}') IS DISTINCT FROM 'Seven nights' THEN RAISE EXCEPTION 'B-04 basics did not round-trip'; END IF;
  IF (g#>>'{event,pass_tax}') IS DISTINCT FROM 'true' THEN RAISE EXCEPTION 'B-05 settings did not round-trip'; END IF;
  ...
  IF (g#>>'{event,theme,business_trip,destinationLocationText}') IS DISTINCT FROM 'Lagos, Nigeria' THEN
    RAISE EXCEPTION 'B-06 destination was not authored into the theme';
  END IF;
  IF (g#>>'{event,departure_text}') IS DISTINCT FROM 'London, UK' THEN
...
--
--    EACH CASE ISOLATES ONE GUARD. That is not pedantry: the obvious fixture
--    (deposit 101% with a 50% instalment) sums to 151 and is rejected by the
--    TOTAL check, so deleting the deposit bound leaves it green — the guard
--    would look proven and be unproven. Every case below therefore satisfies
--    every OTHER rule, and each carries a sanity assertion showing the same
--    schedule with the single offending value corrected is accepted.
-- ---------------------------------------------------------------------------
DO $$
DECLARE v_event uuid; v_rev timestamptz; v_ticket uuid; g jsonb; v_sqlstate text;
BEGIN
  SELECT id, updated_at INTO v_event, v_rev FROM public.events
   WHERE brand_id = '19710000-0000-4000-8000-000000000010' AND event_type = 'trip';
  SELECT ticket_type_id INTO v_ticket FROM public.trip_pricing_tiers WHERE event_id = v_event LIMIT 1;

  -- D-01 deposit ABOVE 100. Deposit 100.005 + one 0.001% instalment totals
  -- 100.006, which is INSIDE the 0.01 tolerance, so the total check cannot fire
  -- and only `v_deposit_pct > 100` can reject this.
  BEGIN
    PERFORM public.biz_apply_trip_draft_graph(
      v_event,
      jsonb_build_object('tiers', jsonb_build_array(jsonb_build_object(
        ...
        'tier_metadata', jsonb_build_object('installments', jsonb_build_object(
          ...
          'installments', jsonb_build_array(jsonb_build_object(
            'ordinal', 1, 'pct', 0.001, 'days_after_booking', 30))))))),
      v_rev, '19710000-0000-4000-8000-0000000000b1');
    RAISE EXCEPTION 'D-01 a deposit above 100 was accepted';
  EXCEPTION WHEN sqlstate '22023' THEN
    IF SQLERRM <> 'installment_deposit_pct_out_of_range' THEN
      RAISE EXCEPTION 'D-01 rejected for the wrong reason: %', SQLERRM;
    END IF;
  END;

  -- D-02 deposit AT OR BELOW 0. 0 + 50 + 50 totals exactly 100 and every
  -- instalment is in range, so only `v_deposit_pct <= 0` can reject it.
  BEGIN
    PERFORM public.biz_apply_trip_draft_graph(
      v_event,
      jsonb_build_object('tiers', jsonb_build_array(jsonb_build_object(
        ...
        'tier_metadata', jsonb_build_object('installments', jsonb_build_object(
          ...
          'installments', jsonb_build_array(
            jsonb_build_object('ordinal', 1, 'pct', 50, 'days_after_booking', 30),
            jsonb_build_object('ordinal', 2, 'pct', 50, 'days_after_booking', 60))))))),
      v_rev, '19710000-0000-4000-8000-0000000000b2');
    RAISE EXCEPTION 'D-02 a zero deposit was accepted';
  EXCEPTION WHEN sqlstate '22023' THEN
    IF SQLERRM <> 'installment_deposit_pct_out_of_range' THEN
      RAISE EXCEPTION 'D-02 rejected for the wrong reason: %', SQLERRM;
    END IF;
  END;

  -- D-03 a NON-POSITIVE instalment. 50 + 60 + (-10) totals exactly 100 and the
  -- deposit is in range, so only `v_installment_pct <= 0` can reject it. This is
  -- the shape independent QA found persisted: `pct = -50`.
  BEGIN
    PERFORM public.biz_apply_trip_draft_graph(
      v_event,
      jsonb_build_object('tiers', jsonb_build_array(jsonb_build_object(
        ...
        'tier_metadata', jsonb_build_object('installments', jsonb_build_object(
          'deposit_pct', 50,
          'installments', jsonb_build_array(
            jsonb_build_object('ordinal', 1, 'pct', 60, 'days_after_booking', 30),
            jsonb_build_object('ordinal', 2, 'pct', -10, 'days_after_booking', 60))))))),
      v_rev, '19710000-0000-4000-8000-0000000000b3');
    RAISE EXCEPTION 'D-03 a negative instalment was accepted';
  EXCEPTION WHEN sqlstate '22023' THEN
    IF SQLERRM <> 'installment_pct_out_of_range' THEN
      RAISE EXCEPTION 'D-03 rejected for the wrong reason: %', SQLERRM;
    END IF;
  END;

  -- D-04 a schedule that does not total 100, with every individual value in
  -- range — so only the total check can reject it.
  BEGIN
    PERFORM public.biz_apply_trip_draft_graph(
      v_event,
      jsonb_build_object('tiers', jsonb_build_array(jsonb_build_object(
    ...
    RAISE EXCEPTION 'D-04 a schedule that does not total 100 was accepted';
  EXCEPTION WHEN sqlstate '22023' THEN
    IF SQLERRM <> 'installment_pct_sum_mismatch' THEN
      RAISE EXCEPTION 'D-04 rejected for the wrong reason: %', SQLERRM;
    END IF;
  END;

  -- D-05 an out-of-order ordinal, everything else valid.
  BEGIN
    PERFORM public.biz_apply_trip_draft_graph(
      v_event,
      jsonb_build_object('tiers', jsonb_build_array(jsonb_build_object(
        ...
        'tier_metadata', jsonb_build_object('installments', jsonb_build_object(
          'deposit_pct', 20,
          'installments', jsonb_build_array(
            jsonb_build_object('ordinal', 2, 'pct', 40, 'days_after_booking', 30),
            jsonb_build_object('ordinal', 1, 'pct', 40, 'days_after_booking', 60))))))),
      v_rev, '19710000-0000-4000-8000-0000000000b5');
    RAISE EXCEPTION 'D-05 out-of-order instalments were accepted';
  EXCEPTION WHEN sqlstate '22023' THEN
    IF SQLERRM <> 'installment_ordinal_invalid' THEN
      RAISE EXCEPTION 'D-05 rejected for the wrong reason: %', SQLERRM;
    END IF;
  END;

  -- D-06 two due modes on one instalment, everything else valid.
  BEGIN
    PERFORM public.biz_apply_trip_draft_graph(
      v_event,
      jsonb_build_object('tiers', jsonb_build_array(jsonb_build_object(
        ...
        'tier_metadata', jsonb_build_object('installments', jsonb_build_object(
          ...
          'installments', jsonb_build_array(jsonb_build_object(
            'ordinal', 1, 'pct', 60,
            'days_after_booking', 30, 'fixed_date', '2027-06-01'))))))),
      v_rev, '19710000-0000-4000-8000-0000000000b6');
    RAISE EXCEPTION 'D-06 an instalment with two due modes was accepted';
  EXCEPTION WHEN sqlstate '22023' THEN
    IF SQLERRM <> 'installment_due_mode_invalid' THEN
      RAISE EXCEPTION 'D-06 rejected for the wrong reason: %', SQLERRM;
    END IF;
  END;
  ...
  -- D-08 the sanity control. The SAME shape as D-01..D-06 with every value in
  -- range is ACCEPTED and persists exactly, so none of the rejections above can
  -- be an artefact of a fixture the command would refuse anyway.
  g := public.biz_apply_trip_draft_graph(
    v_event,
    jsonb_build_object('tiers', jsonb_build_array(jsonb_build_object(
      'ticket_type_id', v_ticket,
    ...
    v_rev, '19710000-0000-4000-8000-0000000000b9');

  IF g IS NULL THEN RAISE EXCEPTION 'D-08b the valid schedule patch returned nothing'; END IF;
  IF (g#>>'{tiers,0,tier_metadata,installments,deposit_pct}') IS DISTINCT FROM '30' THEN
    RAISE EXCEPTION 'D-09 a valid 30/70 schedule did not persist, got %',
      g#>>'{tiers,0,tier_metadata,installments,deposit_pct}';
  END IF;
  IF (g#>>'{tiers,0,ticket_type,quantity_total}') IS DISTINCT FROM '12' THEN
    RAISE EXCEPTION 'D-10 tier capacity did not persist';
  END IF;
END $$;
...
BEGIN
  ...
  SELECT status INTO v_status FROM public.events WHERE id = v_event;
  -- NULL-safe: `NULL NOT IN (...)` is NULL, and `IF NULL THEN` does nothing.
  ...
  IF (v_result#>>'{graph,event,destination_text}') IS DISTINCT FROM 'Lagos, Nigeria' THEN
...
BEGIN
  ...
  END IF;
  IF (SELECT title FROM public.events WHERE id = v_event) IS DISTINCT FROM 'Lagos December (revised)' THEN
...
BEGIN
  ...
  IF (v_result#>>'{event,event_type}') IS DISTINCT FROM 'trip' THEN RAISE EXCEPTION 'J-02 the Ari executor made a non-trip'; END IF;
...
BEGIN
  ...
  IF v_result IS NULL THEN RAISE EXCEPTION 'K-00 manage_trip_days returned nothing'; END IF;
  IF jsonb_array_length(v_result->'days') IS DISTINCT FROM 3 THEN
