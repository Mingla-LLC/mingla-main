...
const path =
  "supabase/migrations/20270502001971_issue_1971_ari_trip_lifecycle.sql";
...
Deno.test("#1971 Ari consumes the shared #1972 terminal receipt without forking it", () => {
  assertStringIncludes(sql, "FUNCTION public.ari_execute_trip_operation(");
...
Deno.test("#1971 draft/live/publish/delete commands enforce optimistic concurrency", () => {
  ...
  assert(
    (sql.match(/IS DISTINCT FROM p_expected_updated_at/g) ?? []).length >= 4,
  );
});

Deno.test("#1971 live updates retain the bounded audit reason contract", () => {
  assertStringIncludes(sql, "trip_update_reason_invalid");
  assertStringIncludes(
    sql,
    "char_length(btrim(COALESCE(p_reason,''))) NOT BETWEEN 10 AND 200",
  );
});
...
Deno.test("#1971 sidecar writers require event_manager and money read returns aggregates only", () => {
  for (
    const policy of [
      "trip_days_write_event_managers",
      "trip_inclusions_write_event_managers",
      "trip_pricing_tiers_write_event_managers",
    ]
  ) {
