...
const MIG = "supabase/migrations/20270303001734_issue_1734_tool_leads_app_lane.sql";
...
Deno.test("T-M1.1 — the 6 additive tool_leads columns", () => {
  ...
  for (const col of ["user_id uuid", "brand_id uuid", "client_ref uuid", "input_hash text", "subject_ref text"]) {
    assert(
      new RegExp(`ADD COLUMN IF NOT EXISTS ${col.split(" ")[0]} ${col.split(" ")[1]}`).test(src),
      `column ${col} added`,
    );
  }
});
...
Deno.test("T-M1.2 — the 3 named CHECKs inside pg_constraint idempotency guards", () => {
  for (const name of ["tool_leads_lane_check", "tool_leads_lane_identity_check", "tool_leads_subject_ref_lane_check"]) {
    assert(
  ...
  assert(src.includes("(lane = 'web' AND user_id IS NULL AND brand_id IS NULL)"));
  assert(src.includes("(lane = 'app' AND user_id IS NOT NULL AND brand_id IS NOT NULL)"));
...
Deno.test("T-M1.3 — the 4 partial indexes", () => {
  assert(/idx_tool_leads_app_quota[\s\S]*?\(brand_id, tool, created_at DESC\) WHERE lane = 'app'/.test(src));
  assert(/idx_tool_leads_app_cache[\s\S]*?\(brand_id, tool, input_hash, created_at DESC\) WHERE lane = 'app'/.test(src));
  assert(/idx_tool_leads_app_client_ref[\s\S]*?\(client_ref\) WHERE client_ref IS NOT NULL/.test(src));
  assert(/idx_tool_leads_app_subject[\s\S]*?\(brand_id, tool, subject_ref, created_at DESC\)\s*\n?\s*WHERE lane = 'app' AND subject_ref IS NOT NULL/.test(src));
});
...
Deno.test("T-CW2 (structural) — tool_competitors: FKs, CHECKs, deny-all RLS, dedup index, cap trigger", () => {
  ...
  assert(/brand_id\s+uuid NOT NULL REFERENCES public\.brands\(id\) ON DELETE CASCADE/.test(src));
  assert(/venue_listing_id uuid NOT NULL REFERENCES public\.venue_listings\(id\) ON DELETE CASCADE/.test(src));
  assert(/place_pool_id\s+uuid NULL REFERENCES public\.place_pool\(id\) ON DELETE SET NULL/.test(src));
  ...
  assert(src.includes("ALTER TABLE public.tool_competitors ENABLE ROW LEVEL SECURITY"));
  assertEquals(/CREATE POLICY/i.test(src), false, "zero policies — deny-all (P-46)");
  // Structural dedup.
  assert(/CREATE UNIQUE INDEX IF NOT EXISTS uq_tool_competitors_venue_site[\s\S]*?\(venue_listing_id, lower\(btrim\(website\)\)\)/.test(src));
  // Cap trigger: FOR UPDATE serialization, cap 5, brand consistency.
  const trigger = src.match(/CREATE OR REPLACE FUNCTION public\.tool_competitors_cap_guard\(\)[\s\S]*?\$\$;/)?.[0] ?? "";
  assert(trigger.length > 0, "cap guard function present");
  assert(trigger.includes("FOR UPDATE"), "per-venue serialization — the cap cannot be raced past");
  ...
  assert(/CREATE TRIGGER trg_tool_competitors_cap_guard\s*\n?\s*BEFORE INSERT ON public\.tool_competitors/.test(src));
});

function fnBody(name: string): string {
  const m = src.match(new RegExp(`create function public\\.${name}\\([\\s\\S]*?\\$\\$;`, "i"));
...
Deno.test("T-L1 — admin_tool_leads_list: 6-arg signature, p_lane filter, lane appended LAST, guard-first, no PII columns", () => {
  ...
  assert(body.includes("p_lane      text    default null"), "p_lane parameter, defaulted");
  assert(body.includes("(p_lane is null or tl.lane = p_lane)"), "house filter shape — null = All");
  ...
  assertEquals(cols[cols.length - 1], "lane", "lane is the LAST output column (positional safety)");
  // Never selects the PII/secret columns.
  for (const forbidden of ["report_token", "ip_hash"]) {
    assertEquals(body.includes(forbidden), false, `list body must not mention ${forbidden}`);
  }
  assert(!/tl\.report(?!\s+is)/.test(body), "list never returns the report jsonb (only `report is not null`)");
  assert(GUARD_RE.test(firstStatement(body)), "is_admin_user() guard is the FIRST executable statement");
});
...
Deno.test("T-L2 — admin_tool_lead_get: gains lane/brand_id/user_id/subject_ref appended last; never ip_hash; guard-first", () => {
  ...
  const returns = body.match(/returns table\(([\s\S]*?)\)/i)![1];
  const cols = returns.split(",").map((c) => c.trim().split(/\s+/)[0]);
  assertEquals(cols.slice(-4), ["lane", "brand_id", "user_id", "subject_ref"], "appended LAST, in order (P-50)");
...
Deno.test("P-50 lockdown — drop old signatures, revoke public+anon, grant authenticated, self-asserts on the NEW 6-arg signature", () => {
  assert(src.includes("drop function if exists public.admin_tool_leads_list(text, boolean, text, int, int);"));
  assert(src.includes("drop function if exists public.admin_tool_leads_list(text, boolean, text, int, int, text);"));
  assert(src.includes("drop function if exists public.admin_tool_lead_get(uuid);"));
  const sig = "public.admin_tool_leads_list(text, boolean, text, int, int, text)";
  ...
  assert(src.includes(`has_function_privilege('anon', '${sig}', 'EXECUTE')`), "anon self-assert on the new signature");
  assert(src.includes("has_function_privilege('authenticated', 'public.admin_tool_lead_get(uuid)', 'EXECUTE')"));
});
...
Deno.test("safe-migration — ROLLBACK block covers every object; additive only", () => {
  ...
  assertEquals(/DROP TABLE/i.test(executable), false, "no executable DROP TABLE");
  assertEquals(/DROP COLUMN/i.test(executable), false, "no executable DROP COLUMN");
});
