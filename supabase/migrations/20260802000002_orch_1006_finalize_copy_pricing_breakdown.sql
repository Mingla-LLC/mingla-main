-- ORCH-1006 [Universal all-in pricing engine] — finalize copies pricing_breakdown
-- ===================================================================
-- Slice 2 persists `ticket_checkout_sessions.pricing_breakdown` (jsonb) on the
-- session row at session create. The finalize RPC (biz_ticket_checkout_finalize,
-- current live definition = ORCH-0921 20260724000000) creates the `orders` row
-- but did NOT copy that breakdown onto it — so receipts / refunds / absorbed-cost
-- reporting had no authoritative per-order money snapshot to read.
--
-- This migration re-declares biz_ticket_checkout_finalize(uuid, text, text, text,
-- text, text, text, boolean) VERBATIM from 20260724000000_orch_0921_finalize_
-- compare_and_correct.sql, changing ONLY the order INSERT to also write
-- `pricing_breakdown` from the session ROWTYPE (`v_session.pricing_breakdown`,
-- which is loaded via `SELECT * ... FOR UPDATE` exactly like every other session
-- value already copied into the order). No signature, return type, security
-- context, search_path, grants, installment logic, ticket-minting, chat, or
-- notification logic is altered.
--
-- Collision-checked 2026-05-30: highest migration prefix across all active
-- worktrees + main = 20260802000001 (ORCH-1006 pricing_views); this uses
-- 20260802000002 (strictly greater, unused everywhere).
--
...
BEGIN
  ...
  IF (SELECT COUNT(*) FROM pg_proc WHERE proname = 'biz_ticket_checkout_finalize' AND pronargs = 8) <> 1 THEN
    RAISE EXCEPTION 'ORCH-1006 self-verify: expected exactly 1 biz_ticket_checkout_finalize overload with 8 params';
  END IF;
END$$;
