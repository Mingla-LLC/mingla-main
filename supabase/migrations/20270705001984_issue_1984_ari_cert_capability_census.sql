-- Issue #1984 — re-pin Ari certification capability census after analytics
-- listing conversion + reservation metrics tools. Adds two ledger capability
-- rows (138 → 140) and restores Pass-5 finalize_run count gate to the live
-- ledger size.
-- MONOTONIC VERSION 20270705001984 (after tip 20270704001982 / #1982 on disk).
...
VALUES
  ('ari.analytics.listing_conversion', 'read'),
  ('ari.analytics.reservation_metrics', 'read')
...
BEGIN
  ...
  END IF;
  IF v_capability_count <> 140 THEN RAISE EXCEPTION 'ari_cert_missing_capabilities:%', v_capability_count; END IF;
...
COMMENT ON FUNCTION public.ari_cert_begin_run(text, jsonb, text, jsonb, jsonb) IS
  'Issue #2060 Pass-5 post-#1984: stamps private.ari_cert_requirements_set_digest_v1() of the live 140-row requirement set.';
COMMENT ON FUNCTION public.ari_cert_finalize_run(uuid) IS
  'Issue #2060 Pass-5 post-#1984: finalizes only when v_run.requirements_digest equals a fresh private.ari_cert_requirements_set_digest_v1().';
