...
CREATE TABLE IF NOT EXISTS public.venue_identity_correction_audit (
  ...
  corrected_place_pool_id uuid NOT NULL,
  actor_uid uuid NOT NULL,
  actor_kind text NOT NULL CHECK (actor_kind IN ('brand_owner','admin')),
  action text NOT NULL CHECK (action IN ('forward','rollback')),
  source_audit_id uuid REFERENCES public.venue_identity_correction_audit(id),
  ...
  CHECK ((action='forward' AND source_audit_id IS NULL) OR (action='rollback' AND source_audit_id IS NOT NULL))
);
ALTER TABLE public.venue_identity_correction_audit OWNER TO postgres;
ALTER TABLE public.venue_identity_correction_audit ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.venue_identity_correction_audit FROM PUBLIC, anon, authenticated, service_role;

CREATE OR REPLACE FUNCTION public.issue_2099_reject_audit_mutation()
RETURNS trigger LANGUAGE plpgsql SET search_path TO 'public','pg_temp' AS $$
BEGIN
  RAISE EXCEPTION USING ERRCODE='42501', MESSAGE='venue_identity_correction_audit_is_immutable';
END $$;
...
CREATE OR REPLACE FUNCTION public.issue_2099_dependency_ddl_guard()
...
AS $$
BEGIN
  LOCK TABLE public.issue_2099_dependency_schema_guard IN EXCLUSIVE MODE;
END $$;
...
BEGIN
  FOR r IN
    WITH semantic AS (
      SELECT c.oid relid, n.nspname, c.relname, a.attname,
      ...
      SELECT src.oid relid, ns.nspname, src.relname, sa.attname,
      ...
      SELECT relid,nspname,relname,attname,target,string_agg(source,',' ORDER BY source) sources
      FROM (SELECT * FROM semantic UNION ALL SELECT * FROM fk) x
      GROUP BY relid,nspname,relname,attname,target
    )
    SELECT relid,nspname,relname,
      string_agg(format('%I = $1',attname),' OR ' ORDER BY attname) FILTER (WHERE target='venue') venue_pred,
      string_agg(format('%I = $2',attname),' OR ' ORDER BY attname) FILTER (WHERE target='pool') pool_pred,
      string_agg(relname||'|'||attname||'|'||target||'|'||sources,E'\n' ORDER BY attname,target) schema_lines
    FROM lanes GROUP BY relid,nspname,relname ORDER BY nspname,relname
  LOOP
    v_schema_lines := v_schema_lines || CASE WHEN v_schema_lines='' THEN '' ELSE E'\n' END || r.schema_lines;
    v_where := concat_ws(' OR ',
      CASE WHEN r.venue_pred IS NOT NULL THEN '('||r.venue_pred||')' END,
      CASE WHEN r.pool_pred IS NOT NULL THEN '('||r.pool_pred||')' END);
    IF r.relname='venue_listings' THEN
      v_where := '('||v_where||') AND id <> $1';
    END IF;
    EXECUTE format('SELECT count(*) FROM %I.%I WHERE %s',r.nspname,r.relname,v_where)
      INTO v_count USING p_venue_id,p_place_pool_id;
    v_classification := CASE
      WHEN r.relname IN ('brand_hours','brand_place_pipeline_state','venue_availability_config','venue_reservation_settings') THEN 'allowed_baseline'
      WHEN r.relname='venue_listings' THEN 'identity_self'
      ELSE 'disallowed' END;
    v_label := CASE r.relname
      WHEN 'brand_hours' THEN 'required_hours'
      WHEN 'brand_place_pipeline_state' THEN 'required_pipeline'
      WHEN 'venue_availability_config' THEN 'required_availability'
      WHEN 'venue_reservation_settings' THEN 'required_reservation_settings'
      WHEN 'venue_listings' THEN 'other_venue_identity'
      ELSE 'dependency' END;
    v_lanes := v_lanes || jsonb_build_array(jsonb_build_object(
      'safe_label',v_label,'count',v_count,'classification',v_classification));
  END LOOP;

  v_schema_fingerprint := encode(extensions.digest(v_schema_lines,'sha256'),'hex');
  SELECT encode(extensions.digest(jsonb_build_object(
      'venue',(SELECT to_jsonb(v) FROM public.venue_listings v WHERE v.id=p_venue_id),
      'pool',(SELECT to_jsonb(p) FROM public.place_pool p WHERE p.id=p_place_pool_id),
      'hours',COALESCE((SELECT jsonb_agg(to_jsonb(h) ORDER BY h.weekday,h.id) FROM public.brand_hours h WHERE h.venue_id=p_venue_id),'[]'::jsonb),
      'pipeline',(SELECT to_jsonb(s) FROM public.brand_place_pipeline_state s WHERE s.venue_id=p_venue_id),
      'availability',(SELECT to_jsonb(a) FROM public.venue_availability_config a WHERE a.venue_id=p_venue_id),
      'reservation_settings',(SELECT to_jsonb(s) FROM public.venue_reservation_settings s WHERE s.venue_id=p_venue_id),
      'lanes',v_lanes)::text,'sha256'),'hex') INTO v_state_fingerprint;
  RETURN jsonb_build_object('schema_fingerprint',v_schema_fingerprint,
    'state_fingerprint',v_state_fingerprint,'lanes',v_lanes);
END $$;
ALTER FUNCTION public.issue_2099_pending_venue_dependency_inventory(uuid,uuid) OWNER TO postgres;
REVOKE ALL ON FUNCTION public.issue_2099_pending_venue_dependency_inventory(uuid,uuid) FROM PUBLIC, anon, authenticated, service_role;
