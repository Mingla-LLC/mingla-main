-- =====================================================================================
-- Issue #2333 — S1. An ONLINE-ONLY event could not be published.
--
-- `business_publish_event_draft` raised `city_required` unconditionally, but the
-- wizard renders NO field that can set a city for an online event: the only writer
-- of `draft.city` is the MapboxAddressInput block, and CreatorStep3Where.tsx:138-140
-- gates it behind `format === "in_person" || format === "hybrid"`. The client's
-- `validateWhere` (draftEventValidation.ts:380-423) therefore never asks for a city
-- on the online branch and `computePublishability` returns publishable — so the app
-- said "Ready to publish" and the server refused, forever. Constitution #2 (one owner
-- per truth): the client contract is the correct one; the server held the wrong truth.
--
-- THE TRAP THIS MIGRATION DELIBERATELY AVOIDS: `is_online` is NOT the key. It is
-- written `draft.format === "online" || draft.format === "hybrid"`
-- (mingla-business/src/utils/serverDraftEventMapper.ts:708), so keying the relax on
-- `p_draft_payload->>'is_online'` would ALSO exempt HYBRID — a format for which the
-- client REQUIRES a venue name, address and city (validateWhere:382-406). That is the
-- same client/server divergence that produced this bug, pointing the other way. The
-- guard keys on `business_draft.format = 'online'` and nothing else, and an absent or
-- unknown format FAILS CLOSED (still demands a city).
...
-- byte-for-byte from the LIVE production definition (`pg_get_functiondef`, project
-- gqnoajqerqhnvulmnyvv, read-only) rather than from a repo migration. The repo copy in
-- 20270413001931_issue_1931_private_event_access.sql was diffed against prod and found
-- IDENTICAL (no drift), and the candidate below diffs against prod in exactly three
-- hunks: the `v_format` declare, the `v_format :=` read, and the guard + its comment.
--
-- Signature, RETURNS jsonb, LANGUAGE plpgsql, SECURITY DEFINER and
-- SET search_path TO 'public','pg_temp' are unchanged, so NO `DROP FUNCTION` is needed
-- and the existing ACL (postgres/authenticated/service_role EXECUTE) and the #1653
-- COMMENT both survive CREATE OR REPLACE untouched.
--
-- NO schema change: `events.city` stays nullable with no CHECK, no NOT NULL and no
-- city-dependent trigger. `city = v_city` in the UPDATE already writes NULL correctly
-- and the `- 'city'` theme strip is unchanged; nothing downstream reads `v_city`.
--
-- Ordering floor: strictly greater than 20270424002267 (origin/main head) and than
-- 20270426002305 (highest across every sibling worktree). Collision scan attached to
-- the implementation report. No historical migration is edited.
-- =====================================================================================
