...
async function generateDescriptions(places: any[]): Promise<string[]> {
  if (!OPENAI_API_KEY || places.length === 0) {
    return places.map(p =>
      `A restorative wellness venue perfect for relaxation and rejuvenation.`
    );
  }
  ...
  try {
        ...
        messages: [
          {
            ...
            content:
              ...
              'Focus on the relaxation and rejuvenation experience. Highlight treatments, atmosphere, and what makes it perfect for unwinding. ' +
    ...
    if (!resp.ok) {
      console.warn(`[discover-wellness] OpenAI error: ${resp.status}`);
      return places.map(p =>
        `A restorative wellness venue perfect for relaxation and rejuvenation.`
      );
    }
    ...
    console.warn('[discover-wellness] AI description generation failed:', err);
    return places.map(p =>
      `A restorative wellness venue perfect for relaxation and rejuvenation.`
    );
  }
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`[discover-wellness] Request: batchSeed=${batchSeed}, limit=${limit}, mode=${travelMode}`);
    ...
    if (userId && batchSeed === 0 && !body.warmPool) {
      try {
        ...
        if (poolResult.cards.length >= Math.ceil(limit * 0.75)) {
          console.log(`[discover-wellness] Served ${poolResult.cards.length} from pool (0 API calls)`);
        ...
        console.warn('[discover-wellness] Pool serve failed, falling back to API:', poolErr);
      }
    }
    ...
    console.log(`[discover-wellness] ${allPlaces.length} places after datetime filter (dateOption=${dateOption}, timeSlot=${timeSlot})`);
    ...
    console.log(`[discover-wellness] Batch ${batchSeed}: offset=${offset}, returning ${batch.length}/${totalAvailable}`);
    ...
    const cards = batch.map((p, i) => {
      ...
      const primaryType = p.primaryType || p._matchedType || 'spa';
    ...
    if (userId) {
      (async () => {
        try {
          ...
          console.warn('[discover-wellness] Pool store failed (non-critical):', e);
        }
      })();
    }

    const elapsed = Date.now() - t0;
    console.log(`[discover-wellness] Done in ${elapsed}ms: ${cards.length} cards, ${apiCallsMade} API calls`);
    ...
    console.error('[discover-wellness] Error:', err);
    return new Response(
