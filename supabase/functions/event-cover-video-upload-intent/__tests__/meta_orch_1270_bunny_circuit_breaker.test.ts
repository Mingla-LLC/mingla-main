...
Deno.test("evaluateCapacityBreaker: >= hard cap fails CLOSED", () => {
  assert(evaluateCapacityBreaker(90, 90).blocked === true, "90 >= 90 → blocked");
  assert(evaluateCapacityBreaker(95, 90).blocked === true, "95 >= 90 → blocked");
  assert(evaluateCapacityBreaker(90, 90).reason === "capacity_reached", "reason capacity_reached");
});
...
Deno.test("evaluateCapacityBreaker: unreadable usage (null) FAILS OPEN", () => {
  ...
  assert(r.reason === "usage_unreadable_fail_open", "reason documents the fail-open");
});
...
const makeRequest = (): Request =>
  new Request("https://example.test/functions/v1/event-cover-video-upload-intent", {
    ...
    headers: { Authorization: "Bearer user-session-jwt", "Content-Type": "application/json" },
    method: "POST",
  });
...
Deno.test("upload-intent: at the hard cap → 503 capacity_reached, no video created", async () => {
  await withBunnyEnv(async () => {
    ...
    const deps = {
      bunnyCreateVideo: () => {
        createCalled = true;
        return Promise.resolve({ ok: true as const, guid: "should-not-happen" });
      },
      bunnyPresignTusUpload: () => Promise.resolve({
        tusEndpoint: "x", libraryId: "x", videoId: "x", authorizationSignature: "x", authorizationExpire: 0,
      }),
      checkBunnyCapacity: () =>
        Promise.resolve({ blocked: true, reason: "capacity_reached", usedPercent: 95 }),
    ...
    const response = await handleEventCoverVideoUploadIntent(makeRequest(), deps as never);
    const body = await response.json();
    assert(response.status === 503, `expected 503, got ${response.status}: ${JSON.stringify(body)}`);
    assert(body.error === "capacity_reached", `expected error capacity_reached, got ${String(body.error)}`);
    assert(createCalled === false, "bunnyCreateVideo must NOT be called when at capacity");
  });
});
