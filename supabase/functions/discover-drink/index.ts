...
async function generateDescriptions(places: any[]): Promise<string[]> {
  if (!OPENAI_API_KEY || places.length === 0) {
    return places.map(p =>
      `A welcoming ${formatPlaceType(p.primaryType || 'venue').toLowerCase()} with a great selection of drinks and relaxed atmosphere.`
    );
  }
  ...
  try {
        ...
        messages: [
          {
            ...
            content:
              ...
              'Focus on the atmosphere, drink selection, and why it\'s great for unwinding or socializing. ' +
    ...
    if (!resp.ok) {
      console.warn(`[discover-drink] OpenAI error: ${resp.status}`);
      return places.map(p =>
        `A welcoming ${formatPlaceType(p.primaryType || 'venue').toLowerCase()} with a great selection of drinks and relaxed atmosphere.`
      );
    }
    ...
    console.warn('[discover-drink] AI description generation failed:', err);
    return places.map(p =>
      `A welcoming ${formatPlaceType(p.primaryType || 'venue').toLowerCase()} with a great selection of drinks and relaxed atmosphere.`
    );
  }
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`[discover-drink] Request: batchSeed=${batchSeed}, limit=${limit}, mode=${travelMode}`);
    ...
    if (userId && batchSeed === 0 && !body.warmPool) {
      try {
        ...
        if (poolResult.cards.length >= Math.ceil(limit * 0.75)) {
          console.log(`[discover-drink] Served ${poolResult.cards.length} from pool (0 API calls)`);
        ...
        console.warn('[discover-drink] Pool serve failed, falling back to API:', poolErr);
      }
    }
    ...
    // ── Search all Drink types using shared cache ───────────────────────
    ...
    console.log(`[discover-drink] Places search: ${cacheHits} cache hits, ${apiCallsMade} API calls`);
    ...
    console.log(`[discover-drink] ${allPlaces.length} unique places across ${Object.keys(results).length} types`);

    // ── Filter out excluded place types ──────────────────────────────────
    const drinkExcluded = getExcludedTypesForCategory('Drink');
    allPlaces = filterExcludedPlaces(allPlaces, drinkExcluded) as typeof allPlaces;
    console.log(`[discover-drink] ${allPlaces.length} places after exclusion filter`);
    ...
    console.log(`[discover-drink] ${allPlaces.length} places after datetime filter (dateOption=${dateOption}, timeSlot=${timeSlot})`);
    ...
    console.log(`[discover-drink] Batch ${batchSeed}: offset=${offset}, returning ${batch.length}/${totalAvailable}`);
    ...
    const cards = batch.map((p, i) => {
      ...
      const primaryType = p.primaryType || p._matchedType || 'bar';
    ...
    if (userId) {
      (async () => {
        try {
          ...
          console.warn('[discover-drink] Pool store failed (non-critical):', e);
        }
      })();
    }

    const elapsed = Date.now() - t0;
    console.log(`[discover-drink] Done in ${elapsed}ms: ${cards.length} cards, ${apiCallsMade} API calls`);
    ...
    console.error('[discover-drink] Error:', err);
    return new Response(
