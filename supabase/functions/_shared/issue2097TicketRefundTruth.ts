...
export type PreflightDecision =
  ...
  | { allowed: false; status: "rejected_preflight"; reason: Issue2097PreflightReason };
...
export function decideFeeRefundPreflight(
  ...
  if (f === null || r === null || c === null) {
    return { allowed: false, status: "rejected_preflight", reason: "invalid_provider_amount" };
  }
  const F = BigInt(f);
  const R = BigInt(r);
  const C = BigInt(c);
  ...
  if (F * R < C) {
    return { allowed: false, status: "rejected_preflight", reason: "partial_fee_below_provider_cent" };
  }
...
export type FeeEvidenceDecision =
  ...
  | { status: "succeeded_positive"; feeRefundId: string; amountText: string; afterAmountText: string }
  | { status: "evidence_conflict"; feeRefundId: string | null; observedAmountText: string | null };
...
export function classifyFeeRefundEvidence(input: {
  ...
  if (!input.listComplete) return { status: "evidence_conflict", feeRefundId: null, observedAmountText: null };
  ...
  if (original === null || before === null || after === null) {
    return { status: "evidence_conflict", feeRefundId: null, observedAmountText: null };
  }
  ...
  if (novel.length === 0 && BigInt(after) === BigInt(before)) return { status: "pending_visibility" };
  if (novel.length !== 1) return { status: "evidence_conflict", feeRefundId: null, observedAmountText: null };
  const one = novel[0];
  const amount = canonicalProviderInteger(one.amount);
  if (amount === null || one.fee !== input.applicationFeeId || one.currency.toLowerCase() !== input.currency.toLowerCase()) {
    return { status: "evidence_conflict", feeRefundId: one.id, observedAmountText: amount };
  }
  const delta = BigInt(after) - BigInt(before);
  if (BigInt(amount) <= 0n || delta !== BigInt(amount) || BigInt(after) > BigInt(original)) {
    return { status: "evidence_conflict", feeRefundId: one.id, observedAmountText: amount };
  }
  return { status: "succeeded_positive", feeRefundId: one.id, amountText: amount, afterAmountText: after };
}

export async function listAllFeeRefunds(
  listPage: (startingAfter?: string) => Promise<{ data: FeeRefundEvidence[]; has_more: boolean }>,
  ...
  for (let page = 0; page < 100; page += 1) {
    ...
    if (!last || last === cursor) throw new Error("fee_refund_pagination_conflict");
    cursor = last;
  }
  throw new Error("fee_refund_pagination_incomplete");
}

export const ISSUE_2097_OBSERVATION_DELAYS_SECONDS = [0, 5, 30, 120, 600, 1800, 7200, 86400] as const;

export function publicFeeTruth(amount: unknown, status: Issue2097Status): number | null {
  if (status === "not_applicable") return 0;
  if (status !== "succeeded_positive") return null;
  const canonical = canonicalProviderInteger(amount);
  return canonical === null || BigInt(canonical) <= 0n ? null : Number(canonical);
}
...
type StripeRefundClient = {
  paymentIntents: { retrieve: (id: string, params: unknown, options: unknown) => Promise<any> };
  charges: { retrieve: (id: string, params: unknown, options: unknown) => Promise<any> };
  applicationFees: {
    retrieve: (id: string) => Promise<any>;
    listRefunds: (id: string, params: Record<string, unknown>) => Promise<any>;
  };
  refunds: { create: (params: Record<string, unknown>, options: Record<string, unknown>) => Promise<any> };
};
...
const rpc = async (client: RpcClient, name: string, args: Record<string, unknown>): Promise<any> => {
  const result = await client.rpc(name, args);
  if (result.error) throw new Error(`${name}:${result.error.message ?? "failed"}`);
  return result.data;
};

async function feeRefundSnapshot(stripe: StripeRefundClient, feeId: string): Promise<{ ids: string[]; rows: FeeRefundEvidence[] }> {
  const rows = await listAllFeeRefunds(async (startingAfter) => {
    const page = await stripe.applicationFees.listRefunds(feeId, {
      limit: 100,
      ...(startingAfter ? { starting_after: startingAfter } : {}),
    });
    return { data: page.data.map((row: any) => ({ id: String(row.id), fee: String(row.fee), amount: row.amount, currency: String(row.currency) })), has_more: page.has_more === true };
  });
 ...
 */
export async function executeTicketRefundWithFeeTruth(input: ExecuteTicketRefundInput): Promise<ExecuteTicketRefundResult> {
  ...
  const chargeId = input.knownChargeId || (typeof pi.latest_charge === "string" ? pi.latest_charge : pi.latest_charge?.id);
  if (!chargeId) throw new Error("application_fee_conflict");
  const charge = typeof pi.latest_charge === "object" && pi.latest_charge?.id === chargeId
    ? pi.latest_charge
    : await input.stripe.charges.retrieve(chargeId, { expand: ["application_fee"] }, { stripeAccount: input.connectedAccountId });
  const captured = canonicalProviderInteger(charge.amount_captured);
  const feeIdentity = typeof charge.application_fee === "string" ? charge.application_fee : charge.application_fee?.id;
  const recordPreRefundState = async (status: "awaiting_application_fee" | "application_fee_conflict") => {
    const result = await rpc(input.supabase, "issue_2097_record_pre_refund_state", {
    ...
    return { status: result.status as Issue2097Status, buyerRefundId: null, applicationFeeRefundedCents: null, attemptId: result.attempt_id as string, httpStatus: status === "awaiting_application_fee" ? 202 : 409 };
  };
  if (!feeIdentity && persistedExpectedFee !== "0") return await recordPreRefundState("awaiting_application_fee");
  if (persistedExpectedFee === "0" && !feeIdentity) {
    const created = input.createBuyerRefund ? await input.createBuyerRefund() : await input.stripe.refunds.create({
      payment_intent: input.paymentIntentId,
      amount: Number(requested),
      reason: "requested_by_customer",
      refund_application_fee: false,
      metadata: { mingla_refund_id: input.refundId, mingla_order_id: input.orderId },
    }, { idempotencyKey: input.providerIdempotencyKey ?? `ticket_refund:${input.refundId}`, stripeAccount: input.connectedAccountId });
    await rpc(input.supabase, "issue_2097_finalize_not_applicable", {
      p_refund_id: input.refundId,
      p_provider: "stripe_proven_no_fee",
      p_provider_refund_id: String(created.id),
    });
    return { status: "not_applicable", buyerRefundId: String(created.id), applicationFeeRefundedCents: 0, attemptId: null, httpStatus: 200 };
  }
  if (!feeIdentity || requested === null || captured === null) return await recordPreRefundState("application_fee_conflict");
  ...
  if (feeAmount !== expectedFee || fee.currency?.toLowerCase() !== input.expectedCurrency.toLowerCase() || fee.charge !== chargeId || fee.account !== input.connectedAccountId || fee.livemode !== charge.livemode || feeRefundedBefore === null) {
    return await recordPreRefundState("application_fee_conflict");
  }
  ...
  const prepared = await rpc(input.supabase, "issue_2097_prepare_refund_attempt", {
    p_refund_id: input.refundId,
    p_request_fingerprint: input.requestFingerprint,
  ...
  if (prepared.status === "rejected_preflight") {
    return { status: "rejected_preflight", terminalReason: prepared.terminal_reason, buyerRefundId: null, applicationFeeRefundedCents: null, attemptId: prepared.attempt_id, httpStatus: 422 };
  }
  ...
  if (prepared.idempotent_replay === true) {
    const { data: existingAttempt, error: existingAttemptError } = await input.supabase.from("ticket_refund_attempts")
      .select("buyer_refund_id,baseline_fee_refund_ids,baseline_amount_refunded_text,fee_refund_amount_text")
      .eq("id", prepared.attempt_id).single();
    if (existingAttemptError || !existingAttempt) throw new Error("refund_attempt_replay_lookup_failed");
    ...
    if (["succeeded_positive", "not_applicable", "rejected_preflight", "fee_evidence_unavailable", "evidence_conflict"].includes(prepared.status)) {
      const proven = canonicalProviderInteger(existingAttempt.fee_refund_amount_text);
      return { status: prepared.status as Issue2097Status, terminalReason: prepared.terminal_reason,
        buyerRefundId, applicationFeeRefundedCents: prepared.status === "succeeded_positive" && proven ? Number(proven) : prepared.status === "not_applicable" ? 0 : null,
        attemptId: prepared.attempt_id, httpStatus: ["succeeded_positive", "not_applicable"].includes(prepared.status) ? 200 : 409 };
    }
    const claimed = await rpc(input.supabase, "issue_2097_claim_refund_attempt", {
      p_attempt_id: prepared.attempt_id,
      p_lease_owner: leaseOwner,
    });
    if (claimed.claimed !== true) {
      return { status: prepared.status as Issue2097Status, buyerRefundId, applicationFeeRefundedCents: null, attemptId: prepared.attempt_id, httpStatus: 202 };
    }
    prepared.provider_call_permitted = !buyerRefundId;
    prepared.lease_epoch = claimed.lease_epoch;
  }
  if (prepared.provider_call_permitted === true && input.allowProviderMutation === false) {
    return { status: prepared.status as Issue2097Status, buyerRefundId: null, applicationFeeRefundedCents: null, attemptId: prepared.attempt_id, httpStatus: 202 };
  }
  ...
  if (decision.status === "pending_visibility") {
    return { status: decision.status, buyerRefundId, applicationFeeRefundedCents: null, attemptId: prepared.attempt_id, httpStatus: 202 };
  }
  await rpc(input.supabase, "issue_2097_finalize_refund_attempt", {
    ...
    p_after_amount_refunded_text: canonicalProviderInteger(afterFee.amount_refunded),
    p_stripe_event_id: null,
  });
  return {
    status: decision.status,
    buyerRefundId,
    applicationFeeRefundedCents: decision.status === "succeeded_positive" ? Number(decision.amountText) : null,
