...
Deno.test("#1974 sparse tier patch preserves complete modifiers and password configuration", () => {
  const next = applyTierPatch(completeTier, { name: "Early bird plus" }, OPERATION_ID, 4);
...
for (const [name, args, message] of [
  ["limited zero capacity", { name: "GA", is_free: true, is_unlimited: false, capacity: 0 }, "capacity"],
  ["paid zero price", { name: "GA", is_free: false, is_unlimited: true, price_cents: 0 }, "price_cents"],
  ["unlimited waitlist", { name: "GA", is_free: true, is_unlimited: true, waitlist_enabled: true }, "waitlist"],
  ["bad limits", { name: "GA", is_free: true, is_unlimited: true, min_purchase_qty: 3, max_purchase_qty: 2 }, "max_purchase_qty"],
  ["long description", { name: "GA", is_free: true, is_unlimited: true, description: "x".repeat(281) }, "280"],
  ["bad sale window", { name: "GA", is_free: true, is_unlimited: true, sale_start_at: "2027-02-01T00:00:00Z", sale_end_at: "2027-01-01T00:00:00Z" }, "sale_end_at"],
] as const) {
...
Deno.test("#1974 confirmed draft create uses theme graph command, derived currency, and immutable pending identity", async () => {
  ...
  const tool = DOMAIN_TOOLS.find((candidate) => candidate.name === "upsert_ticket_tier");
  assert(tool);
  ...
  const mutation = rpcCalls.find((call) => call.name === "business_patch_event_ticket_tiers");
  assert(mutation);
  assertEquals(mutation.args.p_operation_id, OPERATION_ID);
  assertEquals(mutation.args.p_expected_client_revision, 7);
  assertFalse("currency" in mutation.args);
  assertEquals(result.effective_currency, "NGN");
  const tiers = mutation.args.p_tiers as Array<Record<string, unknown>>;
  assertEquals(tiers[0].id, OPERATION_ID);
  assertEquals(tiers[0].priceGbp, 2500);
});

Deno.test("#1974 model schemas expose lifecycle-neutral ids and never expose currency or passwords", () => {
  const tier = DOMAIN_TOOLS.find((candidate) => candidate.name === "upsert_ticket_tier");
  const brand = DOMAIN_TOOLS.find((candidate) => candidate.name === "set_brand_pricing_defaults");
  assert(tier && brand);
  const properties = (tier.parameters as { properties: Record<string, unknown> }).properties;
