...
Deno.test("#1973 Ari experience tools use canonical graph owners", async () => {
  ...
  assertStringIncludes(create, 'client.rpc("business_create_experience_graph"');
  ...
  assertStringIncludes(experienceDomain, 'p_action: "publish"');
  assertStringIncludes(experienceDomain, 'p_action: "update"');
  assertStringIncludes(experienceDomain, 'p_action: "manage_stops"');
  assertStringIncludes(experienceDomain, "business_unpublish_experience_to_draft");
  assertStringIncludes(experienceDomain, "business_discard_experience_draft");
...
Deno.test("#1973 SQL owns graph readback, revision gates, media provenance, demotion and grants", async () => {
  ...
  assertStringIncludes(sql, "status='draft',visibility='draft',published_at=NULL");
  ...
  assert(!sql.includes("agent_operation_receipts"));
});

Deno.test("#1973 ledger covers every experience row without claiming verification", async () => {
  const ledger = JSON.parse(await read("docs/contracts/ari-capability-ledger.json"));
  ...
  assertEquals(byId.get("ari.experience.unpublish")?.ari_tool, "unpublish_experience");
  assertEquals(byId.get("ari.experience.manage_stops")?.ari_tool, "manage_experience_stops");
  assertEquals(byId.get("ari.experience.snap_generation")?.status, "guided_handoff");
  for (const row of rows) assert(row.status !== "verified");
});

Deno.test("#1973 Snap parsers authorize event managers and insert each proposal set atomically", async () => {
  for (const path of [
    ...
    assertStringIncludes(source, ".insert(proposalRows)");
    assert(!source.includes("brand.account_id !== userId"));
    assert(!source.includes("for (const exp of parseResult.experiences) {\n    const tool_args"));
  }
});
