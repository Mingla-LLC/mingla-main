...
Deno.test("#2019 registry is exact, duplicate-free, and fully declared", () => {
  assert(AGENT_TOOLS.length === 69, `expected 69 tools, got ${AGENT_TOOLS.length}`);
  assert(new Set(AGENT_TOOLS.map((t) => t.name)).size === 69, "duplicate tool");
  assert(Object.keys(AGENT_TOOL_AUTHORIZATION).length === 69, "authorization registry drift");
...
Deno.test("#2019 declarations exactly translate the accepted capability ledger", async () => {
  ...
  assert(rows.length === 69, `expected 69 ledger rows, got ${rows.length}`);
...
Deno.test("#2019 full rank x operation-class matrix", async () => {
  ...
  for (const [name, requiredRole, threshold] of classes) {
    ...
    assert(registered.requiredRole === requiredRole, `${name}: fixture role drift`);
...
Deno.test("#2019 foreign, deleted, nonexistent, and type-confused events are unavailable", async () => {
  ...
  const eventClient = (state: "foreign" | "deleted" | "missing" | "wrong_type"): any => ({
    rpc(name: string) {
      return Promise.resolve({ data: name === "biz_role_rank" || state === "wrong_type" ? 40 : 0, error: null });
    },
    from(table: string) {
      ...
      const query: any = {
        ...
        maybeSingle: () => Promise.resolve({
          data: table !== "events" || state === "missing" || (state === "deleted" && activeOnly)
            ? null
            : { brand_id: FOREIGN_BRAND, event_type: state === "wrong_type" ? "event" : "trip" },
          error: null,
        }),
      };
      return query;
  ...
  assert(codes.length === 4 && codes.every((code) => code === "BRAND_ACCESS_DENIED"), "event state leaked by code");
  assert(messages.length === 4 && messages.every((message) => message === messages[0]), "event state leaked by message");
});
...
Deno.test("#2019 wrapped executor cannot be bypassed and catches revocation", async () => {
  ...
  catch (error) { denied = error instanceof ToolError && error.code === "BRAND_ACCESS_DENIED"; }
...
Deno.test("#2019 foreign indirect resources and nonexistent ids are indistinguishable", async () => {
  ...
  const makeResourceClient = (partnerExists: boolean): any => ({
    ...
    from(table: string) {
      const query: any = {
        ...
        maybeSingle: () => Promise.resolve({
          data: table === "partner_brand_links" && partnerExists ? { brand_id: FOREIGN } : null,
          error: null,
        }),
      };
      return query;
  ...
  for (const exists of [true, false]) {
    try {
      await authorizeAgentTool(disconnectPartner, { brand_id: UUID, partner_id: PARTNER, confirm_phrase: "DISCONNECT" }, makeResourceClient(exists), UUID);
    } catch (error) {
...
Deno.test("#2019 partner binding uses the canonical link relation", async () => {
  ...
  const resourceClient: any = {
    rpc(name: string) {
      return Promise.resolve({ data: name === "biz_role_rank" ? 30 : 30, error: null });
    },
    from(table: string) {
      tables.push(table);
      const query: any = {
  ...
  assert(tables.includes("partner_brand_links"), "partner link was not resolved through its canonical table");
  assert(!tables.includes("brand_partners"), "authorization queried the obsolete nonexistent partner table");
});

Deno.test("#2019 malformed final args fail before any authority or resource lookup", async () => {
  let authorityCalls = 0;
  const noLookupClient: any = {
    rpc() { authorityCalls++; return Promise.resolve({ data: 60, error: null }); },
    from() { authorityCalls++; throw new Error("resource lookup must not run"); },
  };
  for (const args of [
    { brand_id: UUID, confirm_phrase: "DISCONNECT" },
    { brand_id: UUID, partner_id: "not-a-uuid", confirm_phrase: "DISCONNECT" },
    { brand_id: UUID, partner_id: PARTNER, confirm_phrase: false },
    { brand_id: UUID, partner_id: PARTNER, confirm_phrase: "DISCONNECT", surprise: true },
  ]) {
    let code = "";
    try { await authorizeAgentTool(tool("disconnect_partner"), args, noLookupClient, UUID); }
    catch (error) { if (error instanceof ToolError) code = error.code; }
    assert(code === "INVALID_ARGS", `malformed args did not fail as INVALID_ARGS: ${code}`);
  }
...
Deno.test("#2019 RSVP plus-one guest binding follows guest to RSVP to event", async () => {
  ...
  const resourceClient: any = {
    rpc(name: string) {
      return Promise.resolve({ data: name === "biz_role_rank" ? 40 : 40, error: null });
    },
    from(table: string) {
      let selected = "";
      const query: any = {
        select: (columns: string) => { selected = columns; selects.push(`${table}:${columns}`); return query; },
        eq: () => query,
        is: () => query,
        maybeSingle: () => Promise.resolve({
