...
// [TEST-MOD-APPROVED #1981] charge_installment_now + send_installment_reminder; 90→92.
  // [TEST-MOD-APPROVED #2830] Twelve Website declarations extend the exact
  ...
  assert(
    AGENT_TOOLS.length === 125,
    `expected 125 tools, got ${AGENT_TOOLS.length}`,
  );
  assert(new Set(AGENT_TOOLS.map((t) => t.name)).size === 125, "duplicate tool");
  assert(
    Object.keys(AGENT_TOOL_AUTHORIZATION).length === 125,
...
Deno.test("#2019 declarations exactly translate the accepted capability ledger", async () => {
  ...
  assert(rows.length === 125, `expected 125 ledger rows, got ${rows.length}`);
