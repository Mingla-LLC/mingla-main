...
async function callResearchOnce(apiKey: string, prompt: string): Promise<TripResearch | null> {
  ...
  if (!res.ok) {
    ...
    console.error("[growth-tools-trips] research HTTP", {
      status: res.status,
...
async function callSynthesisOnce(
  ...
  if (!res.ok) {
    ...
    const detail = await res.text().catch(() => "");
    console.error("[growth-tools-trips] synthesis HTTP", {
      status: res.status,
      detail: detail.slice(0, 200),
    });
...
async function generateSynthesis(
  ...
  const first = await callSynthesisOnce(apiKey, prompt, input.group_size, days, cap1, sink);
  ...
  const second = await callSynthesisOnce(apiKey, prompt, input.group_size, days, cap2, sink);
