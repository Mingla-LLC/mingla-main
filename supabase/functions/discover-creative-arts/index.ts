...
async function generateDescriptions(places: any[]): Promise<string[]> {
  if (!OPENAI_API_KEY || places.length === 0) {
    return places.map(p =>
      `A creative space where art and culture come alive — perfect for an inspiring outing.`
    );
  }
  ...
  try {
        ...
        messages: [
          {
            ...
            content:
              ...
              'Focus on the creative experience and what makes it inspiring. Highlight exhibitions, classes, or unique artistic offerings. ' +
    ...
    if (!resp.ok) {
      console.warn(`[discover-creative-arts] OpenAI error: ${resp.status}`);
      return places.map(p =>
        `A creative space where art and culture come alive — perfect for an inspiring outing.`
      );
    }
    ...
    console.warn('[discover-creative-arts] AI description generation failed:', err);
    return places.map(p =>
      `A creative space where art and culture come alive — perfect for an inspiring outing.`
    );
  }
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`[discover-creative-arts] Request: batchSeed=${batchSeed}, limit=${limit}, mode=${travelMode}`);
    ...
    if (userId && batchSeed === 0 && !body.warmPool) {
      try {
        ...
        if (poolResult.cards.length >= Math.ceil(limit * 0.75)) {
          console.log(`[discover-creative-arts] Served ${poolResult.cards.length} from pool (0 API calls)`);
        ...
        console.warn('[discover-creative-arts] Pool serve failed, falling back to API:', poolErr);
      }
    }
    ...
    console.log(`[discover-creative-arts] Places search: ${cacheHits} cache hits, ${apiCallsMade} API calls`);
    ...
    if (TEXT_SEARCH_KEYWORDS.length > 0) {
      try {
        ...
        console.log(`[discover-creative-arts] Text search: ${textCount} additional places from ${TEXT_SEARCH_KEYWORDS.length} keywords`);
      } catch (err) {
        console.warn(`[discover-creative-arts] Text search failed (non-critical):`, err);
      }
    }
    ...
    console.log(`[discover-creative-arts] ${allPlaces.length} places after datetime filter (dateOption=${dateOption}, timeSlot=${timeSlot})`);
    ...
    console.log(`[discover-creative-arts] Batch ${batchSeed}: offset=${offset}, returning ${batch.length}/${totalAvailable}`);
    ...
    const cards = batch.map((p, i) => {
      ...
      const primaryType = p.primaryType || p._matchedType || 'art_gallery';

      return {
        id: `creative-arts-${p.id}`,
        placeId: p.id,
    ...
    if (userId) {
      (async () => {
        try {
          ...
          console.warn('[discover-creative-arts] Pool store failed (non-critical):', e);
        }
      })();
    }

    const elapsed = Date.now() - t0;
    console.log(`[discover-creative-arts] Done in ${elapsed}ms: ${cards.length} cards, ${apiCallsMade} API calls`);
    ...
    console.error('[discover-creative-arts] Error:', err);
    return new Response(
