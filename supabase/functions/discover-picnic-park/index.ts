 ...
 *
 * A dedicated, self-contained edge function for picnic ground discovery.
 * NOT part of the curated Adventurous pipeline.
 *
 * • Searches 1 picnic Google Place type via shared cache (max 1 API call,
 *   typically 0 on repeat requests thanks to 24h cache).
...
function filterByDateTime(
  ...
  return places.filter(place => {
    ...
    // No opening hours data → assume always open (picnic grounds)
...
async function generateDescriptions(places: any[]): Promise<string[]> {
  if (!OPENAI_API_KEY || places.length === 0) {
    return places.map(p =>
      `A lovely ${formatPlaceType(p.primaryType || 'picnic ground').toLowerCase()} perfect for an outdoor picnic with scenic views.`
    );
  }
  ...
  try {
    const placeList = places
      .map((p, i) => {
        ...
        const type = formatPlaceType(p.primaryType || 'picnic ground');
    ...
    const resp = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: 'user',
            content: `Write descriptions for these ${places.length} picnic spots:\n${placeList}`,
          },
        ],
    ...
    if (!resp.ok) {
      console.warn(`[discover-picnic-park] OpenAI error: ${resp.status}`);
      return places.map(p =>
        `A lovely ${formatPlaceType(p.primaryType || 'picnic ground').toLowerCase()} ideal for a relaxing picnic.`
      );
    }
    ...
    console.warn('[discover-picnic-park] AI description generation failed:', err);
    return places.map(p =>
      `A lovely ${formatPlaceType(p.primaryType || 'picnic ground').toLowerCase()} ideal for a relaxing picnic.`
    );
  }
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`[discover-picnic-park] Request: batchSeed=${batchSeed}, limit=${limit}, mode=${travelMode}`);
    ...
    if (userId && batchSeed === 0 && !body.warmPool) {
      try {
        ...
        if (poolResult.cards.length >= Math.ceil(limit * 0.75)) {
          console.log(`[discover-picnic-park] Served ${poolResult.cards.length} from pool (0 API calls)`);
        ...
        console.warn('[discover-picnic-park] Pool serve failed, falling back to API:', poolErr);
      }
    }
    ...
    // ── Search picnic ground type using shared cache ────────────────────
    // batchSearchPlaces runs in parallel and uses per-type caching.
    // Typical result: 0 API calls on repeat (cached), 1 on cold start.
    ...
    console.log(`[discover-picnic-park] Places search: ${cacheHits} cache hits, ${apiCallsMade} API calls`);
    ...
    console.log(`[discover-picnic-park] ${allPlaces.length} places after datetime filter (dateOption=${dateOption}, timeSlot=${timeSlot})`);
    ...
    console.log(`[discover-picnic-park] Batch ${batchSeed}: offset=${offset}, returning ${batch.length}/${totalAvailable}`);
    ...
    const cards = batch.map((p, i) => {
      ...
      const primaryType = p.primaryType || p._matchedType || 'picnic_ground';

      return {
        id: `picnic-park-${p.id}`,
        placeId: p.id,
        title: p.displayName?.text || 'Picnic Spot',
    ...
    if (userId) {
      (async () => {
        try {
          ...
          console.warn('[discover-picnic-park] Pool store failed (non-critical):', e);
        }
      })();
    }

    const elapsed = Date.now() - t0;
    console.log(`[discover-picnic-park] Done in ${elapsed}ms: ${cards.length} cards, ${apiCallsMade} API calls`);
    ...
    console.error('[discover-picnic-park] Error:', err);
    return new Response(
