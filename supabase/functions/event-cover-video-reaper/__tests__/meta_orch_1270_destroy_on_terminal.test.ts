...
// for an abandoned draft, flips it to failed:reaped_abandoned). On a destroy
...
const makeSupabaseStub = (candidates: ReapCandidate[], patches: Patch[]) => {
  const selectChain = {
    // deno-lint-ignore no-explicit-any
    select() { return this as any; },
    // deno-lint-ignore no-explicit-any
    is() { return this as any; },
    // deno-lint-ignore no-explicit-any
    not() { return this as any; },
    // deno-lint-ignore no-explicit-any
...
Deno.test("reaper: destroys a cancelled Bunny asset + stamps reaped_at", async () => {
  await withServiceKey(async () => {
    ...
    const candidates: ReapCandidate[] = [
      { id: "job-a", status: "cancelled", source_asset_id: "guid-a", reaped_at: null, provider: "bunny", created_at: HOURS(1) },
    ];
    const deps = {
    ...
    assert(destroyed.length === 1, "destroyCoverVideoAsset must be called for the terminal asset");
    assert((destroyed[0] as { source_asset_id?: unknown }).source_asset_id === "guid-a", "destroy targets guid-a");
    const stamp = patches.find((p) => p.id === "job-a");
    assert(!!stamp && typeof stamp.patch.reaped_at === "string", "reaped_at stamped after a successful destroy");
  });
});

Deno.test("reaper: abandoned draft → destroy + failed:reaped_abandoned + reaped_at", async () => {
  await withServiceKey(async () => {
    const patches: Patch[] = [];
    const candidates: ReapCandidate[] = [
      { id: "job-old", status: "ready", source_asset_id: "guid-old", reaped_at: null, applied_at: null, created_at: HOURS(30), provider: "bunny" },
    ];
    const deps = {
    ...
    const body = await response.json();
    assert(body.reaped === 1, `expected reaped=1, got ${body.reaped}`);
    const stamp = patches.find((p) => p.id === "job-old");
    assert(!!stamp, "abandoned job patched");
    assert(stamp!.patch.status === "failed", "abandoned draft flipped to failed");
    assert(stamp!.patch.failure_code === "reaped_abandoned", "failure_code reaped_abandoned");
    assert(typeof stamp!.patch.reaped_at === "string", "reaped_at stamped");
  });
});
...
Deno.test("reaper: destroy FAILURE → no reaped_at (fail-safe retry)", async () => {
  await withServiceKey(async () => {
    const patches: Patch[] = [];
    const candidates: ReapCandidate[] = [
      { id: "job-x", status: "failed", source_asset_id: "guid-x", reaped_at: null, provider: "bunny", created_at: HOURS(1) },
    ];
    const deps = {
      destroyCoverVideoAsset: () => Promise.resolve({ ok: false as const, reason: "bunny_delete_http_500" }),
      serviceRoleClient: () => makeSupabaseStub(candidates, patches) as never,
    };
    const response = await handleReaper(authedRequest(), deps as never);
    const body = await response.json();
    assert(body.reaped === 0 && body.failed === 1, `expected reaped=0 failed=1, got ${JSON.stringify(body)}`);
    assert(patches.length === 0, "a failed destroy must NOT stamp reaped_at (retry next run)");
  });
});

Deno.test("reaper: rejects a non-service-role caller", async () => {
  await withServiceKey(async () => {
    const req = new Request("https://example.test/functions/v1/event-cover-video-reaper", {
      method: "POST",
    ...
    assert(response.status === 401, `expected 401 for a bad bearer, got ${response.status}`);
  });
});
