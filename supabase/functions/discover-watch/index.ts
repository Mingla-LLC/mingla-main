...
/* ─────────────────────────────────────────────────────────────────────────────
 * discover-watch  –  Standalone Watch Card System
 *
 * A dedicated, self-contained edge function for Watch venue discovery.
 * Modeled identically on discover-first-meet.
 *
 * • Searches 3 entertainment-venue Google Place types via shared cache.
...
async function generateDescriptions(places: any[]): Promise<string[]> {
  if (!OPENAI_API_KEY || places.length === 0) {
    return places.map(p =>
      `A great entertainment venue for an enjoyable shared viewing experience.`
    );
  }
  ...
  try {
        ...
        messages: [
          {
            ...
            content:
              ...
              'Focus on the entertainment value and venue experience. Highlight showings, performances, or unique features. ' +
    ...
    if (!resp.ok) {
      console.warn(`[discover-watch] OpenAI error: ${resp.status}`);
      return places.map(p =>
        `A great entertainment venue for an enjoyable shared viewing experience.`
      );
    }
    ...
    console.warn('[discover-watch] AI description generation failed:', err);
    return places.map(p =>
      `A great entertainment venue for an enjoyable shared viewing experience.`
    );
  }
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`[discover-watch] Request: batchSeed=${batchSeed}, limit=${limit}, mode=${travelMode}`);
    ...
    if (userId && batchSeed === 0 && !body.warmPool) {
      try {
        ...
        if (poolResult.cards.length >= Math.ceil(limit * 0.75)) {
          console.log(`[discover-watch] Served ${poolResult.cards.length} from pool (0 API calls)`);
        ...
        console.warn('[discover-watch] Pool serve failed, falling back to API:', poolErr);
      }
    }
    ...
    // ── Search all Watch types using shared cache ───────────────────────
    ...
    console.log(`[discover-watch] Places search: ${cacheHits} cache hits, ${apiCallsMade} API calls`);
    ...
    console.log(`[discover-watch] ${allPlaces.length} unique places across ${Object.keys(results).length} types`);

    // ── Filter out excluded place types ──────────────────────────────────
    const watchExcluded = getExcludedTypesForCategory('Watch');
    allPlaces = filterExcludedPlaces(allPlaces, watchExcluded) as typeof allPlaces;
    console.log(`[discover-watch] ${allPlaces.length} places after exclusion filter`);
    ...
    console.log(`[discover-watch] ${allPlaces.length} places after datetime filter (dateOption=${dateOption}, timeSlot=${timeSlot})`);
    ...
    console.log(`[discover-watch] Batch ${batchSeed}: offset=${offset}, returning ${batch.length}/${totalAvailable}`);
    ...
    const cards = batch.map((p, i) => {
      ...
      const primaryType = p.primaryType || p._matchedType || 'movie_theater';
    ...
    if (userId) {
      (async () => {
        try {
          ...
          console.warn('[discover-watch] Pool store failed (non-critical):', e);
        }
      })();
    }

    const elapsed = Date.now() - t0;
    console.log(`[discover-watch] Done in ${elapsed}ms: ${cards.length} cards, ${apiCallsMade} API calls`);
    ...
    console.error('[discover-watch] Error:', err);
    return new Response(
