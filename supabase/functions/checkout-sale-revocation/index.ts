  ...
  for (const row of (data ?? []) as Revocation[]) {
    ...
    try {
      if (row.subject_type === "rsvp_contribution") {
        // Existing hosted URLs are suppressed locally immediately. Provider
        // success is reconciled by the signed webhook into the canonical
        // source-refund rail; Paystack has no transaction-cancel primitive.
        state = "neutralized";
      } else {
