 ...
 *
 * A dedicated, self-contained edge function for nature place discovery.
 * NOT part of the curated Adventurous pipeline.
 *
 * • Searches 8 nature Google Place types via shared cache (max 8 API calls,
...
async function generateDescriptions(places: any[]): Promise<string[]> {
  if (!OPENAI_API_KEY || places.length === 0) {
    return places.map(p =>
      `A beautiful ${formatPlaceType(p.primaryType || 'nature spot').toLowerCase()} perfect for outdoor exploration and relaxation.`
    );
  }
  ...
  try {
    const placeList = places
      .map((p, i) => {
        ...
        const type = formatPlaceType(p.primaryType || 'nature spot');
    ...
    const resp = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: 'user',
            content: `Write descriptions for these ${places.length} nature spots:\n${placeList}`,
          },
        ],
    ...
    if (!resp.ok) {
      console.warn(`[discover-nature] OpenAI error: ${resp.status}`);
      return places.map(p =>
        `A beautiful ${formatPlaceType(p.primaryType || 'nature spot').toLowerCase()} perfect for outdoor exploration.`
      );
    }
    ...
    console.warn('[discover-nature] AI description generation failed:', err);
    return places.map(p =>
      `A beautiful ${formatPlaceType(p.primaryType || 'nature spot').toLowerCase()} perfect for outdoor exploration.`
    );
  }
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`[discover-nature] Request: batchSeed=${batchSeed}, limit=${limit}, mode=${travelMode}`);
    ...
    // Typical result: 0 API calls on repeat (all cached), 8 on cold start.
    ...
    console.log(`[discover-nature] Places search: ${cacheHits} cache hits, ${apiCallsMade} API calls`);
    ...
    console.log(`[discover-nature] ${allPlaces.length} unique places across ${Object.keys(results).length} types`);

    // ── Filter out excluded place types ──────────────────────────────────
    const natureExcluded = getExcludedTypesForCategory('Nature');
    allPlaces = filterExcludedPlaces(allPlaces, natureExcluded) as typeof allPlaces;
    console.log(`[discover-nature] ${allPlaces.length} places after exclusion filter`);
    ...
    console.log(`[discover-nature] Batch ${batchSeed}: offset=${offset}, returning ${batch.length}/${totalAvailable}`);
    ...
    const cards = batch.map((p, i) => {
      ...
      const primaryType = p.primaryType || p._matchedType || 'park';

      return {
        id: `nature-${p.id}`,
        placeId: p.id,
        title: p.displayName?.text || 'Nature Spot',
    ...
    console.error('[discover-nature] Error:', err);
    return new Response(
