  ...
  try {
    ...
    if (preferences.userId) {
      ...
      console.log("👤 Loaded user preference data:", {
        ...
        frequentLocations:
          userPreferenceData.locationContext.frequentLocations.length,
      });
    }
      ...
      return new Response(
        JSON.stringify({
          ...
          meta: {
            ...
            sources: {
              googlePlaces:
                places.status === "fulfilled" ? places.value.length : 0,
              eventbrite:
    ...
    const scoredCandidates = scoreAndRankWithUserData(
      filteredCandidates,
      preferences,
      userPreferenceData
    );
    ...
    if (userId) {
      try {
        ...
        console.error(
          "⚠️ ML enhancement failed, using base recommendations:",
          mlError
        );
      ...
      JSON.stringify({
        ...
        meta: {
          ...
          personalizationFactors: userPreferenceData
            ? {
                learnedPreferences:
                  userPreferenceData.learnedPreferences.length,
                interactionHistory:
                  userPreferenceData.interactionHistory.length,
                frequentLocations:
                  userPreferenceData.locationContext.frequentLocations.length,
              }
            : null,
...
async function getUserPreferenceData(
  userId: string
  ...
  try {
    ...
    const { data: frequentLocations, error: locationError } =
      await supabase.rpc("get_user_frequent_locations", {
        user_uuid: userId,
        limit_count: 10,
      });
    ...
    const { data: locationHistory, error: locationHistoryError } =
      await supabase
  ...
  candidates.forEach((candidate) => {
    ...
    if (userData) {
      ...
      const categoryPreference = userData.learnedPreferences.find(
        (p) =>
          p.preference_type === "category" &&
          p.preference_key === candidate.category
      );
      if (categoryPreference && categoryPreference.preference_value > 0) {
        const categoryBoost =
          categoryPreference.preference_value * categoryPreference.confidence;
        personalizationScore += categoryBoost * 1.5;
        personalizationReasons.push(
          `Matches your ${candidate.category} preference`
        );
      }
      ...
      if (timePreference && timePreference.preference_value > 0) {
        const timeBoost =
          timePreference.preference_value * timePreference.confidence;
        personalizationScore += timeBoost * 0.5;
        personalizationReasons.push(
          `Matches your ${timeOfDay} activity preference`
        );
      }
      ...
      if (userData.locationContext.frequentLocations.length > 0) {
        ...
        for (const frequentLocation of userData.locationContext
          .frequentLocations) {
          const distance = calculateDistance(
      ...
      const similarInteractions = userData.interactionHistory.filter(
        (interaction) =>
          interaction.interaction_data?.category === candidate.category
      );
      if (similarInteractions.length > 0) {
        const positiveInteractions = similarInteractions.filter((interaction) =>
          ["like", "save", "schedule"].includes(interaction.interaction_type)
        ).length;
        const totalInteractions = similarInteractions.length;

        if (totalInteractions > 0) {
          const interactionBoost =
            (positiveInteractions / totalInteractions) * 0.8;
          personalizationScore += interactionBoost;
          personalizationReasons.push(
            `Based on your ${candidate.category} activity history`
          );
        }
      }
      ...
      if (userData.locationContext.isAtHome) {
        ...
        if (["fine_dining", "drink", "watch"].includes(candidate.category)) {
      ...
      if (userData.locationContext.isAtWork) {
        ...
        if (
          ["drink", "casual_eats", "watch"].includes(candidate.category)
        ) {
          personalizationScore += 0.2;
  ...
  if (!OPENAI_API_KEY) {
    ...
    return candidates.map((candidate) => ({
      ...
      copy: {
        oneLiner: `Discover ${
          candidate.name
        } - a perfect ${candidate.category.replace("_", " ")} experience.`,
        tip: `Great for ${candidate.category.replace(
          "_",
          " "
...
${
  candidate.budgetNearMiss
    ? `Budget Note: Slightly above your $${preferences.budget.min}-${preferences.budget.max} range ($${candidate.estimatedCost} per person)`
    : ""
}
...
Group Size: ${preferences.groupSize || 2} ${
...
- Consider group size: ${
          preferences.groupSize === 1
            ...
            : preferences.groupSize === 2
            ? "ideal for a romantic couple"
            : preferences.groupSize >= 3 && preferences.groupSize <= 6
            ? "great for a small group of friends"
            : "perfect for a large group gathering"
        }
${
  candidate.budgetNearMiss
    ? '- If slightly above budget, frame it as "worth the stretch" or "slightly above your budget but highly rated"'
    : ""
}
...
- Be concise and compelling`;
          ...
          return {
            ...
            copy: {
              oneLiner:
                ...
                `Discover ${
                  candidate.name
                } - a perfect ${candidate.category.replace(
                  "_",
                  " "
                )} experience.`,
              tip:
                parsed.tip ||
                `Great for ${candidate.category.replace(
                  "_",
                  " "
                )} enthusiasts looking for something new.`,
            },
        ...
        console.error(
          "LLM enhancement error for candidate:",
          candidate.id,
          error
        );
      }
...
async function applyAdvancedMLPersonalization(
  ...
  try {
    ...
    mlEnhancedCandidates.sort(
      (a, b) => b.personalizationScore - a.personalizationScore
    );

    console.log(
      "✅ ML personalization applied to",
      mlEnhancedCandidates.length,
      "candidates"
    );
...
function calculateMLScore(
  ...
  const categoryInteractions = interactions.filter(
    (i) => i.interaction_data?.category === candidate.category
  );
  ...
  if (categoryInteractions.length > 0) {
    ...
    const categoryScore =
      (positiveInteractions / categoryInteractions.length) * 100;
    score += (categoryScore - 50) * 0.3;
  }

  // Apply learned preferences
  const relevantPrefs = learnedPrefs.filter(
    (p) =>
      (p.preference_type === "category" &&
        p.preference_key === candidate.category) ||
      (p.preference_type === "price" &&
        Math.abs(p.preference_value - candidate.estimatedCostPerPerson) < 20)
  );

  if (relevantPrefs.length > 0) {
    const avgPreference =
      relevantPrefs.reduce((sum, p) => sum + p.preference_value, 0) /
      relevantPrefs.length;
...
function calculateTimeBasedScore(candidate: any, interactions: any[]): number {
  ...
  const timeInteractions = interactions.filter((i) => {
    ...
    return (
      Math.abs(interactionHour - currentHour) <= 2 &&
      interactionDay === currentDay
    );
  });
  ...
  if (currentHour >= 6 && currentHour <= 10 && hour >= 6 && hour <= 10)
    return 80; // Morning
  if (currentHour >= 11 && currentHour <= 16 && hour >= 11 && hour <= 16)
    return 75; // Afternoon
  if (currentHour >= 17 && currentHour <= 21 && hour >= 17 && hour <= 21)
    return 85; // Evening
