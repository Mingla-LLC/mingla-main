...
const PR_FAMILY_WITHOUT_CONCURRENCY_SHA256 =
  ...
  // `needs`, and each gains a `push: branches: [main]` trigger carrying the SAME
  // one-file `paths-ignore` its pull-request side already had. The batch lane
  ...
  // on main into a permanent silent absence of coverage. #3079 is the standing
  // evidence that incompleteness is a live risk, not a hypothetical one.
  //
  // AND WHY THE PUSH TRIGGER CARRIES `paths-ignore`, which the first attempt did
  // NOT and which the #2524 auto-merge guard caught: an unfiltered push trigger
  // means a baseline-only merge starts all six lanes on `main`, reviving exactly
  // the ~51,000 job-min/month fan-out #2885 AC-4 removed. That guard enumerates
  // which workflows such a merge starts, and all six had appeared in the set.
  // Excluding the one machine-written file on the push side satisfies both: a
  // HUMAN merge runs all six in full, a baseline-only merge starts none. This is
  // the same trade the red-`main` alert landed on this morning, and the residual
  // is precisely one machine-written commit that changes a recorded boot-payload
  // number and nothing these lanes read.
  //
  ...
  // glossed, because "389 insertions, 8 deletions" would otherwise read as an
  ...
  // and no `paths-ignore` ENTRY on the pull-request side was altered.
  //
  // REBASED TWICE. This branch pinned d4e5b8ae... before #3076 merged and
  // 58f17807... before the push filter was added; both were computed on trees
  // that no longer exist and taking either would be a stale pin that merely
  // looked deliberate. Nor is main's 404b2394... correct, since it knows nothing
  // of these seven lanes. The value below was re-derived fresh from the current
  // merged tree.
  //
  // WHAT WAS VERIFIED BEFORE RE-DERIVING, running #3081's checklist:
  //   - CONTAMINATION CHECKED FIRST, per the #3081 lesson: every ADDED line in
  ...
  //   - the workflow diff is "7 files changed, 389 insertions(+), 8
  ...
  //   - #2885 AC-4 holds on BOTH sides, proven by execution rather than reading:
  //     the six `paths-ignore` entry lists are byte-identical to main, the
  //     registry's re-derived `pathScope` is still exactly the one baseline file
  //     (the inspector uniqs it across triggers, so the push filter adds no
  //     entry), the router was RUN against the real baseline-only commit
  //     5692d0358 and all six report `selected=false`, and the #2524 auto-merge
  //     guard passes 56/56 including "merging that baseline does not simply move
  //     the fan-out onto main".
  ...
  //   - THE DELTA IS EXACTLY THIS CHANGE: with all SEVEN lanes restored to their
  //     main bytes the digest COMPUTES to
  ...
  // Every earlier re-derivation is preserved, not replaced.
  //
  // [TEST-MOD-APPROVED #3134] Re-derived again. #3134 added ONE deno test step
  // (plus its comment) to the existing unified-sharing lane, so the cover-video
  // webhook's new identity re-poll runs where every other cover-video edge
  // contract already does. That lane is PR-family, so its non-concurrency
  // document is inside this digest, exactly as every re-derivation above. It is
  // named here the way they all name their lanes — never by its `.yml` path,
  // because a workflow FILENAME written in this file is counted by
  // `discoverWorkflowProviders()` as an external provider reference and moves
  ...
  //
  // WHAT WAS VERIFIED BEFORE RE-DERIVING:
  //   - the change is PURELY ADDITIVE: the workflow diff is 7 insertions and 0
  //     deletions, and grepping added AND removed lines for `concurrency`,
  //     `group:` and `cancel-in-progress` returns ZERO. No concurrency block,
  //     group expression or cancellation value is touched, and no `paths:`
  //     entry is added — the lane already scopes
  //     `supabase/functions/event-cover-video-*/**`, where the new test lives.
  ...
  //
  // [TEST-MOD-APPROVED #3186] Re-derived again. #3186 added exactly TWO lines to
  // the existing secret-budget lane's two test-runner steps, so this issue's new
  // Deno and Node regression suites actually execute in CI — that lane's steps
  // match by explicit glob and filename, so without them the suites would exist
  // and never run, which is half of the defect #3186 exists to end. The lane is
  // named here the way every re-derivation above names its lane — never by its
  // `.yml` path, because a workflow FILENAME written in this file is counted by
  // `discoverWorkflowProviders()` as an external provider reference and would
  // move the frozen #2148 provider seal.
  //
  // Evidence, executed rather than asserted:
  //   - the change is PURELY ADDITIVE: `git diff --stat` over
  //     `.github/workflows/` for `origin/main...HEAD` is `1 file changed,
  //     2 insertions(+)`. No line is removed.
  //   - grepping the added AND removed lines under `.github/workflows/` for
  //     `concurrency`, `group:` and `cancel-in-progress` returns ZERO. No
  //     concurrency block, group expression or cancellation value is touched;
  //     this digest covers the NON-concurrency document, which is precisely
  //     what moved.
  //   - `--diff-filter=ADR` over `.github/workflows/` returns ZERO files. No
  //     workflow was added, removed or renamed, so PR_FAMILY_COUNT and
  //     PR_FAMILY_IDENTITY_SHA256 cannot have moved — corroborated by
  //     execution: subtests 1 and 4 PASS at 124 PR-family and seven non-PR
  //     workflows against this tree.
  //   - THE DELTA IS EXACTLY THIS CHANGE: restoring that one lane to its
  //     `origin/main` bytes recomputes
  //     42772929e794fd18c4fa9a3a771a23cad49391c6f3d9dd64286b495ff013d991,
  //     the value this pin replaces, and subtest 2 goes green — so nothing else
  //     in the tree drifted into this digest.
  //   - the value is stable across consecutive runs (derived twice, identical).
  //   - per #3015 the `actual:` printed by a PR run must not be copied. This
  //     value was derived LOCALLY on the rebased branch, whose workflow set is
  //     the tree a squash merge puts on `main`. If another PR touching any
  ...
  //
  // The touched lanes are the business Jest suite lane and the
  // migrations-and-Stripe Deno lane. They are DESCRIBED, never spelled as
  // filenames: a workflow filename written in this file becomes a provider
  // reference and drifts the frozen #2148 external-reference inventory (see the
  // #2948 note below). Naming them cost nine red #2148 gates once; do not.
  //
  // PURELY ADDITIVE, and auditable from the commit itself: the complete
  // added/removed set under .github/workflows in #3249 is eight lines — a jest
  // step for #1983's account-deletion confirm card, and one more Deno suite
  // path appended to an existing list. Grepping those lines for concurrency /
  // group: / cancel-in-progress returns ZERO. PR_FAMILY_COUNT and
  ...
  //
  // [TEST-MOD-APPROVED #3313] Re-derived on top of that combined tree after
  // #3313 added four whole blocks: its recurring-event SQL suite and edge node
  // suite on the migrations job, the end of the #2333 re-apply set, and its
  ...
  // 9356c425.... MEASURED FROM DISK with this file's RUBY_CANONICAL: current
  // tree c78d72b5...; without the four #3313 blocks 330740ba... (the previous
  // pin, asserted below as the #3313 receipt, and the base every older receipt
  // now starts from).
  "c78d72b58b7c9ed66a70706ac9ea31868d59f715fc0888eb5ba0f884a1fb9149";
...
test("the real tree independently classifies 124 PR-family and seven non-PR workflows", () => {
  ...
  // reproduce the previous combined pin exactly. Every older receipt below now
  // starts from that tree, so each owner's delta stays independently measured.
