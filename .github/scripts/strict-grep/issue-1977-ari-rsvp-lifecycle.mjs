...
const files = {
  migration: "supabase/migrations/20270409001977_issue_1977_ari_rsvp_guest_contribution.sql",
...
export function audit(base) {
  ...
  if (!migration.includes("rsvp_domain_operation_receipts") || !migration.includes("rsvp_idempotency_hash_mismatch") || !migration.includes("pg_advisory_xact_lock")) failures.push("RSVP domain replay authority is incomplete");
  ...
  if (!refund.includes("p_event_id: boundEventId") || !refund.includes('error: "refund_mode_invalid"') || !refund.includes('error: "refund_reason_invalid"')) failures.push("refund edge does not bind exact event/mode/reason");
