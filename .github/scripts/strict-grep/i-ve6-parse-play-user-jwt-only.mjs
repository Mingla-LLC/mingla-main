...
/**
 * I-VE6-PARSE-PLAY-USER-JWT-ONLY (ORCH-0882)
 *
 * parse-play-activities must use caller JWT only — no service role.
 *
...
if (process.argv.includes("--self-test")) {
  ...
  if (self.length) {
    console.error("I-VE6-PARSE-PLAY-USER-JWT-ONLY self-test FAIL:");
    self.forEach((m) => console.error("  - " + m));
    process.exit(1);
  }
  console.log("I-VE6-PARSE-PLAY-USER-JWT-ONLY self-test PASS (3/3 cases).");
  process.exit(0);
}
...
if (violations.length > 0) {
  console.error("I-VE6-PARSE-PLAY-USER-JWT-ONLY FAILED:");
...
console.log(`I-VE6-PARSE-PLAY-USER-JWT-ONLY OK (${TARGET_FILES.length} files scanned)`);
