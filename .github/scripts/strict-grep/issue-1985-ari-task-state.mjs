...
const files = {
  migration: "supabase/migrations/20270403001985_issue_1985_ari_conversation_task_state.sql",
...
export function check(s) {
  const failures = [];
  for (const token of ["task_state jsonb", "task_state_revision bigint", "client_turn_id uuid", "idx_agent_messages_user_client_turn", "WHERE client_turn_id IS NOT NULL", "WHERE role = 'user' AND client_turn_id IS NOT NULL"]) {
    if (!s.migration.includes(token)) failures.push(`migration contract missing ${token}`);
  }
  for (const token of ["TASK_STATE_SCHEMA_VERSION = 1", "MAX_TASK_STATE_BYTES", "parseTaskState", "applySlotUpdates", "beginInterruption", "resumeInterruption", "export function reconcilePendingAction", "assertCreateEventProposal"]) {
    if (!s.state.includes(token)) failures.push(`reducer contract missing ${token}`);
  }
  for (const token of ["chooseEffectiveTimezone", "localDateTimeToInstants", "function endOfMonthChoices(", "invalidReason: \"past\"", "resolved_iso"]) {
    if (!s.time.includes(token)) failures.push(`deterministic-time contract missing ${token}`);
  }
  for (const token of ["extractEventBrief", "jollof", "acrobat", "dateChoices", "export function applyStoredChoice", "assertCreateEventProposal(state)"]) {
    if (!s.planner.includes(token)) failures.push(`planner contract missing ${token}`);
  }
  if (!s.choices.includes("schema_version: 2") || !s.choices.includes("question_id") || !s.choices.includes("slot_patch") || !s.choices.includes("const AFFIRMATIVE_ONLY")) {
    failures.push("typed AgentChoicesV2 contract incomplete");
  }
  if (s.chat.includes("detectChoices") || !s.chat.includes(": validateChoiceSubmission(body.choice_response)") || !s.chat.includes("recoveredTurn?.conversation_id") || !s.chat.includes("existingAssistant") || !s.chat.includes("response_message_id")) {
    failures.push("chat choice/idempotency boundary incomplete");
  }
  for (const token of ["appendSafeSummary", '.eq("task_state_revision", args.expectedRevision)', '.select("task_state, task_state_revision")', "TASK_STATE_CONFLICT", "beginInterruption", "resumeInterruption"]) {
    if (!s.chat.includes(token)) failures.push(`chat state ownership missing ${token}`);
  }
  const authorization = s.confirm.indexOf("await authorizeAgentTool(tool, finalArgs");
  const executing = s.confirm.indexOf('status: "executing"', authorization);
  if (authorization < 0 || executing < authorization || !s.confirm.includes("persistTaskOutcome") || !s.confirm.includes("reconcilePendingAction")) {
    failures.push("confirmation reconcile/authorization ordering incomplete");
  }
  if (!s.service.includes("AgentChoiceSubmissionV2") || !s.service.includes("choice_response?: AgentChoiceSubmissionV2") || !s.service.includes("client_turn_id: string")) {
  ...
  for (const token of ["newClientTurnId", "turnPayloads", "payload, clientTurnId", "local_delivery: \"failed\"", "setPendingAction(unresolved)"]) {
    if (!s.hook.includes(token)) failures.push(`Business retry/refresh contract missing ${token}`);
  }
  if (!s.screen.includes('import { useShareNetworkState } from "../../components/ui/useShareNetworkState";') || !s.screen.includes("You're offline. Reconnect to continue this plan.") || !s.screen.includes("choicesDisabled={chat.isSending || !online}")) {
...
if (process.argv.includes("--self-test")) {
  ...
  const mutations = [
    { key: "migration", from: "task_state_revision bigint", to: "removed_revision" },
    { key: "state", from: "export function reconcilePendingAction", to: "function removedReconcile" },
    { key: "time", from: "function endOfMonthChoices(", to: "function removedEndMonth(" },
    { key: "planner", from: "export function applyStoredChoice", to: "function removedStoredChoice" },
    { key: "choices", from: "const AFFIRMATIVE_ONLY", to: "const removedAffirmativeGuard" },
    { key: "chat", from: ": validateChoiceSubmission(body.choice_response)", to: ": null /* validation removed */" },
    { key: "chat", from: '.eq("task_state_revision", args.expectedRevision)', to: '.eq("id", args.conversationId)' },
  ...
  for (const mutation of mutations) {
    ...
    if (check(mutated).length === 0) {
      console.error(`issue-1985 self-test FAIL: ${mutation.key} revert escaped`);
      process.exit(1);
    }
  }
...
if (failures.length) {
  console.error("issue-1985-ari-task-state FAIL:\n" + failures.map((failure) => `  - ${failure}`).join("\n"));
  process.exit(1);
}
