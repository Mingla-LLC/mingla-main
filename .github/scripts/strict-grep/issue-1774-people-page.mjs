...
export function audit(base) {
  ...
  if (/People you can reach|Reach unavailable|Export unavailable|Book export is coming soon/.test(page) || /followersCount|extendedCircleCount|estimatedReach/.test(page)) failures.push("reach/export dependency is fabricated");
  if (!page.includes('useBrandCircleReach(brand?.id??null,"follower"') || !page.includes('useBrandCircleReach(brand?.id??null,"extended"') || !page.includes('<CircleReachBlock ring="follower"') || !page.includes('<CircleReachBlock ring="extended"')) failures.push("Circle UI is not bound to the server authority");
...
function selfTest() {
  ...
  try {
    ...
    if (!audit(tmp).some((x)=>x.includes("fabricated"))) throw new Error("true mutation: fabricated reach was not detected");
    fs.copyFileSync(path.join(root,files.page),page);
    fs.appendFileSync(page,'\nconst futurePlaceholder = "People you can reach";\n');
    if (!audit(tmp).some((x)=>x.includes("fabricated"))) throw new Error("true mutation: fabricated reach UI was not detected");
    fs.copyFileSync(path.join(root,files.page),page);
