#!/usr/bin/env node
/**
 * ORCH-1272 [venue deck-readiness AI step flashes/closes on web create] —
 * I-PROPOSED-1272-CREATE-LANDS-ON-DURABLE-DECK-READINESS.
 *
 ...
 * I-PROPOSED-1272-CREATE-LANDS-ON-DURABLE-DECK-READINESS ACTIVE).
 */
...
if (process.argv.includes("--self-test")) {
  ...
  const commentOnly =
    goodMount +
    "// ORCH-1272 — no longer mounts <VenueDeckReadinessSetup> from createdVenue.\n";
  f = [];
  check(commentOnly, f);
  if (f.length) self.push("comment mention wrongly flagged: " + f.join("; "));

  if (self.length) {
    console.error("I-PROPOSED-1272-CREATE-LANDS-ON-DURABLE-DECK-READINESS self-test FAIL:");
    self.forEach((m) => console.error("  - " + m));
    process.exit(1);
  }
  console.log(
    "I-PROPOSED-1272-CREATE-LANDS-ON-DURABLE-DECK-READINESS self-test PASS (5/5 cases).",
  );
  process.exit(0);
}
...
if (!fs.existsSync(WIZARD_FILE)) {
  console.error(
    `I-PROPOSED-1272-CREATE-LANDS-ON-DURABLE-DECK-READINESS FAIL — wizard not found at ${WIZARD_FILE}.`,
  );
  process.exit(1);
}
...
if (failures.length > 0) {
  console.error(
    "I-PROPOSED-1272-CREATE-LANDS-ON-DURABLE-DECK-READINESS FAIL:\n  " +
      failures.join("\n  "),
  );
  process.exit(1);
}
console.log(
  "I-PROPOSED-1272-CREATE-LANDS-ON-DURABLE-DECK-READINESS PASS — create tier-1 " +
