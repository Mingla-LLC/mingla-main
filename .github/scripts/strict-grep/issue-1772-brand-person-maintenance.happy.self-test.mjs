...
const tracked = [
  ...
  "supabase/migrations/20270612001772_issue_1772_brand_person_maintenance.sql",
...
const mutations = [
  ["supabase/migrations/20270612001772_issue_1772_brand_person_maintenance.sql", "CREATE OR REPLACE FUNCTION public.biz_guest_roster_set_rsvp_approval(", "", "legacy person-key RSVP projection"],
  ...
  ["supabase/migrations/20270612001772_issue_1772_brand_person_maintenance.sql", "issue_1772_require_brand_rank(p_brand_id,50)", "issue_1772_require_brand_rank(p_brand_id,20)", "rank-50 authority"],
  ...
  ["supabase/migrations/20270612001772_issue_1772_brand_person_maintenance.sql", "SET superseded_at=NULL,superseded_by=NULL,superseded_by_merge_event_id=NULL", "SET superseded_at=now()", "separation restoration"],
  ...
  [".github/ci-batch/MANIFEST.json", '        ".github/scripts/strict-grep/issue-2013-ari-tenant-containment.mjs",\n        "mingla-business/server/__tests__/publicSearchMigration.issue2986.security.test.ts",\n        "scripts/issue-1860/issue-1860-public-tables-rls.tester.adversarial.test.mjs",', '        ".github/scripts/strict-grep/issue-2013-ari-tenant-containment.mjs",\n        "scripts/issue-1860/issue-1860-public-tables-rls.tester.adversarial.test.mjs",', "A4 reviewed #2986 Supabase reference"],
  ...
  [".github/scripts/parity/l5-terminal-mutations.sql", "CREATE OR REPLACE FUNCTION public.l5_issue_1772_keep_rsvp_pending()", "CREATE OR REPLACE FUNCTION public.l5_issue_1772_keep_rsvp_pending_removed()", "A7 L-5 fixture-scoped trigger function"],
  [".github/scripts/parity/l5-terminal-mutations.sql", "AND OLD.approval_status='approved'", "AND OLD.approval_status='pending'", "A7 L-5 post-approval boundary"],
  ...
  ["supabase/migrations/20270612001772_issue_1772_brand_person_maintenance.sql", "CREATE OR REPLACE FUNCTION public.issue_1772_lock_brand_person_address(\n  p_brand_id uuid", "CREATE OR REPLACE FUNCTION public.issue_1772_lock_brand_person_address_removed(\n  p_brand_id uuid", "A6 shared address lock"],
  ["supabase/migrations/20270612001772_issue_1772_brand_person_maintenance.sql", "WHERE b.id=r.batch_id AND b.brand_id=v_challenge.brand_id", "WHERE b.id=r.batch_id", "A6 brand-scoped import scrub"],
  ["supabase/migrations/20270612001772_issue_1772_brand_person_maintenance.sql", "actor_id uuid NOT NULL,", "actor_id uuid NOT NULL REFERENCES auth.users(id),", "A6 immutable actor snapshot"],
  ["supabase/migrations/20270612001772_issue_1772_brand_person_maintenance.sql", "IF v_row.state='completed' THEN", "IF false THEN", "A6 absorbing cleanup"],
