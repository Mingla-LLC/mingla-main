...
// T-10 INVENTORY  exactly 4 filtered lanes, glob counts 17/1/3/15,
//                 zero violations
//
...
// [TEST-MOD-APPROVED #2986] The #2117 lane moves from SIX to SEVEN exact
...
// [TEST-MOD-APPROVED #3193] #3193's migration re-emits the public-search
// source reader and its closing DO block asserts on the resolver #2986
// creates, so the #2117 lane now skips it by EXACT filename in phase 1 and
...
// [TEST-MOD-APPROVED #3285] The #1931 lane moves from ELEVEN globs to TWELVE.
// #3285's migration changes the ON DELETE rule of the ticket_event_dates FK and
// defines a `LANGUAGE sql` helper that reads that table, which on this lane's
// pre-#1931 phase exists only via the already-skipped #2160. The #2492 guard
// itself named the exact filename to add. ONLY counts move: T-10 11 -> 12 (and
// its skipped-file count 11 -> 12), T-23 12 -> 13 (branchCount and globs),
// T-24 11 -> 12 (branchCount), T-2723 11 -> 12 (globs and skipped). Every
// assertion, parser scenario and the exact inventory deepEqual are unchanged.
// [TEST-MOD-APPROVED #3313] Both filtered lanes gain ONE exact skip for
...
// [TEST-MOD-APPROVED #3314] BOTH filtered lanes gain one exact skip for
// 20270704003314, which re-emits the direct checkout bundle from #3284's body to
// add one display key; that `LANGUAGE sql` body reaches objects each lane's phase
// lacks, and the #2492 guard named the exact filename for each. ONLY counts move:
// T-10 14/11 -> 15/12 (and the #1931 skipped-file count 14 -> 15), T-23 15 -> 16
// (branchCount and globs), T-24 14 -> 15 (branchCount), T-2723 14 -> 15 (globs and
// skipped). Every assertion, parser scenario and the exact inventory deepEqual are
// unchanged.
// [TEST-MOD-APPROVED #3197] The #2117 lane gains ONE exact skip for
// 20270706003197, whose closing DO block asserts on #2986's readers and
// validator that the lane's phase 1 omits; it is applied once straight after
// #3193. ONLY T-10's #2117 count moves, 12 -> 13. Every assertion, parser
// scenario and the exact inventory deepEqual are unchanged.
...
// section reader; both `LANGUAGE sql` bodies reach objects each lane's phase
// lacks, and the #2492 guard named the exact filename for each. ONLY counts move:
// T-10 15/13 -> 16/14 (and the #1931 skipped-file count 15 -> 16), T-23 16 -> 17
// (branchCount and globs), T-24 15 -> 16 (branchCount), T-2723 15 -> 16 (globs and
// skipped). Every assertion, parser scenario and the exact inventory deepEqual are
// unchanged.
// [TEST-MOD-APPROVED #3443] BOTH filtered lanes gain one exact skip for
// 20270709000000_unlisted_rsvp_invite_link.sql, which re-emits `pg_public_rsvp_by_slug`;
// its `LANGUAGE sql` body reaches `issue_1931_event_ordinary_read_blocked` and the
// #2489 address/theme helpers, which each lane's phase lacks. Removing the two skips
// makes the real analyser report five C-1 violations naming that file, so they are
// required. ONLY counts move: T-10 16/14 -> 17/15 (and the #1931 skipped-file count
// 16 -> 17), T-23 17 -> 18 (branchCount and globs), T-24 16 -> 17 (branchCount),
// T-2723 16 -> 17 (globs and skipped). Every assertion, parser scenario and the
// exact inventory deepEqual are unchanged.
test("T-10 — lane inventory is exactly 4 lanes at 17/1/3/15 (a blind parser reds here)", () => {
  ...
  assert.deepEqual(inventory, {
    ...
    [LANE]: 17,
    "issue-2117-offering-visibility-gate-tests.yml": 15,
  });
  ...
  assert.equal(pinned.skipped.length, 17);
...
test("T-23 — the two-line branch form is read, and its skip actually takes effect (R-5)", (t) => {
  ...
  assert.equal(lane.branchCount, 18, "`<pattern>)` on one line and `continue ;;` on the next is one branch, not none");
  assert.equal(lane.globs.length, 18);
  assert.ok(
...
test("T-24 — C-4(c) reds on a branch the parser cannot read, where C-4(b) cannot", (t) => {
  ...
  const { lanes, violations } = analyseLanes({ workflowsDir, migrationsDir });
  const lane = lanes.find((l) => l.workflow === LANE);
  assert.equal(lane.branchCount, 17, "precondition: the 3-line branch really is unread");
...
test("T-2723-I1/I2/I3 — exact #2696 skip is present and C-3 fails on its revert", (t) => {
  ...
  assert.equal(lane.globs.length, 17, "the #1931 lane must expose exactly seventeen skip globs");
  assert.equal(lane.skipped.length, 17, "those seventeen globs must resolve exactly seventeen skipped migrations");
