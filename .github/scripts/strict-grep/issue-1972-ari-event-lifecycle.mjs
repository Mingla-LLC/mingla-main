...
export function check(sources) {
  ...
  for (const [name, source] of [["menu", parseMenu], ["play", parsePlay]]) {
    ...
    if (!source.includes('rpc("issue_1973_create_snap_proposals"'))
      failures.push(`${name} parser lacks the server-owned atomic proposal boundary`);
    if (source.includes('.from("agent_pending_actions").insert'))
      failures.push(`${name} parser reintroduces caller-JWT table INSERT`);
...
if (process.argv.includes("--self-test")) {
  ...
  const mutations = [
    ...
    { ...sources, parseMenu: sources.parseMenu.replace("issue_1973_create_snap_proposals", "removed_snap_boundary") },
