...
export function checkContract(fixture) {
  assert.equal(fixture.ledger.capabilities.length, 116, "canonical ledger must stay exactly 116 rows");
  assert.equal(new Set(fixture.ledger.capabilities.map((row) => row.id)).size, 116, "capability IDs must be unique");
  ...
  assert.equal(fixture.schema.properties.capabilities.minItems, 116, "evidence schema row floor");
  assert.ok(
  ...
  need(fixture.certifier, [
    "ledger.capabilities.length !== 116",
...
function selfTest() {
  ...
  bad(good, (x) => { x.certifier = x.certifier.replace("ledger.capabilities.length !== 116", "false"); }, "row completeness");
...
if (process.argv.includes("--self-test")) selfTest();
  ...
  console.log("issue-2060 Ari reliability foundation: PASS (116 capabilities)");
}
