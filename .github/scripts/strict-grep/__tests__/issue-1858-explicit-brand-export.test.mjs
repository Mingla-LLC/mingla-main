...
const valid = {
  migration: "DROP FUNCTION public.biz_export_brand_people(text,uuid,text,text,text,jsonb,uuid); p_brand_id uuid DEFAULT NULL IF p_scope IS NULL OR p_filter IS NULL OR p_sort IS NULL WHERE b.id=p_brand_id AND b.deleted_at IS NULL biz_brand_effective_rank(v_brand,v_actor)<public.biz_role_rank('brand_admin') [[:space:]]+",
  ...
  // [TEST-MOD-APPROVED #1858] CLOSE advances the guarded lifecycle state.
  invariant: "I-PROPOSED-BRAND-EXPORT-EXPLICIT-TARGET-1 (ACTIVE)",
};
...
test("#1858 guard rejects invariant lifecycle regression", () => {
  const drift = { ...valid, invariant: "I-PROPOSED-BRAND-EXPORT-EXPLICIT-TARGET-1 (DRAFT)" };
  assert.deepEqual(inspect(drift), ["missing ACTIVE invariant"]);
});
