...
//
// [TEST-MOD-APPROVED #3313] Counts only, same method: #3313's migration is
// registered by exact filename in the #1931 lane's pre-#1931 phase and in the
// #2117 lane (it re-emits the checkout bundle, a `LANGUAGE sql` body reaching
// #2160's and #2489's objects). Every derived #1931-lane pin shifts by one:
// 12 -> 13 (read side, T-13/T-16/T-17/T-18), 11 -> 12 (T-12), 13 -> 14 (T-22
// read side and census), 12 -> 13 (three-line side). No assertion logic changed.
//
...
//
// [TEST-MOD-APPROVED #3284] Counts only, same method: #3284's migration is
// registered by exact filename in BOTH filtered lanes (it re-emits the direct
// checkout bundle and the public experience reader, whose `LANGUAGE sql` bodies
// reach objects each lane's phase lacks). The inventory moves 13/1/3/10 ->
// 14/1/3/11; no assertion in this suite pins the #2117 lane's count, so only the
// derived #1931-lane pins shift by one: T-13 13 -> 14, T-12 12 -> 13,
// T-16/T-17/T-18 13 -> 14, T-22 14 -> 15 (read side) and 13 -> 14 / census
// 14 -> 15 (three-line side). No assertion logic changed.
//
// [TEST-MOD-APPROVED #3314] Counts only, same method: #3314's migration
// (20270704003314, which re-emits the direct checkout bundle from #3284's body to
// add one display key) is registered by exact filename in BOTH filtered lanes. The
// inventory moves 14/1/3/11 -> 15/1/3/12; only the derived #1931-lane pins shift by
// one: T-13 14 -> 15, T-12 13 -> 14, T-16/T-17/T-18 14 -> 15, T-22 15 -> 16 (read
// side) and 14 -> 15 / census 15 -> 16 (three-line side). No assertion logic changed.
//
// [TEST-MOD-APPROVED #3426] Counts only, same method: #3426's migration
// (20270707003426, which re-emits `pg_public_brand_upcoming` and adds the brand
// section reader) is registered by exact filename in BOTH filtered lanes. The
// inventory moves 15/1/3/13 -> 16/1/3/14; only the derived #1931-lane pins shift by
// one: T-13 15 -> 16, T-12 14 -> 15, T-16/T-17/T-18 15 -> 16, T-22 16 -> 17 (read
// side) and 15 -> 16 / census 16 -> 17 (three-line side). No assertion logic changed.
//
// [TEST-MOD-APPROVED #3443] Counts only, same method: the unlisted RSVP invite
// link migration (20270709000000_unlisted_rsvp_invite_link.sql, which re-emits
// `pg_public_rsvp_by_slug`) is registered by exact filename in BOTH filtered lanes;
// without it the analyser reports five C-1 violations. The inventory moves
// 16/1/3/14 -> 17/1/3/15; only the derived #1931-lane pins shift by one: T-13 16 -> 17,
// T-12 15 -> 16, T-16/T-17/T-18 16 -> 17, T-22 17 -> 18 (read side) and 16 -> 17 /
// census 17 -> 18 (three-line side). No assertion logic changed.
...
test("T-13 — a lane rewritten to the basename nested-quote form is still discovered (R-2)", (t) => {
  ...
  assert.equal(lane.globs.length, 17);
  assert.equal(lane.skipped.length, 17, "subject-correct matching (R-4) must still resolve all ten files");
...
test("T-12 — independent fails-on-revert, built differently from the implementor's fixture", (t) => {
  ...
  const lane = lanes.find((l) => l.workflow === LANE);
  assert.equal(lane.globs.length, 16);
...
test("T-16 — a `;;&` fall-through terminator does not hide a branch", (t) => {
  ...
  const { lanes, violations } = analyseLanes({ workflowsDir, migrationsDir });
  const lane = lanes.find((l) => l.workflow === LANE);
  assert.equal(lane.globs.length, 17, "a `;;&` terminator must not drop the branch it terminates");
...
test("T-17 — the leading-paren branch form `(glob)` is still read", (t) => {
  ...
  const { lanes, violations } = analyseLanes({ workflowsDir, migrationsDir });
  assert.equal(lanes.find((l) => l.workflow === LANE).globs.length, 17);
...
test("T-22 — R-5's scope boundary, both sides: the form it reads, and the form only the census catches", (t) => {
  ...
  // ---- in scope: R-5 reads it, and the skip is real -----------------------
  {
    ...
    const { lanes, violations } = analyseLanes({ workflowsDir, migrationsDir });
    const lane = lanes.find((l) => l.workflow === LANE);
    assert.equal(lane.branchCount, 18, "R-5 must read the two-line branch form");
    assert.equal(lane.globs.length, 18, "and extract its glob");
    assert.ok(
  ...
  // ---- out of scope: three lines, unread, and only the census sees it ------
  {
    ...
    const { lanes, violations } = analyseLanes({ workflowsDir, migrationsDir });
    const lane = lanes.find((l) => l.workflow === LANE);
    assert.equal(lane.branchCount, 17, "R-5 is scoped to the two-line form; a three-line branch stays unread");
    assert.equal(
      lane.skipped.includes(REAL_UNSKIPPED),
      false,
    ...
    const census = branchCensus(fs.readFileSync(path.join(workflowsDir, LANE), "utf8"), lane);
    assert.equal(census.terminators, 18);
    assert.equal(census.continues, 18);
    assert.ok(
