...
const eventCoverMedia = fs.readFileSync(
  path.join(root, "packages/offering-rendering/EventCoverMedia.tsx"),
  "utf8",
);
const packageIndex = fs.readFileSync(
  path.join(root, "packages/offering-rendering/index.ts"),
  "utf8",
);
...
if (!packageIndex.includes("EventCoverMedia")) {
  failures.push("@mingla/offering-rendering must export the shared EventCoverMedia contract.");
}
