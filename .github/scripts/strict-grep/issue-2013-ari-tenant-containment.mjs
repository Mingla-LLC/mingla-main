...
export function check(sources) {
  const failures = [];
  const { helper, chat, confirm, tools, domain, sites, prompt, hook, screen, input, list, publicMigration, scopeMigration, turnMigration, scopeTest, migrationWorkflow, writerTest, ciManifest } = sources;
  for (const token of [
  ...
  if (!/\.select\(\s*"summary,brand_id,title,task_state,task_state_revision"\s*\)[\s\S]{0,160}\.eq\("id", conversationId\)[\s\S]{0,100}\.eq\("user_id", userId\)/.test(chat)) {
    failures.push("conversation lifecycle lacks one id/user scoped canonical context chain");
  }
  ...
  // [TEST-MOD-APPROVED #3429] #3429 registers named service-only SQL writers;
  // the test still rejects unscoped provenance and read/insert mispairing.
  for (const token of ['"agent-chat": 1', '"agent-confirm-action": 1', 'claim_agent_chat_turn', 'append_agent_chat_tool_result', 'read cannot be paired']) {
  ...
  const claimWindow = turnMigration.slice(
    turnMigration.indexOf("CREATE OR REPLACE FUNCTION public.claim_agent_chat_turn"),
    turnMigration.indexOf("CREATE OR REPLACE FUNCTION public.commit_agent_chat_assistant_turn"),
  );
  if (!chat.includes('serviceClient.rpc(\n    "claim_agent_chat_turn"')
      || !claimWindow.includes("p_user_id uuid") || !claimWindow.includes("p_brand_id uuid")
      || !claimWindow.includes("TO service_role")) {
    failures.push("#3429 turn claim is not explicit-scope and service-only");
  }
  ...
  for (const token of ["brandEpoch.current += 1", "setConversationId(null)", "setPendingAction(null)", "sendIntentRef.current = null", "turnsRef.current = []", "subscriptions.current.clear()", "setErrorMessage(null)", "setErrorCode(null)"]) {
  ...
  // [TEST-MOD-APPROVED #3429] InputBar splits permanent/hydration and
  // send-readiness props, while its effective canSend must require both.
...
if (process.argv.includes("--self-test")) {
  ...
  const turnScopeReverted = check({ ...sources, chat: sources.chat.replace('serviceClient.rpc(\n    "claim_agent_chat_turn"', 'userClient.rpc(\n    "claim_agent_chat_turn"') });
  const turnScopeRevertDetected = turnScopeReverted.some((failure) => failure.includes("turn claim is not explicit-scope"));
  const turnClaimReverted = check({ ...sources, turnMigration: sources.turnMigration.replace("CREATE OR REPLACE FUNCTION public.claim_agent_chat_turn", "CREATE OR REPLACE FUNCTION public.removed_agent_chat_turn") });
  const turnClaimRevertDetected = turnClaimReverted.some((failure) => failure.includes("turn claim is not explicit-scope"));
  const resetReverted = check({ ...sources, hook: sources.hook.replace("turnsRef.current = [];", "turnsRef.current = turnsRef.current;") });
  const resetRevertDetected = resetReverted.some((failure) => failure.includes("brand-switch reset"));
  const writerRegistryReverted = check({ ...sources, writerTest: sources.writerTest.replaceAll("append_agent_chat_tool_result", "unregistered_agent_chat_tool_result") });
  const writerRegistryRevertDetected = writerRegistryReverted.some((failure) => failure.includes("writer-registry tester"));
  ...
  if (good.length > 0 || !revertDetected || !historyRevertDetected || !confirmationRevertDetected || !writerProvenanceRevertDetected
      ...
      || !turnScopeRevertDetected || !turnClaimRevertDetected || !resetRevertDetected || !writerRegistryRevertDetected
      || !terminalLifecycleAccepted || !forgedLifecycleDetected || !wrapperDecoupled) {
    console.error("issue-2013 self-test FAIL", { good, reverted, historyReverted, confirmationReverted, writerProvenanceReverts, shadowReverted, scopeReverted, composerReverted, siteRegistryReverted, siteExecutorReverted, turnScopeReverted, turnClaimReverted, resetReverted, writerRegistryReverted, acceptedLifecycles, forgedLifecycles, wrapperDecoupled });
    process.exit(1);
  }
  console.log(`issue-2013 self-test PASS: clean source passes; owner, history, confirmation-writer, #3429 turn scope/claim/reset/writer registry, ${writerProvenanceReverts.length} typed writer-provenance, immutable-scope, composer-gate, Sites registry/executor and ${forgedLifecycles.length} forged-lifecycle reverts fail; ${acceptedLifecycles.length} reviewed lifecycles pass and the deletable CI wrapper is decoupled from provider ${CI_BATCH_PROVIDER}.`);
  process.exit(0);
}
