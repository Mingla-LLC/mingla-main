...
const check = (s) => {
  for (const token of ["paystack_transaction_id", "stripe_charge_id", "issue_2079_capture_ticket_paid_identity_attention", "issue_2079_verify_ticket_paid_identity", "claim_source_refund_operations", "buyer_state='needs_attention'", "financial_state='needs_attention'"]) {
  ...
  if (!s.worker.includes('row.reason.startsWith("paid_provider_")') || !s.worker.includes("paid_provider_identity_pending")) fail("paid identity can false-terminalize");
  ...
  for (const token of ["issue_2079_paystack_late_refund_identity.happy.test.ts", "issue_2079_paystack_late_refund_identity.test.sql", "issue-2079-paystack-late-refund-identity.mjs --self-test"]) {
