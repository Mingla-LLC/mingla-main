...
| Post-launch reliability & polish (parked to Later 2026-07-18 by Seth) | Trust & operational hardening | The platform stays trustworthy as it scales — booking deadlines actually enforce, capped events can't oversell, invite errors read clearly, releases stay guarded, and adding admins is safe. | `ORCH-1390` (silently-failing booking-deadline cron — investigate-first of the cluster; may mean deadlines aren't enforced) · `ORCH-1391` (sell-out double-booking race; zero impact until an event caps) · `ORCH-1393` (invite-accept web page shows a confusing spinner/500 to the wrong visitor) · `ORCH-1389` (harden the framework-upgrade + security CI guards; add an early native-bundle build check) · `ORCH-1394` (admin pre-login gate must be redesigned before onboarding a database-only admin; inert while the sole admin is hardcoded) | `Mingla_Artifacts/WORLD_MAP.md` Issue Registry (each ORCH stanza carries full evidence) | Booking deadlines enforced; zero oversell at cap; zero silent-fail crons; clean invite-error UX; guard blind-spots closed | Medium | `Later / parked` |
