 ...
 * Takes the second-to-last segment as city and last segment as country.
 */
...
function SearchImportView({ addToast }) {
  ...
  // Used to force-remount MapContainer on new searches
  ...
  const existingCount = searchResults.filter((p) =>
    existingPlaceIds.has(p.googlePlaceId)
  ).length;
  const newOnlyCount = searchResults.filter(
    (p) => !existingPlaceIds.has(p.googlePlaceId)
  ).length;
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      // Check which places already exist in pool
      if (places.length > 0) {
        ...
        if (mountedRef.current) {
          setExistingPlaceIds(
            new Set((existing || []).map((e) => e.google_place_id))
          );
        }
      }
  ...
  const handlePush = useCallback(async () => {
    ...
    try {
      ...
      const placesToPush = searchResults.filter((p) =>
        selectedPlaces.has(p.googlePlaceId)
      );
      ...
      addToast({
        variant: "success",
        title: `${result.total} places pushed to pool`,
      });

      // Refresh existing IDs so badges update
  ...
  }, [selectedPlaces, searchResults, addToast]);
  ...
  const selectNewOnly = useCallback(() => {
    setSelectedPlaces(
      new Set(
        searchResults
          .filter((p) => !existingPlaceIds.has(p.googlePlaceId))
    ...
    () => [
      {
        ...
        render: (_, row) => (
          <button
            ...
            className="w-5 h-5 rounded border border-[var(--gray-300)] flex items-center justify-center cursor-pointer hover:border-[#f97316] transition-colors"
            aria-label={
              selectedPlaces.has(row.googlePlaceId) ? "Deselect" : "Select"
            }
          >
      ...
      },
      {
        ...
        render: (_, row) => (
          <div className="flex items-center gap-2">
            ...
            {existingPlaceIds.has(row.googlePlaceId) && (
              <Badge variant="info">In Pool</Badge>
            )}
          </div>
        ),
      },
  ...
  // Prepare rows with a stable key for DataTable
  ...
  return (
    <div className="space-y-5">
      ...
      <SectionCard
        title="Search Places"
        subtitle="Each search uses 1 Google API call"
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <Input
            ...
            placeholder="e.g., rooftop bars, museums, best coffee shops"
          />
        </div>
        <Button
          icon={Search}
        ...
        <SectionCard
          ...
          subtitle={
            !searching && searchResults.length > 0
              ? `${selectedCount} selected · ${existingCount} already in pool`
              : undefined
          }
          noPadding
        >
          ...
          {searchResults.length > 0 && (
            <div className="flex items-center justify-between px-5 py-3 border-b border-[var(--gray-200)]">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {existingCount > 0 && (
                  <Button size="sm" variant="ghost" onClick={selectNewOnly}>
                    Select New Only ({newOnlyCount})
                  </Button>
                )}
              </div>
          ...
          {viewMode === "table" && (
            <DataTable
              ...
              loading={searching}
              emptyIcon={Search}
              emptyMessage="No places found. Try a different search."
            />
          )}
              ...
              >
                ...
                {searchResults.map((place) => (
                  <Marker
                    ...
                    icon={
                      selectedPlaces.has(place.googlePlaceId)
                        ? selectedIcon
                        : defaultIcon
                    }
                  >
                    <Popup>
                      <div style={{ minWidth: 180, fontFamily: "inherit" }}>
                        ...
                        <span style={{ fontSize: 12 }}>
                          Rating: {formatRating(place.rating)} · Reviews:{" "}
                          {place.reviewCount}
                        </span>
                        {existingPlaceIds.has(place.googlePlaceId) && (
                          <>
                            <br />
                            <em style={{ fontSize: 11, color: "#3b82f6" }}>
                              Already in pool
                            </em>
                          </>
                        )}
                        ...
                        <button
                          onClick={() =>
                            toggleSelection(place.googlePlaceId)
                          }
                          style={{
                            marginTop: 6,
                            padding: "3px 10px",
...
function BrowsePoolView({ addToast }) {
  ...
  const [poolFilter, setPoolFilter] = useState("active"); // "active" | "inactive" | "all"
  ...
  const fetchPool = useCallback(async () => {
    ...
    try {
      ...
      if (poolSearch.trim())
        query = query.ilike("name", `%${poolSearch.trim()}%`);
      if (cityFilter.trim())
        query = query.ilike("address", `%${cityFilter.trim()}%`);
      if (typeFilter.trim())
        query = query.contains("types", [typeFilter.trim()]);
  ...
  // Clear selections that are no longer on the current page
  useEffect(() => {
  ...
  const handleToggleActive = useCallback(
    async (row) => {
      ...
      if (
        !newStatus &&
        !window.confirm(
          `Deactivate "${row.name}"? It will be hidden from the active pool.`
        )
      ) {
        return;
      }
      ...
      try {
        ...
        addToast({
          variant: "success",
          title: `${row.name} ${newStatus ? "reactivated" : "deactivated"}`,
        });
        fetchPool();
      } catch (err) {
        addToast({
  ...
  const handleRefreshPlace = useCallback(
    async (googlePlaceId) => {
      ...
      try {
        ...
        addToast({
          variant: "success",
          title: `Refreshed ${result.refreshed} place${result.refreshed !== 1 ? "s" : ""}`,
        });
        fetchPool();
      } catch (err) {
        addToast({
  ...
  const handleBulkDeactivate = useCallback(async () => {
    ...
    if (
      !window.confirm(
        `Deactivate ${selectedRows.size} selected places?`
      )
    )
      return;
    ...
    try {
      ...
      addToast({
        variant: "success",
        title: `${ids.length} places deactivated`,
      });
      setSelectedRows(new Set());
      fetchPool();
    } catch (err) {
  ...
  const handleBulkRefresh = useCallback(async () => {
    ...
    try {
      ...
      addToast({
        variant: "success",
        title: `Refreshed ${result.refreshed} of ${ids.length} places`,
      });
      setSelectedRows(new Set());
      fetchPool();
    } catch (err) {
  ...
  const toggleSelectAllPage = useCallback(() => {
    ...
    if (allSelected) {
      ...
      setSelectedRows(new Set(allOnPage));
    }
    ...
    () => [
      {
        ...
        render: (_, row) => (
          <button
            ...
            className="w-5 h-5 rounded border border-[var(--gray-300)] flex items-center justify-center cursor-pointer hover:border-[#f97316] transition-colors"
            aria-label={selectedRows.has(row.id) ? "Deselect" : "Select"}
          >
      ...
      },
      {
        ...
        render: (val) =>
          val ? (
            <Badge variant={PRICE_TIER_VARIANT[val] || "default"}>
              {val}
            </Badge>
          ) : (
            <span className="text-[var(--color-text-muted)]">—</span>
      ...
      },
      {
        ...
        render: (val) =>
          val ? (
            <Badge variant="outline">{val.replace(/_/g, " ")}</Badge>
          ) : (
            <span className="text-[var(--color-text-muted)]">—</span>
          ),
      },
        ...
        render: (_, row) => (
          <div className="flex items-center gap-1">
            <Button
              ...
              variant="ghost"
              icon={RefreshCw}
              loading={refreshingIds.has(row.google_place_id)}
              onClick={() => handleRefreshPlace(row.google_place_id)}
              aria-label="Refresh place data"
            />
            <Button
              size="sm"
              variant="ghost"
  ...
  const paginationFrom =
    poolCount === 0 ? 0 : poolPage * PAGE_SIZE + 1;
  const paginationTo = Math.min(
    (poolPage + 1) * PAGE_SIZE,
    poolCount
  );
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        <SearchInput
          ...
          onClear={() => { setPoolSearch(""); setPoolPage(0); }}
          placeholder="Search by name..."
          className="w-full sm:w-56"
        />
        <Input
          value={cityFilter}
          onChange={(e) => { setCityFilter(e.target.value); setPoolPage(0); }}
          placeholder="Filter by city..."
          className="!h-10 sm:!w-44"
        />
        <Input
          value={typeFilter}
          onChange={(e) => { setTypeFilter(e.target.value); setPoolPage(0); }}
        ...
        <div className="flex items-center gap-1">
          {["active", "inactive", "all"].map((f) => (
            <button
              ...
              className={[
                "px-3 py-1.5 text-xs font-medium rounded-full border cursor-pointer transition-all duration-150",
                poolFilter === f
      ...
      {selectedRows.size > 0 && (
        <div className="flex items-center gap-2 px-1">
          ...
          <Button size="sm" variant="secondary" icon={RefreshCw} onClick={handleBulkRefresh}>
            Refresh Selected
          </Button>
          {poolFilter !== "inactive" && (
            <Button size="sm" variant="danger" icon={EyeOff} onClick={handleBulkDeactivate}>
              Deactivate Selected
            </Button>
          )}
        </div>
      ...
      {poolData.length > 0 && (
        <div className="px-1">
          <Button size="sm" variant="ghost" onClick={toggleSelectAllPage}>
            {poolData.every((p) => selectedRows.has(p.id))
      ...
      <DataTable
        ...
        emptyMessage={
          poolFilter === "inactive"
            ? "No inactive places."
            : "No places in the pool yet. Use Search & Import to add some."
        }
        striped
        pagination={{
          page: poolPage,
  ...
  useEffect(() => {
    ...
    async function fetchStats() {
      ...
      try {
        ...
        // Fetch ALL active places in batches of 1000 (Supabase default limit)
        ...
        while (hasMore) {
          const { data: batch } = await supabase
            .from("place_pool")
            .select(
              "address, types, primary_type, price_tier, rating, created_at"
            )
            .eq("is_active", true)
        ...
        for (const p of places) {
          ...
          if (!cityMap[key]) {
            cityMap[key] = { city, country, count: 0, totalRating: 0, ratedCount: 0 };
          }
          cityMap[key].count++;
          if (p.rating != null) {
            cityMap[key].totalRating += p.rating;
            cityMap[key].ratedCount++;
          }
        }
        const cities = Object.values(cityMap)
          .map((c) => ({
            ...c,
            avgRating: c.ratedCount > 0 ? c.totalRating / c.ratedCount : null,
          }))
        ...
        for (const p of places) {
          ...
          if (!typeMap[type]) {
            typeMap[type] = { type, count: 0, totalRating: 0, ratedCount: 0, tiers: {} };
          }
          typeMap[type].count++;
          if (p.rating != null) {
            typeMap[type].totalRating += p.rating;
            typeMap[type].ratedCount++;
          }
          if (p.price_tier) {
            typeMap[type].tiers[p.price_tier] =
              (typeMap[type].tiers[p.price_tier] || 0) + 1;
          }
        }
        const types = Object.values(typeMap)
          .map((t) => ({
            ...t,
            avgRating: t.ratedCount > 0 ? t.totalRating / t.ratedCount : null,
            topTier: Object.entries(t.tiers).sort(
              (a, b) => b[1] - a[1]
            )[0]?.[0] || "—",
          }))
          .sort((a, b) => b.count - a.count)
        ...
        for (const p of places) {
          if (p.price_tier && tiers[p.price_tier] !== undefined) {
            tiers[p.price_tier]++;
          }
        }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div>
          ...
          />
          {totalInactive > 0 && (
            <p className="text-xs text-[var(--color-text-tertiary)] mt-1 ml-1">
        ...
        <div className="space-y-3">
          {Object.entries(tierDistribution).map(([tier, count]) => (
            <div key={tier} className="flex items-center gap-3">
              <div className="w-16 text-sm font-medium text-[var(--color-text-primary)]">
                {tier}
              </div>
              <div className="flex-1 h-7 bg-[var(--gray-100)] rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${maxTierCount > 0 ? (count / maxTierCount) * 100 : 0}%`,
                    backgroundColor:
                      tier === "chill"
                        ? "#22c55e"
                        : tier === "comfy"
              ...
              <div className="w-12 text-sm text-[var(--color-text-secondary)] text-right">
                {count}
              </div>
            </div>
          ))}
        ...
        {cityBreakdown.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr>
                  <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[var(--color-text-tertiary)] bg-[var(--table-header-bg)] border-b border-[var(--table-border)]">
                    City
                  </th>
                  <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[var(--color-text-tertiary)] bg-[var(--table-header-bg)] border-b border-[var(--table-border)]">
                    Country
                  </th>
                  <th className="px-5 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--color-text-tertiary)] bg-[var(--table-header-bg)] border-b border-[var(--table-border)]">
                    Places
                  </th>
                  <th className="px-5 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--color-text-tertiary)] bg-[var(--table-header-bg)] border-b border-[var(--table-border)]">
                    Avg Rating
                  ...
                  >
                    ...
                    <td className="px-5 py-3 text-right">
                      {t.topTier !== "—" ? (
                        <Badge
                          variant={
                            PRICE_TIER_VARIANT[t.topTier] || "default"
                          }
                        >
                          {t.topTier}
                        </Badge>
