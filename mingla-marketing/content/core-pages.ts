...
export type CorePageSlug = 'about' | 'for-explorers' | 'cities'
...
export const CORE_PAGES: Readonly<Record<CorePageSlug, CorePageRecord>> = {
  ...
  // #3371 — renamed from `/explorer` (which now permanently redirects here) so
  // the menu's Explorer item can mean the home page. H1, description and body
  // copy are unchanged; only the eyebrow and the title prefix say For Explorers.
  'for-explorers': {
    slug: 'for-explorers', pathname: '/for-explorers', lifecycle: CORE_PAGE_RELEASE_LIFECYCLE,
    title: 'For Explorers: Date Plans, Events & City Gems | Mingla',
...
export function allCoreTrustPagesSearchReady(): boolean {
  return allCityHubsSearchReady() &&
    ...
    CORE_PAGES['for-explorers'].lifecycle === 'search_ready' &&
