...
export default function AboutPage() {
  ...
  return <CorePageShell>
    ...
    <AnswerBlock question={record.directQuestion} answer={record.directAnswer} crumbs={[{ name: 'Home', path: '/' }, { name: 'About Mingla', path: '/about' }]} />
    <CutoutSection id="two-jobs"><div className="core-section-heading"><h2>One platform, two jobs.</h2><p>People need a decision they can act on. Hosts need a clear way to make the experience understandable and available.</p></div><div className="core-two core-two--product-story">
      <article className="core-card core-product-story"><h3>For people making the plan</h3><div className="core-product-story-copy"><p>Explore places, events, trips and experiences; compare what fits the people, timing, budget, location and mood; save and share ideas; and take the real reservation, RSVP or ticket action when one is available.</p><p>Mingla does not match strangers. It helps couples, friends, groups and solo Explorers make real-world plans.</p></div><figure className="core-product-proof core-product-proof--explorer"><Image src="/product-proof/explorer-saved-details.png" alt="Mingla Explorer showing the saved Sample sunset gallery plan open in its details sheet." width={1206} height={2622} sizes="(min-width: 768px) 40vw, 92vw" /></figure><DeviceCta surface="explorer" location="about_explorer" label="Explore Your City" /></article>
      <article className="core-card core-product-story"><h3>For people making the experience possible</h3><div className="core-product-story-copy"><p>Mingla Host is for organisers, restaurants, bars, cafés, venues, activity spaces, experience brands, trip operators and independent creators. Hosts can present, publish, promote and run only the workflows Mingla currently supports.</p></div><figure className="core-product-proof core-product-proof--host"><Image src="/product-proof/host-live-public-page.png" alt="Mingla Host showing the Sample rooftop listening session open in its public event page with a free-ticket action." width={1206} height={2622} sizes="(min-width: 768px) 40vw, 92vw" /></figure><DeviceCta surface="host" location="about_host" label="Host Your City" /></article>
    </div></CutoutSection>
    ...
    <CutoutSection id="launch-cities"><div className="core-section-heading"><h2>Where Mingla is launching.</h2><p>Lagos, Durham, Cary, Raleigh, New York City, Brussels, Paris, London, Fort Lauderdale and Washington, DC each have their own Mingla city page.</p></div><CityDirectory /></CutoutSection>
