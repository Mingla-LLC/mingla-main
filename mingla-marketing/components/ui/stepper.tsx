...
// ORCH-1010 — adapted from the reui Stepper (21st.dev). Logic kept verbatim;
// shadcn color tokens re-themed to Mingla's warm system. Used by the
// "What is Mingla?" section's vertical, auto-advancing explainer.
...
function StepperTrigger({ asChild = false, className, children, tabIndex, ...props }: StepperTriggerProps) {
  ...
  return (
    <button
      ...
      className={cn(
        'cursor-pointer outline-none focus-visible:z-10 disabled:pointer-events-none disabled:opacity-60',
        className,
      )}
...
function StepperIndicator({ children, className }: React.ComponentProps<'div'>) {
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        'relative flex size-10 shrink-0 items-center justify-center overflow-hidden rounded-full text-xs transition-colors duration-300',
        'bg-[var(--color-warm-tint)] text-[var(--color-warm-ink)]',
        'data-[state=completed]:bg-warm data-[state=completed]:text-white',
        'data-[state=active]:bg-warm data-[state=active]:text-white',
        className,
      )}
    >
...
function StepperSeparator({ className }: React.ComponentProps<'div'>) {
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        'm-0.5 rounded-full bg-black/10 transition-colors duration-300 data-[state=completed]:bg-warm group-data-[orientation=vertical]/stepper-nav:h-12 group-data-[orientation=vertical]/stepper-nav:w-0.5 group-data-[orientation=horizontal]/stepper-nav:h-0.5 group-data-[orientation=horizontal]/stepper-nav:flex-1',
        className,
      )}
    />
  )
