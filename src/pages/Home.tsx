...
const Home = () => {
  ...
  const calculateTravelInfo = (fromLocation: string, toLat: number | null, toLng: number | null, travelMode: string, constraint: 'time' | 'distance'): string => {
    ...
    if (constraint === 'time') {
      ...
      const distance = travelMode === 'walk' ? travelTime * 0.08 :
      // ~5 km/h walking speed
      travelMode === 'drive' ? travelTime * 0.5 :
      // ~30 km/h average city speed
      travelTime * 0.3; // ~18 km/h public transport

      return measurementSystem === 'metric' ? `${distance.toFixed(1)} km` : `${(distance * 0.621371).toFixed(1)} mi`;
    }
  };
  ...
  }, [activePreferences.categories, activePreferences.budgetRange, activePreferences.groupSize, activePreferences.time, activePreferences.travel, activePreferences.travelTime, activePreferences.travelDistance, activePreferences.location]);
  ...
  const {
    ...
    error
  } = useExperiences(experienceFilters);
  ...
  const fetchRealData = useCallback(async () => {
    ...
    try {
      ...
      const placesPromises = activePreferences.categories.map(category => supabase.functions.invoke('places', {
        body: {
          lat,
          lng,
          radiusMeters: activePreferences.travelConstraint === 'distance' ? activePreferences.travelDistance * 1000 : 10000,
          // Increased to 10km radius for better results
      ...
      const eventsPromise = supabase.functions.invoke('events', {
        body: {
          location: `${lat},${lng}`,
          radius: activePreferences.travelConstraint === 'distance' ? activePreferences.travelDistance : 10 // 10km default radius
        }
      });

      // Execute all API calls
      const [placesResults, eventsResult] = await Promise.all([Promise.all(placesPromises), eventsPromise]);
      ...
      if (eventsResult.data && Array.isArray(eventsResult.data.events)) {
        allExperiences = allExperiences.concat(eventsResult.data.events.map((event: any) => ({
          ...event,
      ...
      const formattedTrips = budgetFiltered.slice(0, 10).map(exp => {
        ...
        const estimatedTravelTime = calculateTravelTime(activePreferences.location, exp.lat, exp.lng, activePreferences.travel);
        ...
        const travelInfo = calculateTravelInfo(activePreferences.location, exp.lat, exp.lng, activePreferences.travel, activePreferences.travelConstraint);
  ...
  const sentSessions = availableSessions.filter(session => session.invitedBy === user?.id && (session.status === 'pending' || session.status === 'dormant'));
  ...
  const handleSwipeRight = async () => {
    if (currentTrip) {
      ...
      const {
        writeThroughHelpers
      } = await import('@/store/writeThroughHelpers');
  ...
  const acceptExperience = async (experienceId: string, scheduledDate?: Date) => {
    ...
    const result = await writeThroughHelpers.scheduleExperience(experienceId, scheduledDate ? scheduledDate.toISOString() : new Date().toISOString());
    if (result.success) {
      toast({
  ...
  return <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5 relative overflow-hidden touch-optimized">
      {/* Background Pattern */}
      ...
      <div className="relative z-10 px-6 pt-safe pb-6">
        <div className="flex items-center justify-center mb-6">
          <img src={minglaLogo} alt="Mingla" className="h-12 w-auto" onError={e => {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={fetchRealData} disabled={isLoading} className="bg-background/80 backdrop-blur-sm border-primary/20">
    ...
    <>
        ...
        {currentTrip ? <div className="relative w-full max-w-sm">
            ...
            <Card className="relative overflow-hidden bg-gradient-to-br from-card to-card/80 backdrop-blur-sm border-primary/10 shadow-xl">
              ...
              <CardContent className="p-0 relative">
                <div className="relative">
                  <img src={currentTrip.image} alt={currentTrip.title} className="w-full h-96 object-cover" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
                  
                  {/* Floating badges */}
                  <div className="absolute top-4 left-4 flex flex-wrap gap-2">
                    {currentTrip.badges?.slice(0, 2).map((badge, index) => <Badge key={index} variant="default" className="bg-black/80 text-white backdrop-blur-sm text-xs font-medium border border-white/20 shadow-lg">
                        {badge}
                      </Badge>)}
                  </div>
                  ...
                  <div className="absolute top-4 right-4">
                    <Badge variant={isInSolo ? "outline" : "default"} className={cn("backdrop-blur-sm text-xs font-medium border shadow-lg", isInSolo ? "bg-black/80 text-white border-white/20" : "bg-primary text-primary-foreground border-primary/20")}>
                      {isInSolo ? <>
            ...
            <div className="flex justify-center gap-6 mt-8">
              <Button size="lg" variant="outline" onClick={handleSwipeLeft} className="w-16 h-16 rounded-full border-2 border-muted hover:border-destructive hover:bg-destructive/10 group">
                <X className="h-6 w-6 group-hover:text-destructive" />
              </Button>
              
              <Button size="lg" onClick={() => setExpandedTrip(currentTrip.id)} className="w-20 h-16 rounded-full bg-primary/10 hover:bg-primary/20 text-primary border-2 border-primary/20">
                <Sparkles className="h-6 w-6" />
              </Button>

              <Button size="lg" onClick={handleSwipeRight} className="w-16 h-16 rounded-full bg-primary hover:bg-primary/90 group">
            ...
            <Button onClick={() => setShowPreferences(true)} className="bg-primary hover:bg-primary/90">
      ...
      return <TripCardExpanded trip={foundTrip} preferences={activePreferences} isOpen={!!expandedTrip} onClose={() => setExpandedTrip(null)} onAccept={() => {
      ...
      {/* Preferences Sheet */}
      <PreferencesSheet isOpen={showPreferences} onClose={() => setShowPreferences(false)} activePreferences={activePreferences} onPreferencesUpdate={handlePreferencesUpdate} />
    </div>;
};
export default Home;