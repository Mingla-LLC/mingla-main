...
- **Regression test:** fails-on-revert — removing any of the three `serverRowToDraft` reads makes the chip-in-ON assertion red (the field comes back `false`/`null`). Append-only.
- **Established:** DRAFT 2026-07-27 at issue #1026 SPEC. Flips ACTIVE at CLOSE.

### I-PROPOSED-1026-DRAFT-MAPPER-COMPILE-COMPLETE (DRAFT)
...
- **Enforcement:** batch:A strict-grep gate on `serverDraftEventMapper.ts` asserting `serverRowToDraft` contains no `as DraftEvent` / `as unknown as DraftEvent` and does contain a `satisfies DraftEvent` (or a bare-return under the typed signature); plus the existing `npm run typecheck` (`tsc`) green gate.
- **Regression test:** fails-on-revert — re-introducing `as DraftEvent` (or dropping a required-field read) turns the gate red and/or surfaces a `tsc` error. Append-only.
- **Established:** DRAFT 2026-07-27 at issue #1026 SPEC. Flips ACTIVE at CLOSE.
