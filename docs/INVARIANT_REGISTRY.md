...
## DRAFT — issue #1369 (business team "Remove member" / "Revoke invitation" / "Disconnect partner" confirmations never appeared — both ConfirmDialogs rendered as root siblings of the open sheet, and the disconnect success toast fired in the sheet's close tick; Tier 2 of the #1342 modal-collision sweep — IN FLIGHT, registered at IMPLEMENT 2026-07-29)
...
### I-PROPOSED-1369-TEAM-MEMBER-CONFIRM-NESTED-IN-SHEET (DRAFT)
...
- **Enforcement:** append-only source-grep test `mingla-business/src/components/team/__tests__/MemberDetailSheet.confirmNestedInSheet.issue1369.source.test.ts` (Part A: exactly two `<ConfirmDialog`, BOTH indices between `<Sheet ` and `</Sheet>`, ZERO after `</Sheet>`, root JSX matches `/return\s*\(\s*<Sheet\b/` and NOT `/return\s*\(\s*<>/`; Part B: the disconnect toast fires through `deferAfterDismiss(() => setToast("Partner disconnected"))` and NOT the synchronous `onPartnerDisconnected={() => setToast(...)}`), modeled on `CoverPickerSheet.nestedToast.issue1356.test.tsx`. Fails-on-revert: restoring either confirm to a root sibling after `</Sheet>` (or the fragment root) turns the Part-A pins RED; reverting the toast to the synchronous form turns T-7 RED. Verified fails-on-revert at implementation commit `b5fe035bfd9cc4b39dee7ca1e85a3e29e5f5feac`.
...
- **Established:** DRAFT 2026-07-29 at issue #1369 IMPLEMENT. Flips ACTIVE at CLOSE after independent tester PASS.
