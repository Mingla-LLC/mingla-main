...
- **Enforcement (as shipped in PR #1247):** the in-code `} satisfies DraftEvent;` closure of `serverRowToDraft` + the `tsc` typecheck run in the full mingla-business suite + the T-4 compile-completeness test in the append-only `serverDraftEventMapper.test.ts` (wired into the PR-blocking `issue-1022-theme-control-tests.yml`). Together these make dropping a required-field read a hard `tsc`/ts-jest error. NOTE: a dedicated batch:A strict-grep gate asserting the literal absence of `as DraftEvent` / `as unknown as DraftEvent` was specified but NOT built — without it, a `satisfies`→`as` revert that does NOT also drop a field would restore the masking silently. That strict-grep gate is a recommended low-cost hardening follow-up.
...
- **Established:** ACTIVE 2026-07-27 at issue #1026 CLOSE (PR #1247 merged). Enforcement partial pending the optional strict-grep hardening noted above.
