...
## DRAFT — issue #1360 (business order refund/cancel/door-refund confirmation toasts silently dropped on iOS while the sheet was still dismissing — the close→toast race; Tier 3 of the #1342 modal-collision sweep)
...
### I-PROPOSED-1360-PAYMENT-CONFIRM-DEFERRED-PAST-DISMISSAL (DRAFT)
...
- **Enforcement:** `mingla-business/src/utils/__tests__/deferAfterDismiss.test.ts` (timing contract: the deferred callback fires strictly AFTER the sheet unmount window, not synchronously — fake timers) + `mingla-business/src/utils/__tests__/paymentConfirmDefer.issue1360.test.ts` (source-wiring pins over all four handlers: each closes-first and routes EVERY `showToast` through `deferAfterDismiss`, modeled on `CoverPickerSheet.nestedToast.issue1356.test.tsx`). Fails-on-revert: reverting any handler to the pre-fix `showToast(...) ; close()` shape turns its wiring pin RED (deferred count < showToast count); making `deferAfterDismiss` synchronous turns the timing test RED (callback observed before any timer advance). Verified at commit `3df1b33a5`.
- **Established:** DRAFT at #1360 SPEC (registry entry pre-staged at #1360 IMPLEMENTATION); flips ACTIVE at #1360 CLOSE after tester PASS.
