...
- **Never start Metro with `CI=1`.** `CI=1 npx expo start` puts Metro in CI mode, where it prints `Metro is running in CI mode, reloads are disabled` and stops watching files. It then keeps serving the graph it built when it started, so every edit you make afterwards is invisible: the app reloads, downloads a bundle, and runs your OLD code. The failure has no error and no warning — it looks exactly like the bug you were trying to fix still being there, and you can burn an hour re-diagnosing a defect you already fixed (#3429 REWORK-2 lost two cold rebuilds to it). Start it without `CI`, keep the local-stack env, and an edit hot-refreshes: `EXPO_PUBLIC_SUPABASE_URL=http://127.0.0.1:<kong port> EXPO_PUBLIC_SUPABASE_ANON_KEY=<local anon> npx expo start --dev-client --port <port> --localhost`. If you inherit a Metro someone else started, assume nothing and apply the rule above: confirm the app shows something that exists only in the commit under test.
- **If HEAD moves during a device pass, the evidence you already captured may be stale.** A merge landing mid-run (#3429 REWORK-4 moved `81885958d` → `487dfecfd` while simulators were being driven) invalidates every screenshot and accessibility frame taken before it if the merge touched a file behind a finding. Before publishing, re-verify byte-identity of the files behind each finding against the new HEAD, or re-run the pass.
- **`npm install --no-save` is correct in CI and destructive in an orch worktree.** The `#1486` dormant-render workflow installs `react-test-renderer` and `@testing-library/react-native` that way, and CI provides them at the repo root. Run the same line inside a worktree and it prunes a **nested transitive** (`expo/node_modules/expo-asset`), after which Metro cannot bundle at all — while `git status` stays clean and jest, `tsc` and strict-grep all keep passing over a tree that cannot build. Run it **last**, or in a throwaway checkout, and restore with `npm ci`. **Catch it with a bundle fetch, not a resolve probe:** `require.resolve("expo-asset")` failing from the package root is NORMAL (it is nested, not a direct dependency), so the only honest check is requesting a bundle from Metro and asserting HTTP 200 with zero `Unable to resolve module` lines — on both platforms, on a port nobody else owns.
- **React Native needs a COLD app relaunch to re-measure font scale.** Changing iOS Dynamic Type or Android `settings put system font_scale` on a running app leaves stale measurements, so the first reading is wrong (#3429 RETEST-2 had to discard one). Kill and relaunch between settings. Related: a node-height ratio of 1.00 does NOT prove the text ignored the setting — a fixed `lineHeight` pins the line box while `fontSize` scales inside it.
- **The Android emulator's keyboard plumbing goes dead after heavy driving, and looks exactly like a pass.** The IME reports `mInputShown=true` while the app stays at its RESTING layout: the composer never rises, the empty state never clamps, and a screenshot of that is indistinguishable from a correct capture. It survives repeated cold relaunches and then recovers on its own (#3429 REWORK-5 on `ISSUE3429-Pixel7`; the tester hit the same shape and nearly filed it as a P1 for “invisible typed characters”). The rule: before trusting ANY keyboard-dependent capture on Android, assert the composer actually moved — compare its top edge before and after focus — and re-run until it does. Related trap from the same pass: `uiautomator dump` needs all three animation scales at 0 to reach idle, but `react-native-keyboard-controller` stops reporting a keyboard height at 0, so accessibility frames and a live keyboard clamp cannot be captured in the same run.
- **On the Android emulator the Metro reverse rides IPv6, so `/proc/net/tcp` is EMPTY and says nothing.** The obvious check — `adb shell cat /proc/net/tcp` and look for the port in hex — returns nothing for a reverse that is working perfectly, because `adb reverse` lands on the IPv6 loopback. Read **`/proc/net/tcp6`** instead. Measured on #3429's emulator leg: `tcp6` held **19 sockets on `1F9F` (8095, the Metro actually in use) and zero on `8081`**, while `tcp` was empty for both — so anyone following the `tcp` form literally concludes the device is connected to nothing and goes hunting a networking fault that does not exist. Check both files, and treat `tcp` alone as unmeasured rather than as a negative. It also answers the question that matters when several Metros are running — WHICH port the device is actually talking to — which no amount of host-side `lsof` can tell you.
- **A module-scope import of an expo package can kill a render suite before your component renders a single node.** Several expo modules build an `EventEmitter` against a native global AT IMPORT TIME, so merely having the import at module scope is enough: a suite that only mounts a screen dies with `Cannot read properties of undefined (reading 'EventEmitter')`, pointing at a file the test never meant to exercise. Worse, **the offenders hide behind each other** — the import graph stops at the first one, so each fix reveals exactly one more, and you cannot enumerate them up front (four surfaced one at a time on #3429). Budget for iteration rather than reading the failure as "one bad import". The repair is to **resolve at use**, not at module scope: keep the import lazy (inside the function that needs it) so nothing touches the native global until something actually calls it. `orch-1296-no-toplevel-boot-fragile-native-import.mjs` enforces this, but only for the packages it names, so a package outside its list reintroduces the same failure with the gate green — that gap is filed as **#3527**.
...
- **Class A runs as three shards, and its full results live in `class-a-shard-aggregate` (#3449).** Adding a `batch:A` gate is unchanged: add the script and ONE `MANIFEST.json` entry, and the gate is placed in a shard automatically — membership is computed by `run-batch.mjs`'s `classAShardPlan()` from the registry, `.github/scripts/strict-grep/class-a-shard-costs.json` and `MANIFEST.json`'s `classAShardCount`. Never put a `shard` field on a gate row, never convert the shard jobs to a `strategy: matrix` (its `fail-fast` default cancels siblings, and a cancelled shard classifies as a neutral concurrency eviction rather than a failure), and never enable `merge-multiple` on the aggregate's artifact download (every shard writes a file of the same name, so merging silently overwrites two shards' rows). `gate-results-A` is now **shard 1's rows only** — it keeps that name because the name is inside shard 1's sealed step array — so acceptance tooling must download `class-a-shard-aggregate`, which holds every execution exactly once in the same row shape plus an additive `shard` field. That aggregate is named OUTSIDE the `gate-results-A*` namespace on purpose: a completeness job whose own upload matches its own download pattern gets a fourth file on a "Re-run failed jobs" click and can stay red until the whole workflow is re-run, so the plan gate asserts statically that the two cannot collide. Run one shard locally with `STRICT_GREP_CLASS_A_SHARD=<n> node .github/scripts/strict-grep/run-batch.mjs --class A`, and check the plan with `node .github/scripts/strict-grep/issue-3449-class-a-shard-plan.mjs`.
- **Refresh the class A shard cost table by hand, reviewed, never automatically (#3449).** `for r in <three or more run ids>; do gh run download "$r" -n class-a-shard-aggregate -D /tmp/costs/"$r"; done`, then `node scripts/ci/refresh-class-a-shard-costs.mjs --results /tmp/costs --top 50` to read the key diff and the projected shard loads, and again with `--write` to commit. It refuses fewer than three samples because single-run row timings vary by about ±1.2 s on shared runners. A stale table is a balance question, never a correctness one: an uncosted execution takes the committed default and still runs exactly once, and every run's log prints `costed=<k> defaulted=<m>` so the rot is visible without anyone going looking.
