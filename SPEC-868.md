...
`.github/scripts/strict-grep/issue-0868-cover-gallery.mjs` (register in the MANIFEST; origin+delta counter per the rebase-safe pattern). Assert: (1) migration `*issue_868*cover_gallery.sql` exists + declares `cover_media_gallery` + the `_is_array` CHECK; (2) the additive write paths (`eventCoverMediaService.ts` `setEventCoverGallery`, `tripsService.ts`, the publish/live RPCs incl. `business_publish_trip_draft` + `biz_publish_experience`) reference `cover_media_gallery`; (3) `event-cover-video-apply/index.ts` does **NOT** reference `cover_media_gallery` (negative grep — proves the video path stays independent); (4) each of the 4 anon hero RPCs projects `coverGallery`; (5) `CoverGalleryRow.tsx` + `CoverGalleryPager.tsx` exist, `ParallaxCoverShell.tsx` references `galleryImages`, and each of the 3 consumer detail screens references `CoverGalleryRow`/`CoverGalleryPager`; (6) the RSVP `<Head>` in business `PublicEventPage.tsx` contains `og:image`. Any miss ⇒ non-zero exit (fails-on-revert of the whole feature).

### I.5 Second-pass (Section M) coverage tests (append-only, fails-on-revert)
- **Consumer render:** `coverGalleryPager.test.tsx` (`packages/offering-rendering/__tests__/`) — `CoverGalleryPager` renders `1 + N` pages (page 0 = `coverNode`, incl. a video cover), `onScroll` maps offset→index, empty gallery ⇒ page-0-only. PLUS a per-screen guard (RN test) asserting each consumer screen renders the pager + `CoverGalleryRow` when `coverGallery.length ≥ 1` and the SINGLE `<EventCoverMedia>` (no pager/row, byte-identical) when empty — for event, trip, AND experience.
- **Consumer read:** unit asserting `useConsumerTripDetail`/`useConsumerExperienceDetail` map `coverGallery` from the RPC payload, and `mapConsumerEventToFoundation` threads `card.coverGallery` (default `[]`).
- **Trip/experience authoring + publish:** unit asserting `TripCreatorStep1Basics.handleCoverChange` forwards `patch.coverGallery`; and a DB round-trip asserting `business_publish_trip_draft` + `biz_publish_experience` persist `cover_media_gallery` while leaving the cover fields UNCHANGED (coexistence).
- **Providers-into-gallery:** unit asserting the "Add to: Gallery" mode appends a `{url,type}` provider item (clamp 8) WITHOUT mutating the cover fields.
- **Tester adversarial (extends I.3):** drive author→publish→CONSUMER-view for trip + experience + event on iOS sim + plugged-in Android: set a video cover + 3-photo gallery via the trip/experience authoring steps, publish, open the consumer detail screen, and verify the hero sequence = [video cover]++photos, swipe/tap flips + rings, body scrolls the row away, no sideways scroll, single-cover byte-identical when gallery empty. Confirm marketing previews still show the primary still (M.4). Seth eyeball before PASS.
...
**app-mobile (Pass 1 buyer-web-style read hooks):** the read hooks touched for buyer-web parity.

**mingla-marketing:** **NONE** — the Trip/Event PreviewClient replicas stay primary-only (M.4); Pass-1 mount-table #11/#12 are NOT `ParallaxCoverShell` mounts and take no `galleryImages` (correction).

**Second pass — Section M (app-mobile consumer + trip/exp authoring):**
- Shared pkg: `packages/offering-rendering/CoverGalleryPager.tsx` (NEW).
- Consumer render: `app-mobile/src/screens/Event/ConsumerEventDetailScreen.tsx`, `.../Trip/ConsumerTripDetailScreen.tsx`, `.../Experience/ConsumerExperienceDetailScreen.tsx` (screen-level pager + `CoverGalleryRow`).
- Consumer read: `app-mobile/src/hooks/useConsumerTripDetail.ts` (map), `useConsumerExperienceDetail.ts` (map), `useConsumerEventFoundation.ts` (model + mapper), `app-mobile/src/services/publicEventSeedService.ts` (select + map), `app-mobile/src/types/mergedDiscover.ts` (`BusinessEventCard.coverGallery`).
- Trip/exp authoring: `mingla-business/src/components/trip/TripCreatorStep1Basics.tsx` (+ trip draft model), `EditPublishedTripScreen.tsx` (confirm patch passthrough), `mingla-business/src/components/experience/ExperienceCoverStep.tsx` + its wizard/edit parent handler.
- Publish RPCs (new migration files, additive; cover fields VERBATIM): `business_publish_trip_draft`, `biz_publish_experience`.
- Authoring providers-into-gallery: `mingla-business/src/components/ui/CoverPicker.tsx` (Add-to Cover/Gallery segmented mode — M.3).

**Tests/gates:** `coverGalleryRow.test.tsx`, `parallaxCoverGallery.test.tsx`, `coverGalleryPersist.test.ts`, `coverGalleryPager.test.tsx` + per-screen guards (I.5), `.github/scripts/strict-grep/issue-0868-cover-gallery.mjs` (+ MANIFEST registration), `docs/INVARIANT_REGISTRY.md` (DRAFT invariants).
...
**Second pass — Section M (same worktree, same merge):**
10. **Consumer read** — thread `coverGallery`: trip/exp hook mappers (RPCs already return it), event seed-service select + `BusinessEventCard` + foundation model (M.1d).
11. **Shared pager** — `CoverGalleryPager.tsx` (M.1a) using the `ImageGallery` paging pattern.
12. **Consumer render** — wire the 3 detail screens (pager in `nativeCover` + `CoverGalleryRow` as `nativeBody`'s first child; spacer `pointerEvents:none` in gallery mode) (M.1b/c); byte-identical when empty.
13. **Trip/exp authoring** — persist `coverGallery` in the trip/experience create + edit steps (M.2); add the additive `cover_media_gallery` write to `business_publish_trip_draft` + `biz_publish_experience` (new migration files, Management-API apply + curl-verify).
14. **Providers-into-gallery** — CoverPicker "Add to: Cover/Gallery" mode (M.3).
15. **Tests + gate updates** — I.5, extend I.4 gate for consumer + trip/exp publish RPCs.
16. **Runtime-validate consumer iOS + Android** (author→publish→consumer view, video cover + gallery) + Seth eyeball; confirm marketing previews unchanged (M.4).
...
## M. Coverage completion (v1 full — SECOND BUILD PASS)

Seth chose "complete everything, one merge." Pass 1 shipped buyer-web + business (native/desktop) hero galleries, event/RSVP authoring, share/OG + RSVP fix, DB + 4 anon RPCs + 2 views. This pass closes IMPL-868.md gaps **D-1..D-4** so all 5 shipping surfaces + all authoring paths carry the gallery. **All Pass-1 work stays intact** — additive, independent-of-cover, image/GIF-only, no video in gallery. This section is a set of ADDITIVE deltas; it changes nothing in Sections A–J beyond what the I/K/L amendments below state.

### M.0 Fresh investigation findings (this pass)
- **Consumer app does NOT use `ParallaxCoverShell`.** All three consumer detail screens hand-roll the same native pinned-cover pattern inside a gorhom `BaseBottomSheet`: an absolute-sibling `<View style={styles.nativeCover} pointerEvents="none"><EventCoverMedia .../><coverScrim/><ThemeEntranceAnimation/></View>` BEHIND a `BottomSheetScrollView` whose content is `<View style={styles.coverSpacer}/>` (`aspectRatio 4/5`) + `<View style={styles.nativeBody}>{OfferingBody}</View>`. Evidence: `ConsumerEventDetailScreen.tsx:1003-1048`, `ConsumerTripDetailScreen.tsx:965-1008`, `ConsumerExperienceDetailScreen.tsx:931-963` (styles `:1111,:1127`). `CoverGalleryRow`/the pager reach these screens ONLY if added at the SCREEN level — Pass 1's injection was inside `ParallaxCoverShell`, which consumer never mounts. app-mobile currently has **zero** `coverGallery`/`cover_media_gallery` references.
- **Consumer READ paths — trip/experience ALREADY carry the gallery from Pass 1; event does not.** `useConsumerTripDetail.ts` fetches via `supabase.rpc("pg_public_trip_by_slug", …)` (`:350`) and maps `p.coverMediaUrl` (`:437`); `useConsumerExperienceDetail.ts` fetches `pg_public_experience_by_slug` (`:273`) and maps `p.coverMediaUrl` (`:205`). **Those two RPCs were amended in Pass 1 (§C.1) to return `coverGallery`** — so trip/experience only need a one-line mapper add. The EVENT detail does NOT use an anon RPC: `useConsumerEventFoundation.mapConsumerEventToFoundation(card, …)` (`:90-173`) maps from a deck-seed `BusinessEventCard`; the cold-path seed read is `publicEventSeedService.ts` (explicit cover-col select `:28`, map `:138`). So the event needs the gallery threaded through the seed read + card type + foundation model.
- **Marketing previews are bespoke growth-tools replicas, NOT the shared shell.** `EventPreviewClient.tsx` / `TripPreviewClient.tsx` call `fetchEventPreview(runId)`/`fetchTripPreview(runId)` from `@/lib/growth-tools-submit` (`:11,:92`/`:11,:101`) and render a single `<img src={render.cover_url}>` (`:156`/`:164`). They consume a growth-tools render payload with ONE `cover_url` field — no shared renderer, no `PublicEventProps`. (Pass 1's mount table #11/#12 mislabeled these as `ParallaxCoverShell` mounts — CORRECTION in K below: they are not shell mounts and take no `galleryImages`.)

### M.1 — D-1 CONSUMER APP (event · trip · experience native detail)
Add a SCREEN-LEVEL cover pager over `[cover] ++ gallery` + `CoverGalleryRow` beneath, in each of the three consumer detail screens, WITHOUT editing `EventCoverMedia` (DO-NOT-TOUCH). Preserve exact single-cover behavior when the gallery is empty.

**M.1a — NEW shared pinned pager: `packages/offering-rendering/CoverGalleryPager.tsx`** (so all three screens — and optionally, later, `ParallaxCoverShell` — reuse ONE implementation; RN-pure, uses the `ImageGallery.tsx:100-146` paging pattern verbatim):
```ts
interface CoverGalleryPagerProps {
  coverNode: React.ReactNode;             // page 0 — the screen's EXISTING <EventCoverMedia> (unchanged)
  gallery: OfferingGalleryImage[];        // pages 1..N (image/GIF)
  activeIndex: number;                    // controlled by the screen (shared with CoverGalleryRow)
  onActiveIndexChange: (i: number) => void;
  scrollRef?: React.Ref<ScrollView>;      // so the row's onSelect can scrollTo a page
  width: number;                          // measured cover-box width (paging math)
}
```
- Renders a horizontal `pagingEnabled` `ScrollView` (`onScroll` → `Math.round(offsetX / layoutMeasurement.width)` → `onActiveIndexChange`, exactly `ImageGallery.tsx:54-59`): page 0 = `coverNode` (the EXISTING `EventCoverMedia`, video-capable, byte-identical), pages 1..N = one `EventCoverMedia` per gallery item (`mediaType = item.type ?? "image"`, `height/width "100%"`). Must be `pointerEvents="auto"` (the screen's `nativeCover` is `pointerEvents="none"` today — see gesture note).

**M.1b — Per-screen wiring (identical shape in all three):** the screen owns `const [coverIndex, setCoverIndex] = useState(0)` + `const coverPagerRef = useRef<ScrollView>(null)` + a measured cover width. Then:
- In `nativeCover`: when `gallery.length >= 1`, replace the single `<EventCoverMedia>` with `<CoverGalleryPager coverNode={<EventCoverMedia … the current props …/>} gallery={gallery} activeIndex={coverIndex} onActiveIndexChange={setCoverIndex} scrollRef={coverPagerRef} width={coverWidth} />` and set the `nativeCover` `pointerEvents` to `"auto"`; when `gallery.length === 0`, render the EXISTING single `<EventCoverMedia>` UNCHANGED (byte-identical, `pointerEvents="none"`). Keep `coverScrim` + `ThemeEntranceAnimation` as overlays.
- As the FIRST child of `nativeBody` (before the OfferingBody): when `gallery.length >= 1`, render `<CoverGalleryRow cover={{ url: coverMediaUrl, type: coverMediaType }} gallery={gallery} activeIndex={coverIndex} onSelect={i => { setCoverIndex(i); coverPagerRef.current?.scrollTo({ x: i * coverWidth, animated: true }); }} palette={palette} variant="phone" />`. It scrolls away with the body over the pinned pager (parallax preserved by the existing `coverSpacer`).
- Files: `app-mobile/src/screens/Event/ConsumerEventDetailScreen.tsx` (cover `:1003-1023`, body `:1039`), `.../Trip/ConsumerTripDetailScreen.tsx` (cover `:965-982`, body `:1004`), `.../Experience/ConsumerExperienceDetailScreen.tsx` (cover `:931-...`, body). RSVP renders inside `ConsumerEventDetailScreen`'s `!isRsvp` sibling branch — apply the SAME cover+row for the RSVP branch (it shares the pinned `nativeCover`).

**M.1c — Gesture arbitration:** SAME plan + risk as §E.3/RISK-1 (the screen's pinned pager is behind the `BottomSheetScrollView`). Row-tap + `scrollTo` is the guaranteed control; horizontal swipe is the enhancement — set the `coverSpacer` (`ConsumerEventDetailScreen.tsx:1038`, trip `:1003`, exp `:963`) to `pointerEvents="none"` in gallery mode and runtime-validate; fallback = tap chevrons (`ImageGallery.tsx:148-170`). Do NOT pull in `react-native-gesture-handler` without amendment.

**M.1d — Consumer READ path (thread `coverGallery`):**
- **Trip:** `useConsumerTripDetail.ts` — add `coverGallery: OfferingGalleryImage[]` to the detail model (near `:183`) and map `p.coverGallery ?? []` in the RPC mapper (`:437`). The RPC already returns `coverGallery` (Pass 1 §C.1). No SQL change.
- **Experience:** `useConsumerExperienceDetail.ts` — add `coverGallery` to the model (near `:74`/`:151`) and map `p.coverGallery ?? []` (`:205`). RPC already returns it. No SQL change.
- **Event (deck-seed path):** add `coverGallery: OfferingGalleryImage[]` to (1) `app-mobile/src/types/mergedDiscover.ts` `BusinessEventCard`; (2) `app-mobile/src/services/publicEventSeedService.ts` — add `cover_media_gallery` to the column select (`:28`) + map `row.cover_media_gallery ?? []` (`:138`); (3) `useConsumerEventFoundation.ts` `ConsumerEventFoundationModel` (`:26-83`) + `mapConsumerEventToFoundation` return (`:144-172`) → `coverGallery: Array.isArray(card.coverGallery) ? card.coverGallery : []`. Warm deck cards that lack it default `[]` (no regression). (The `discover-cards` deck edge fn + `deckService.ts` are thumbnail-only and stay UNCHANGED — the detail's gallery comes from the cold-path seed read.)
- Each screen passes `gallery = fnd.coverGallery` / `detail.coverGallery` into M.1b. Empty ⇒ single cover, byte-identical.

### M.2 — D-3 TRIP / EXPERIENCE AUTHORING + initial-publish persistence
The steps already host `CoverPickerSheet` (whose `CoverPatch` now carries `coverGallery` from Pass 1 §D.3) — they currently DROP it. Wire it through.
- **Trip create:** `TripCreatorStep1Basics.tsx` `handleCoverChange` (`:200-211`) forwards only `coverMediaUrl`/`coverMediaType`; ADD `coverGallery: patch.coverGallery` to the `onChange({...})` and pass `initial.coverGallery` (default `[]`) into `<CoverPickerSheet initial={…}>` (`:576-594`). The trip draft model/store gains `coverGallery` (mirror §D.3 `tripsService` types). **Trip live-edit** (`EditPublishedTripScreen.tsx`) routes through `updateTripBasics` (§G.5) + `biz_update_live_trip` (§G.4) — already wired in Pass 1; confirm the edit screen passes `coverGallery` through its patch (consistent).
- **Experience:** `ExperienceCoverStep.tsx` forwards the whole patch via `onCoverChange` (`:179`) to the experience wizard/edit parent — the PARENT's cover handler must persist `patch.coverGallery` into the experience draft and pass `initial.coverGallery` back down (props `:42-43`). Name the experience create wizard + edit screen that own that handler (grep `onCoverChange` consumers under `mingla-business/src/components/experience/`).
- **Trip INITIAL publish RPC — `business_publish_trip_draft`** (latest `supabase/migrations/20270108001014_issue_1014_free_only_publish_currency_relax.sql:1147`; cover locals `:1164-1170`): RE-PUBLISH VERBATIM + **additive** delta — add `v_cover_media_gallery jsonb := COALESCE(p_draft_payload->'cover_media_gallery','[]'::jsonb)` and `cover_media_gallery = v_cover_media_gallery` in the `UPDATE public.events SET …`. Cover-field reads/writes stay VERBATIM (no derive). Land as a NEW migration file after `20270116000868`.
- **Experience INITIAL publish RPC — `biz_publish_experience(uuid,jsonb,boolean)`** (latest `supabase/migrations/20261009000000_orch_1153_recurrence_topup_and_guard.sql:230`): SAME additive delta (read `v_cover_media_gallery`, write `cover_media_gallery = v_cover_media_gallery`), cover fields VERBATIM. New migration file.
- Both apply via Management API + `curl`-verify `coverGallery` round-trips while the cover fields are unchanged.

### M.3 — D-4 PROVIDERS-INTO-GALLERY (GIPHY + Pexels → gallery, not only cover)
Extend `CoverPicker.tsx` so a chosen provider item can be APPENDED to the gallery, not only set as the cover.
- Add a small segmented control at the top of the GIF + Photos tabs: **"Add to: Cover · Gallery"** (default **Cover** — today's behavior byte-identical). When **Gallery** is selected, tapping a `GridTile` appends `{ url: result.mediaUrl, type: kind === "gif" ? "gif" : "image", alt: result.alt, credit: result.credit }` to the gallery (clamp 8; toast "Up to 8 extra photos" at cap) via the same append path as the device "Add photo" (§F.1), and does NOT touch the cover fields. When **Cover** is selected, `selectGiphy` (`:828-836`) / `selectPexels` (`:880-888`) keep setting the cover exactly as today.
- No video (GIF/image only). The move-menu ("Make cover / Move earlier / Move later / Remove") applies to provider-added items identically.

### M.4 — D-2 MARKETING PREVIEWS (decision: STAY PRIMARY-ONLY — acceptable)
`EventPreviewClient.tsx` / `TripPreviewClient.tsx` are **not** offering surfaces — they are growth-tools marketing preview replicas fed by a server render payload (`EventPreviewRender`/`TripPreviewRender`) that exposes a SINGLE `cover_url` and render a lone `<img>` (`EventPreviewClient.tsx:11,92,156`; `TripPreviewClient.tsx:11,101,164`). **Decision: they REMAIN primary-cover-only.** Rationale: (1) they consume a growth-tools payload, not `PublicEventProps` — threading the gallery would require extending the growth-tools backend render contract (`@/lib/growth-tools-submit` + the server that builds `cover_url`) plus bespoke pager JSX, all OUTSIDE the #868 offering read/render path; (2) a marketing preview's job is a single representative still, and the primary cover IS that still (unchanged); (3) zero user-facing offering regression. **Follow-up (not #868):** if a gallery is ever wanted here, extend the growth-tools render payload with a `gallery: {url,type}[]` field + a small carousel — tracked as a growth-tools task, not this issue. No files change in this pass.

---

## Downstream routing
**Next = mingla-implementor** (build side), inside `868-cover-gallery` worktree, following the allowlist (Section K, incl. the M additions) + stop-and-amend on anything outside it. Then **mingla-tester** (adversarial I.3 across all 5 surfaces + consumer iOS/Android + Seth eyeball). Then **orchestrator CLOSE** (flip DRAFT invariants ACTIVE, register the strict-grep gate in the MANIFEST, ship-log line in `REPORTS.md`).
