...
import { assertRestaurantArtifact, isSafeHref } from "../../../../contracts/artifact";
...
export async function POST(request: Request) {
  ...
  if (!UUID.test(nonce) || Math.abs(Date.now() - Date.parse(timestamp)) > 60_000) {
  ...
  try {
    ...
    const object = await fetch(
      `${config.artifactReadBaseUrl}/${encodeURIComponent(config.artifactBucket)}/${key}`,
      { headers: { authorization: `Bearer ${config.artifactReadToken}` }, cache: "no-store" },
    );
    ...
    const data = {
      ...
      assets_ok: artifact.media.every((item) => /^[0-9a-f]{64}$/.test(item.integrity)),
