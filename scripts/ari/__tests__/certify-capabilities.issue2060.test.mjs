...
test("#2060 plan covers the current exact 140-row ledger with one cache/readback owner", () => {
  const { ledger, owners } = loadCertificationInputs();
  const plan = buildCertificationPlan(ledger, owners);
  assert.equal(plan.capability_count, 140);
  assert.equal(new Set(plan.rows.map((row) => row.capability_id)).size, 140);
  assert.equal(
    plan.rows.every((row) =>
