...
test("#2060 plan covers the current exact 138-row ledger with one cache/readback owner", () => {
  const { ledger, owners } = loadCertificationInputs();
  const plan = buildCertificationPlan(ledger, owners);
  assert.equal(plan.capability_count, 138);
  assert.equal(new Set(plan.rows.map((row) => row.capability_id)).size, 138);
  assert.equal(
    plan.rows.every((row) =>
