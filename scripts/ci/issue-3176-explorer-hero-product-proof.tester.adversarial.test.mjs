...
async function runtimeContract() {
  ...
  let page
  try {
    await waitFor(async () => (await fetch(`http://127.0.0.1:${appPort}/for-explorers`)).status === 200, 'Next server did not start')
    let target
    await waitFor(async () => {
    ...
    await page.send('Page.navigate', { url: `http://127.0.0.1:${appPort}/for-explorers` })
