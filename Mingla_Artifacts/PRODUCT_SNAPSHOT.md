...
## Operational Alert (2026-04-17, post ORCH-0481 tester cycle 1 FAIL)

**Admin Place Pool page IS NOT YET UNBLOCKED. Cycle 2 rework required.**

- **Severity:** S1 operational — blocks internal place-curation work; not launch-gating (user-facing app unaffected).
- **Fix status:** Cycle 1 rework applied to Mingla-dev (commit 6f281d6b) — functionally improved but tester cycle 1 retest **FAIL** per `reports/QA_ORCH-0481_ADMIN_MV_LAYER_REPORT_RETEST_1.md`.
- **Partial wins confirmed:** `admin_place_country_overview` 87ms warm (Index-Only Scan — 475× faster than ORCH-0480's 54s attempt); `admin_place_pool_overview` global ~400ms (Index-Only Scans); `admin_place_category_breakdown` MV architecture proven in cycle 0 (107ms fully-cached).
- **Blocking P0 (new):** pg_cron refresh has **failed on every single run** (6/6 runs since 2026-04-17 18:45 — all hitting Supabase's 2-min statement timeout during `REFRESH MATERIALIZED VIEW CONCURRENTLY`). The MV has not been automatically refreshed since migration apply. Any write to place_pool after that moment is invisible to admin stats. Constitutional #3 violation (silent failure).
- **Non-blocking discoveries:** P1-1 cache-sensitivity on Mingla-dev (2-5s on cold cache — registered as ORCH-0488 for prod-side measurement before over-engineering). P1-2 category_breakdown regression (107ms→4.2s; fixable via ANALYZE in refresh path).
- **Cycle 2 in flight:** surgical ~15-line fix — wrap `REFRESH` in a plpgsql function with `SET LOCAL statement_timeout = '15min'` + `ANALYZE admin_place_pool_mv`. Re-schedule cron to call that function. Prompt: `prompts/IMPL_ORCH-0481_REWORK_V3.md`. Cycle count 2 (not stuck-in-loop yet).
- **Data-integrity:** ORCH-0484 (776 empty-array rows) — unchanged, still open.
- **Production deploy:** BLOCKED until cycle 2 PASS.
