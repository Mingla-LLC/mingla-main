...
## Operational Alert (2026-04-17, updated post-ORCH-0481 cycle 0 FAIL)

**Admin Place Pool page NOT yet unblocked. Cycle 0 FAIL, rework cycle 1 dispatched.**

- **Severity:** S1 operational — blocks internal place-curation work; not launch-gating (user-facing app unaffected).
- **Fix status:** ORCH-0480 → FAIL-superseded. ORCH-0481 migration applied to Mingla-dev (commit 352fc4c7) but **tester verdict: FAIL cycle 0** per `reports/QA_ORCH-0481_ADMIN_MV_LAYER_REPORT.md` (2 P0, 1 P1, 2 P2, 3 P4).
- **Great news:** MV architecture PROVEN — `admin_place_category_breakdown` runs in **107ms** (84× faster than ORCH-0480's 9s). The other 19 RPCs are the ones needing rework.
- **Bad news (P0-1):** `admin_place_country_overview` = **53,875ms** — actually 6× WORSE than ORCH-0480. Root cause: rewrite uses `COUNT(mv.*) FILTER (...)` which forces PostgreSQL to materialize every 624-byte MV row to check for non-null — creating a 318MB external disk sort. Same pattern suspected in `admin_place_city_overview`.
- **Bad news (P0-2):** `admin_place_pool_overview` global = 4,932ms (under 8s PostgREST timeout so the page loads, but 25× over the <200ms spec target). Mingla-dev's I/O can't seq-scan the 27-col wide MV fast.
- **Surgical fix in flight:** rework prompt `prompts/IMPL_ORCH-0481_REWORK_V2.md` dispatched. P0-1 is a mechanical `COUNT(mv.*)` → `COUNT(mv.id)` swap (~10 lines) expected to drop country_overview to <500ms. P0-2 requires a design decision (multi-subquery split vs covering index) — implementor's call.
- **Deploy state:** Mingla-dev has the failing ORCH-0481 migration. **NOT applied to production.** Production still on pre-ORCH-0480 state (admin page presumably 500'd there too, but untested — no deploy was made to prod for either ORCH-0480 or ORCH-0481).
- **Data-integrity finding (tightened):** ORCH-0484 — 776 active+approved places have `ai_categories = '{}'` (empty array, ZERO NULL). Tester-verified. Currently surface in MV as `primary_category='uncategorized'` and are correctly excluded from category_breakdown stats.
