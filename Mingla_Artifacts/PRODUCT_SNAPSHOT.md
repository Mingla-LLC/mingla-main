...
## Operational Alert (2026-04-17, updated post-ORCH-0481 rework cycle 1)

**Admin Place Pool page unblock is one `supabase db push` + tester PASS away.**

- **Severity:** S1 operational — blocks internal place-curation work; not launch-gating (user-facing app unaffected).
- **Fix status:** Cycle 1 rework delivered 2026-04-17 — migration `supabase/migrations/20260418000002_orch0481_rework_count_fix.sql` (298 lines, 3 function rewrites only). Implementor report: `reports/IMPLEMENTATION_ORCH-0481_REWORK_V2_REPORT.md`. Self-EXPLAIN verified on Mingla-dev: **country_overview 53,875ms → 113ms (475× win, PASS)**, pool_overview global ~500ms (PASS), city_overview 5,118ms → 3,061ms (partial — under 8s timeout, over <500ms). All three now render within PostgREST's 8s limit. Structural review APPROVED.
- **Next steps:** (1) user runs `supabase db push` — applies the rework migration on top of the cycle 0 MV layer. (2) user dispatches tester cycle 1 (reuse `prompts/TESTER_ORCH-0481_ADMIN_MV_LAYER.md`). (3) on PASS → apply to production via `supabase db push` → admin dashboard unblocked on prod.
- **Scope discipline:** MV + 5 indexes + pg_cron + 17 other admin RPCs untouched across both cycles.
- **Partial residual (ORCH-0487):** `admin_place_city_overview` 3s vs <500ms target. Covering-index follow-up scoped; not blocking this close.
- **Data-integrity finding:** ORCH-0484 (776 empty-array `ai_categories` rows) — remains open, unchanged by cycle 1 rework.
