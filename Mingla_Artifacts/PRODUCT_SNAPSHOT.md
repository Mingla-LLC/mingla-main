...
## Operational Alert (2026-04-17, post ORCH-0481 cycle 2 implementor return)

**Admin dashboard unblock is one `supabase db push` + 11-min wait + tester PASS away.**

- **Severity:** S1 operational — blocks internal place-curation work; not launch-gating (user-facing app unaffected).
- **Cycle 2 fix delivered:** `supabase/migrations/20260418000003_orch0481_cron_fix.sql` (128 lines). Creates `cron_refresh_admin_place_pool_mv()` plpgsql fn with `SET statement_timeout TO '15min'` + `REFRESH MATERIALIZED VIEW CONCURRENTLY` + `ANALYZE`. Re-schedules cron to call the fn. One-shot call at migration end validates fn body at deploy time + catches up the stale MV. Report: `reports/IMPLEMENTATION_ORCH-0481_REWORK_V3_REPORT.md`. Structural review APPROVED.
- **What cycle 2 fixes:** P0-1 (cron refresh 8/8 failures at 2-min timeout) + P1-2 (category_breakdown regression via ANALYZE keeping planner stats fresh). Both in one migration.
- **What's still untouched (intentional):** MV definition, 5 MV indexes, 20 rewritten admin_* RPCs from cycles 0/1, user-facing `admin_refresh_place_pool_mv()`. Rollback is a single DROP FUNCTION + restore prior cron.
- **Deploy plan:** (1) `supabase db push` — if the migration's embedded one-shot succeeds, the fn body is proven live. (2) Wait ~11 min for next scheduled cron tick. (3) Confirm `cron.job_run_details.status='succeeded'`. (4) Dispatch tester cycle 2 retest. (5) On PASS → apply to production.
- **Cycle history:** cycle 0 FAIL → cycle 1 rework/FAIL → cycle 2 rework/pending. Cycle count 2 (stuck-in-loop threshold 3+).
- **Deferred:** ORCH-0487 (city_overview <500ms covering-index), ORCH-0488 (cache-sensitivity — needs prod measurement), ORCH-0484 (776 empty-array rows).
