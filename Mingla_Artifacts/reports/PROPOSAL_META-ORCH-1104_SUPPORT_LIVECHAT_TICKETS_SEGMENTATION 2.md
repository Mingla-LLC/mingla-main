...
## 0. SCOPE CORRECTION (2026-06-08, operator) — NO consumer-app support

Seth corrected a scope over-reach: there is **NO "Help & Support" in the consumer (Explorer)
app**. Support is **business-side only**. The support REQUESTER is a **business user filing
from the business-app account page**; the support HANDLERS are the admin (PC) + support-tagged
business users (phone). The Explorer/consumer app gets nothing. "Explorer users" appears ONLY
as an admin **segmentation label**, not as a support audience.

Consequences: old **Phase A (consumer app) is DELETED**. The requester surface is the business
app (was Phase D, now the primary requester phase). Lane C's consumer-app mount-point findings
are out of scope; the business-app chat UI is what we reuse. The §1 list below is superseded by
this correction and will be re-laid once scope is confirmed in discussion.

---

## 1. Shared understanding (what Seth asked for) — SUPERSEDED BY §0 CORRECTION
