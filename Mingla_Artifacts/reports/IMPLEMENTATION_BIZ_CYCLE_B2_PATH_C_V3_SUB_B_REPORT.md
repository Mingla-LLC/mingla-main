...
3. **Publishable key not yet wired in `mingla-business`.** Operator has the `[REDACTED_STRIPE_PUBLISHABLE_KEY]` value in `stripe-values.md`. It needs to land in `mingla-business/.env.local` or expo extra config for Sub-dispatch C Phase 1. Already noted in the handoff brief but worth re-flagging.
