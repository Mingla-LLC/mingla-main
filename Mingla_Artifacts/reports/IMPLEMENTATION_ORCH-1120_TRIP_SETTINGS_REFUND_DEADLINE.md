...
> **RECOMPOSE (2026-06-12, per COMMS-0029): migration body now composed off the live-prod (1119-preserving) base.** A cross-session collision was proven: production's live `biz_update_live_trip` already carries ORCH-1119's per-day-media §5b logic (applied straight to prod ahead of merge; NOT yet on origin/main), but this migration originally re-emitted the function from the pre-1119 (1075) body — applying it would have SILENTLY CLOBBERED 1119's day-media feature on prod. The migration `20260929000000_orch_1120_trip_settings_refund_deadline.sql` was recomposed so the function body = LIVE-PROD-BODY (all 3 `1119` markers + the `INSERT … media` / `COALESCE(d->'media','[]'::jsonb)` / `media = EXCLUDED.media` §5b upsert preserved verbatim from `pg_get_functiondef`) + the ORCH-1120 §4g gate / §5f apply / §6 severity / 3 emitted reasons grafted on. Verified on an ephemeral Postgres 15: the composed `CREATE OR REPLACE` applies cleanly (plpgsql defers the 1119 column refs), in-DB introspection confirms `media = EXCLUDED.media` + `ORCH-1119` + `4g. ORCH-1120` all present in the applied body, the §4g/§5f/M-11..M-16 regression test passes, and fails-on-revert holds (delete §4g → M-10 RAISEs; restore → PASS). The day-media EXECUTION path can't be exercised locally (1119's `trip_days.media` schema is absent from git) — that's expected; the 1119 bytes are preserved verbatim from prod by construction. **HOLD-MERGE: do NOT merge this PR until ORCH-1119's migrations merge to origin/main** (else a git-fresh CI build re-emits the function referencing `trip_days.media` not yet in git). Prod itself is safe to apply now (prod already has the 1119 schema).
