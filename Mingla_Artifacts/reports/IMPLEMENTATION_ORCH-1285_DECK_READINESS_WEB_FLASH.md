# IMPLEMENTATION — ORCH-1285 · Venue deck-readiness "Recommend me" step flashes/closes on web create (venue stuck at `processing`)

- **Phase:** IMPLEMENT (client-nav-only fix; web-facing)
- **Date:** 2026-07-03
- **Worktree:** `~/Desktop/mingla-orchs/orch-1285-[deck-readiness-web-flash]/` on branch `orch-1285-deck-readiness-web-flash` (rebased on `origin/main` @ `9dc99ea46`).
- **Binding contract:** `Mingla_Artifacts/reports/INVESTIGATION_ORCH-1285_DECK_READINESS_WEB_FLASH.md` §G / §F-9.
...
The binding artifact is an investigation (§F-9 recommended fix), not a numbered SPEC; the acceptance criteria are derived from the dispatch. Commit hash `c6e94b2c2` (recorded below at commit time; = the ORCH-1285 implement commit).

| SC | Criterion | Status | Evidence |
|----|-----------|--------|----------|
| SC-1 | Create tier-1 success navigates via `router.replace` to the durable `/venue/deck-readiness` route (never the ephemeral inline mount) | ✓ | `VenueCreatorWizard.tsx` handleSubmit create branch; jest `venueCreateDurableDeckReadiness.orch1285` test 1 |
...
| `.github/scripts/strict-grep/i-proposed-1285-create-lands-on-durable-deck-readiness.mjs` | +185 (new) | Gate for `I-PROPOSED-1285-CREATE-LANDS-ON-DURABLE-DECK-READINESS` (`--self-test` 5/5) |
| `.github/workflows/strict-grep-mingla-business.yml` | +13 | New job `orch-1285-create-lands-on-durable-deck-readiness` (self-test + run) |
| `mingla-business/src/components/venue/__tests__/venueCreateDurableDeckReadiness.orch1285.test.ts` | +185 (new) | Happy-path AST regression test (4 assertions) |
...
1. **⚠️ ORCH-ID COLLISION.** "ORCH-1285" is ALSO the merged admin Identity console (`cf0cff156`, `i-1285-identity-admin-read.mjs`, `I-PROPOSED-1285-IDENTITY-ADMIN-READ`, registry §"DRAFT — ORCH-1285 (admin Identity console)"). This deck-readiness ORCH is a SECOND, later dispatch on the same number — an INTAKE ID-scan miss. I kept the dispatched ID (investigation is committed as ORCH-1285) but used a DISTINCT gate filename/job/invariant suffix (`…-CREATE-LANDS-ON-DURABLE-DECK-READINESS`) so nothing overlaps. **Recommend the orchestrator renumber one of the two at CLOSE** and reconcile the registry/World Map.
