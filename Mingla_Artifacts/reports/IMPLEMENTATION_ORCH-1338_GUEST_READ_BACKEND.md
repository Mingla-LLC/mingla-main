...
- **SC-5 nuance (per-spec, flagged for the tester):** an anon PostgREST call to `peer_list_event_guests` is rejected at the GRANT layer (permission denied) BEFORE the in-function `authentication_required` guard runs — the guard is defense-in-depth exactly as the SPEC layers it (§4.1.2 grants + guard 1). SC-5's observable ("zero rows ever cross the wire unauthenticated") holds either way; the tester should accept either the Postgres permission error or the `authentication_required` token for the anon case.
  - **CORRECTION (post-TEST, tester P2-1 — the grant-layer claim above was FALSE on prod):** live probes (`TEST_META-ORCH-1337_SOCIAL_PROOF_GUEST_LIST.md` §5 P2-1) proved `has_function_privilege('anon', 'public.peer_list_event_guests(uuid,integer,integer)', 'EXECUTE') = true` (same for `biz_set_event_guest_privacy`) — Supabase's `ALTER DEFAULT PRIVILEGES` grants per-ROLE EXECUTE to `anon` on every function CREATE, and this migration's `REVOKE ALL ... FROM PUBLIC` strips only the PUBLIC path, not the direct role grant. The anon rejection observed on prod came from the in-function `authentication_required` guard ALONE (which held — zero rows/writes crossed the wire; no exposure, hence P2 not P0). Fixed by follow-up migration `20261227000000_orch_1338_p2_revoke_anon_execute.sql` (explicit `REVOKE EXECUTE ... FROM PUBLIC, anon` + explicit re-`GRANT ... TO authenticated` on both authed-only RPCs) with static suite `supabase/migrations/__tests__/orch_1338_p2_revoke_anon_execute.test.ts`. Post-apply retest: both `has_function_privilege('anon', ..., 'EXECUTE')` → `false`.
