...
**Rev 2 (review corrections, same date):** (1) D-3 re-derived from gate 0769's actual full output + a consumer-app coverage-gap finding added (D-3b) — the gate scans `mingla-business/` ONLY; its one violation was correctly anchored, but live consumer-app GBP fallbacks exist that NO gate covers. (2) F-1 rows 9/10 (`orch-0770`/`orch-0776a`) **flipped RETIRE → WIRE-AFTER-RECONCILE**: the guarded path is LIVE (dual-protocol service called from live UI; META-1270 flipped the provider Cloudinary→Bunny with the Cloudinary branch retained dark) — the gates pin the pre-Bunny CLIENT implementation and must be rewritten, not deleted. New discovery D-7 (Cloudinary branch residue). Rev-1's RETIRE call was the exact mistake class this ORCH exists to prevent; credit: REVIEW.
...
Verdict (proven): 13 exit 0 today (immediately wireable), 7 exit 1 on live code (wire-after-reconcile; 4 of the reds point at possible real product regressions — registered as discoveries — and 2 of them, `orch-0770`/`orch-0776a`, pin the pre-Bunny client upload implementation that META-1270 replaced while the guarded surface stays live), 1 exits 2 by design (the orch-1369 adversarial harness: its blocking half passes, its exit-2 is a documented-coverage-gap signal). **0 are SUPERSEDED and 0 are RETIRED** — the currency successors (`orch-1146`, `meta-orch-1236`) were checked and do NOT cover `orch-0769`'s target file, and the 0770/0776a guarded path was traced LIVE (Rev 2). Full table §5 F-1.
...
| Runtime (all 21 executed; RD-1 executed; 6 self-tests executed) | 13 green / 7 red / 1 exit-2; RD-1 11/12; 6/6 self-tests pass | **Runtime vs Docs:** "wire the 21" is not a flip — 7 need reconciliation (2 of them full expectation rewrites vs the TUS pipeline, Rev 2) |
...
| 9 | `orch-0770-event-cover-video-processing` | **exit 1** — expects `EVENT_COVER_FINAL_MAX_BYTES = 25 * 1024 * 1024` etc., constants gone from the client | **Yes (Rev 2)** — `eventCoverVideoProcessingService.ts` is LIVE: imported by `useEventCoverVideoUpload.ts` + `CoverPicker.tsx`, which renders from `app/experience/[id]/edit.tsx`, `app/venue/deck-readiness.tsx`, `ExperienceCoverStep.tsx`; service is dual-protocol (`EventCoverVideoUploadProtocol = "cloudinary" \| "tus"`, line 46); META-1270 flipped `EVENT_COVER_VIDEO_PROVIDER=bunny` (client leg device-verified at its CLOSE); the 25MB budget concept survives server-side (service line 29 + 974) | **WIRE-AFTER-RECONCILE** — rewrite expectations against the live TUS/Bunny pipeline (`eventCoverVideoTusPatch.ts`, upload-intent/webhook family). Deleting it would remove a guard from a live upload surface → Discovery D-4 (rewrite scope) + D-7 | caller grep + service read + META-1270 memory |
| 10 | `orch-0776a-video-upload-progress-honesty` | **exit 1** — expects the Expo `createUploadTask` path the TUS refactor replaced | **Yes (Rev 2)** — same live service/callers as #9; the progress-honesty concern (real byte progress in the upload UI) is alive under TUS (`EventCoverVideoUploadProgress` still exported, lines 93/367/484/556) | **WIRE-AFTER-RECONCILE** — same rewrite target as #9 | caller grep + service read |
...
**Tally (Rev 2): WIRE 13 · WIRE-AFTER-RECONCILE 7 · RETIRE 0 · WIRE-special 1 (+ RD-1 WIRE-after-fix) · SUPERSEDED 0.**

**Rev-2 evidence for the 9/10 flip (review-driven trace):** `event-cover-video-webhook/index.ts` imports BOTH `verifyCloudinaryNotificationSignature` and the Bunny branch (`verifyBunnyWebhookSignature`, header comment "META-ORCH-1270 — Bunny library-level webhook branch (Cloudinary path unchanged)"); 11 `event-cover-video-*` functions exist on disk incl. in `supabase/config.toml`. META-1270 memory: Cloudinary retired, secrets REMOVED, provider flipped to Bunny — so the Cloudinary protocol branch in the client service (lines 46/62/387/538/579-592/1084/1158) and the webhook's Cloudinary arm are **operationally dark but structurally reachable** (a flag-flip away, and it would fail on missing secrets). Supabase edge-function invocation logs were NOT queried (not needed for the disposition — surface-liveness, not invocation frequency, decides WIRE-vs-RETIRE; the client leg was device-verified at META-1270 CLOSE); any claim about recent webhook traffic is therefore uncapped and unmade.
...
CI-only ORCH; no client surface changes in scope. In-scope: `.github/scripts/strict-grep/**` (+ MANIFEST + parity gate + run-batch), `tests-append-only` interplay, `mingla-business/package.json` test-script rows (unchanged in Phase 1 — no retirements after Rev 2). Out-of-scope surfaces: all 7 product surfaces (consumer iOS/Android, buyer web, business iOS/Android, admin web, business web preview) — no runtime behavior changes. HOWEVER the 7 red gates point INTO product surfaces; those are handed to the orchestrator as discoveries, not silently absorbed (below).
...
- Conflict flagged (not resolved here): Rev 2 removes all Phase-1 deletions (no RETIRE), so no GATE-REMOVAL token is needed this phase; the mechanism note stands for any FUTURE retirement (deletion of a guard = append-only token + Seth-notify item per dispatch).
...
- **D-3 (Rev 2, re-derived from gate 0769's actual full output + independent grep):**
  - **D-3a — the gate's real violation set (Phase-1b reconciliation target), exactly ONE site:** `mingla-business/app/rsvp/[id]/preview.tsx:112: currency: "GBP",` — verbatim gate output, re-verified by file read (BUSINESS app, not app-mobile). The gate's path reporting is correctly anchored — no reporting bug.
  - **D-3b — consumer-app GBP fallbacks NO gate covers (NEW coverage gap):** gate 0769 walks ONLY `mingla-business/app` + `mingla-business/src` (script lines 12–13) despite its "app-wide" name, and `orch-1146`'s TARGET_FILES are all `supabase/functions/*`. Live, unguarded consumer sites (each read-verified): `app-mobile/src/components/ConnectionsPage.tsx:243` (`currency: event.currency ?? 'GBP'`), `app-mobile/src/components/expandedCard/TicketCartSheet.tsx:141` and `:156` (`currency: currency || "GBP"`), `app-mobile/src/hooks/useTicketCart.ts:81` (`DEFAULT_CURRENCY = "GBP"`). Display-currency fabrication risk per the ORCH-1034/1236 program (display-not-data).
  - **How the citation confusion arose:** the gate's output was accurate; Rev-1's D-3 one-liner carried the gate's misleading "app-wide" name without stating its business-only scan scope, inviting the site to be sought in `app-mobile/` where it does not exist. Report-clarity defect + a genuine gate-name-vs-scope mismatch (the latter is the D-3b finding).
- **D-4 (Rev 2):** the LIVE TUS/Bunny event-cover upload pipeline is guarded only by 0770/0776a's stale pre-Bunny expectations — the reconcile rewrite (WIRE-AFTER-RECONCILE, F-1 #9/#10) must target `eventCoverVideoTusPatch.ts` + the `event-cover-video-*` function family; until then the surface is effectively unguarded.
...
- **D-7 (NEW, Rev 2):** post-META-1270 Cloudinary residue — the client service's `"cloudinary"` protocol branch, the webhook's Cloudinary arm (`verifyCloudinaryNotificationSignature`), and related shared helpers remain in tree and in `supabase/config.toml` with Cloudinary secrets REMOVED: operationally dead, structurally reachable, and guaranteed to fail if ever re-entered. Remove-or-formalize as its own ORCH; this residue is also what made gates 0770/0776a's state misleading.
- (Known, already boarded: #957 NUL-byte grep-invisibility; #958 bracket-path + orch-0964 `.next/` walker.)

## 9. Repro evidence
Not a UI reproducer; runtime evidence = the 21 gate executions (13/7/1 split), RD-1 `node --test` (11/12, T-11 assertion error quoted), 6 wired self-test executions (6/6 exit 0), 3 prototype self-tests (all directions proven). Rev-2 probes: gate 0769 re-run with FULL output (exactly 1 violation, quoted in D-3a); consumer GBP grep + per-line reads (D-3b); 0770/0776a liveness trace (caller grep, service dual-protocol read, webhook read, META-1270 memory) — Supabase function logs not queried, claim scoped accordingly (F-1 Rev-2 note). No simulator relevance.
...
**SPEC (this ORCH, phased):** Phase 1 = close H1–H3 in the parity gate + MANIFEST schema, and execute the F-1 disposition (wire 13 + RD-1 + 1369-Part-1; hold the 7 reconciles — incl. the 0770/0776a TUS-pipeline rewrite — with their discoveries routed to the orchestrator first; a gate must not be wired red, and no guard is deleted). Phase 2 = self-test backfill for the highest-risk class (single/multi-file token gates guarding money/release/auth surfaces), with the fixture convention fixed as in-source fixtures (never committed fixture files — #957 NUL hazard + absence-gate cross-trigger hazard). Phase 3+ = remaining classes by measured rates with a per-merge ratchet. Explicit non-goal: proving gates catch defects OTHER than their original class.
