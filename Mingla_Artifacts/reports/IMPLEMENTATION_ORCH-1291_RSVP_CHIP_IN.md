...
| SC-2-iOS/-Android/-Web | Per-surface visibility | Suspected | Shared body → automatic parity; needs device runtime. |
...
| SC-7 | Anon web guest chips in end-to-end, no login | Suspected | Create fn `verify_jwt=false`, web hosted-Checkout; body returns `{kind:'redirecting'}` → `contributionState='paid'`. Web mount config-plumbing gap (§10.A). Live-fire = TEST. |
...
Both test files appear in `git diff origin/main...HEAD --name-only` (ship with the fix; append-only — no existing test modified/deleted).
...
|---|---------|----------|--------|-------|
| 1 | Consumer iOS | YES | AUTOMATIC (shared body) + thin `onChipIn` | Stripe PaymentSheet / Paystack browser wired; gated by config-plumbing (§10.A). |
| 2 | Consumer Android | YES | AUTOMATIC (opaque glass fallback) | Same handler. |
| 3 | Buyer/anon Web | YES | AUTOMATIC (shared body) | Web hosted-Checkout redirect; needs the buyer-web caller to pass `onChipIn` + config (§10.A). |
...
**A. Config-plumbing gap (consumer + buyer-web) — the panel stays DARK until closed.** The chip-in panel renders only when `config.rsvp_contribution_enabled` reaches the shared body. The publish RPC persists the 3 `events` columns AND (via the theme passthrough) `theme.business_event.rsvpContribution*`, but the CONSUMER read (`app-mobile/src/services/rsvpDeckService.ts::fetchRsvpMomentum` / `business_public_events_view`) and the buyer-web read do NOT yet surface them, and `PublicEventProps` has no contribution field. These reads are OUTSIDE the spec allowlist. The consumer `onChipIn` handler + config mapping are wired defensively (they light up the moment the read exposes the columns). **Conductor: dispatch a small follow-up to surface `rsvp_contribution_enabled / _suggested_cents / _min_cents` + settlement currency through `fetchRsvpMomentum` (+ the buyer-web RSVP read) and wire the buyer-web `FoundationRsvpPreview` caller's `onChipIn` (→ `submitRsvpContribution` surface `web`).**
...
- **D-1 (config-plumbing):** see §10.A — the shared body needs the RSVP reads to surface the 3 contribution columns; a self-contained follow-up ORCH.
