...
**STILL PENDING (could NOT capture — harness began dropping content-bearing Bash stdout + Read of /tmp files mid-verification):** the project `eslint` run + a `vite build` for full import/type resolution. These are the last two `/goal` clause-3 gates. Label: **`implemented, partially verified`** for the admin page (parse-clean + built from verified signatures + persist lib/RPCs already proven; eslint + vite-build not yet captured). The continuation/tester must run, from the anchor admin: `node_modules/.bin/eslint src/pages/PricingPage.jsx src/App.jsx src/components/layout/Sidebar.jsx` + `npm run build`. Any fix needed would be cosmetic (a prop name), not logic.
