...
The target sim **iPhone 17 Pro Max `2C3312D9-EE52-4EBD-9704-15811D49A2EC` is booted** (confirmed via `simctl list devices booted`). I began the live-fire setup (app-container confirm → Metro on 8089 → Maestro walk of BrandCreationFlow → VenueCreatorWizard → menu parse → confirm) but the **Bash tool channel entered a sustained outage mid-session** (≥8 consecutive no-result calls), blocking Metro start + Maestro + simctl launch. Per Prime Directive 7 + `[[sim-boot-blocker-must-resolve-not-note]]`, a tooling-channel outage is a genuine external blocker (not an honor-system skip): I escalate it here rather than silently downgrading. **The funnel mechanism is nonetheless `proven` at the runtime-DATA layer** (live prod rows + the exact lazy-expiry code path) — which is a stronger evidence class than the source-only inference Prime Directive 7 guards against. The remaining sim leg (capturing the operator-facing 410 toast on device) caps the *user-perceived-UX* sub-claim at `probable`. **Recommended before SPEC sign-off:** re-run the Maestro walk once the Bash channel recovers; screenshots would land under `Mingla_Artifacts/reports/screenshots/sub_e_funnel/`.
