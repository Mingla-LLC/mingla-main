...
| Open path | `app-mobile/src/components/ConnectionsPage.tsx` | Detects `conversation.type === 'group'`, passes `conversationType`, `sessionId`, `participantCount`, and participant profile metadata into `MessageInterface`, and skips DM-only block/friendship/profile probes for group chats. |
| Header UI | `app-mobile/src/components/MessageInterface.tsx` | Renders group chat header as session name + stacked participant avatars + `N people in chat`; hides the one-on-one friend action menu for group chats. |
