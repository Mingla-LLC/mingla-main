...
| SC-4 | Pull-to-refresh on home invalidates `brandKeys.all` + `eventOrdersKeys.all` | Read [home.tsx:152-167](mingla-business/app/(tabs)/home.tsx#L152-L167) — `handleRefresh` runs `Promise.all([invalidate(brandKeys.all), invalidate(eventOrdersKeys.all)])`. `RefreshControl` wired on ScrollView line 319-322. | ✅ PASS |
