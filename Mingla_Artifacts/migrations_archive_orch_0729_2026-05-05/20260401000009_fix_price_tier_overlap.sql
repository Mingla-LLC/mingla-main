-- Fix P0: Migration 008 accidentally reverted price_tiers array overlap to scalar equality.
-- This migration restores cp.price_tiers && p_price_tiers while preserving 008's v_any_tier bypass.
...
BEGIN
  ...
  WITH
  excluded AS (SELECT unnest(p_exclude_card_ids) AS card_id),
  seen AS (
    SELECT uci.card_pool_id AS card_id
    FROM public.user_card_impressions uci
    WHERE uci.user_id = p_user_id AND uci.created_at >= p_pref_updated_at
  ),
  filtered AS (
    SELECT cp.*
    ...
    WHERE cp.is_active = true
      ...
      AND (
        v_any_tier
        OR (v_use_tiers AND NOT v_any_tier AND cp.price_tiers && p_price_tiers)
  ...
  deduped AS (
    ...
    FROM filtered f ORDER BY COALESCE(f.google_place_id, f.id::TEXT), f.popularity_score DESC
  )
  SELECT COUNT(*) INTO v_total_unseen FROM deduped;

  IF v_total_unseen > 0 THEN
    -- ── Primary path: unseen cards ──────────────────────────────────────────
    RETURN QUERY
    WITH
    excluded AS (SELECT unnest(p_exclude_card_ids) AS card_id),
    seen AS (
    ...
    enriched AS (
      SELECT r.*, pp.stored_photo_urls, pp.photos,
        COALESCE(NULLIF(r.website, ''), NULLIF(pp.website, '')) AS resolved_website
      FROM ranked r LEFT JOIN public.place_pool pp ON pp.id = r.place_pool_id
  ...
  ELSE
    ...
    ranked AS (
      SELECT w.*,
        ROW_NUMBER() OVER (PARTITION BY w.category ORDER BY w.last_seen_at ASC, w.popularity_score DESC) AS cat_rank,
        ROW_NUMBER() OVER (PARTITION BY w.category ORDER BY w.last_seen_at ASC, w.popularity_score DESC) AS cat_position
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- Fix P2-3: admin_edit_place must distinguish NULL (no change) from empty array (clear)
-- ═══════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Determine effective tiers:
  -- NULL p_price_tiers = no change
  -- empty array p_price_tiers = explicit clear
  -- non-empty array = set these tiers
  ...
  ELSE
    v_effective_tiers := NULL;  -- no change
  END IF;
  ...
  SET
    ...
    price_tier = CASE
      ...
      WHEN v_effective_tiers IS NOT NULL THEN NULL  -- explicit clear
      ELSE price_tier  -- no change
    END,
  ...
  IF v_effective_tiers IS NOT NULL THEN
    ...
    SET price_tiers = v_effective_tiers,
        price_tier = CASE WHEN array_length(v_effective_tiers, 1) > 0 THEN v_effective_tiers[1] ELSE NULL END,
        updated_at = now()
    WHERE place_pool_id = p_place_id AND card_type = 'single';
  END IF;
