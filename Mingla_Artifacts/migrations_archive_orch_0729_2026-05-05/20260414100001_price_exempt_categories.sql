-- ORCH-0421: Category-aware price tier filtering
--
-- Problem: query_pool_cards applies price tier filtering uniformly to all 13 categories.
-- For categories where price is structurally irrelevant (parks, picnic grounds, theaters,
-- museums, concert venues), this eliminates 73-100% of cards. Nature & Views lost 98.7%
-- of cards in Raleigh when user had comfy+bougie preferences.
--
-- Fix: Add v_price_exempt array. Cards whose PRIMARY category is in this list bypass
-- price tier filtering entirely. Also: NULL price_tier cards are now always included
-- (previously silently excluded via SQL NULL = ANY(...) semantics).
--
-- The price filter clause is updated in TWO places (count query + filtered CTE).
-- Both MUST be identical or total_unseen will diverge from actual results.
...
BEGIN
  ...
  filtered AS (
    ...
    WHERE cp.is_active = true
      ...
      AND (v_any_tier OR cp.category = ANY(v_price_exempt) OR cp.price_tier IS NULL OR (v_use_tiers AND cp.price_tier = ANY(p_price_tiers)) OR (NOT v_use_tiers AND cp.price_min <= p_budget_max))
      AND (p_experience_type IS NULL OR cp.experience_type = p_experience_type)
      AND cp.id != ALL(p_exclude_card_ids)
      AND (NOT v_has_place_exclusions OR cp.google_place_id IS NULL OR cp.google_place_id != ALL(p_exclude_place_ids))
