-- ORCH-0366: Optimize query_pool_cards to single-pass count+data
--
-- Problem: The function ran the entire complex filter/dedup CTE chain TWICE —
-- once for COUNT(*) and once for the actual data. With 7K+ cards, 56K+ places,
-- and 4 NOT EXISTS subqueries, this doubled execution time.
--
-- Fix: Execute the filter/dedup ONCE. Use COUNT(*) OVER() window function to
-- compute total_unseen alongside the data in a single pass. Both the primary
-- path (unseen cards) and fallback path (rotation) are preserved.
--
-- Expected improvement: ~50% reduction in query execution time.

-- Drop current signature
...
BEGIN
  ...
  -- ── Single-pass: count unseen cards and determine which path to take ─────
  -- This replaces the old two-step approach that ran the full CTE chain twice.
  ...
  -- ── Branch on whether unseen cards exist ─────────────────────────────────
