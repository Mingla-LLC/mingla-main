-- ============================================================
-- Deterministic Preferences → Deck Contract
-- Combines Fix 1 (remove double-pagination), Fix 4 (NULL price tier),
-- Fix 5 (category interleave after capping), and Fix 3 prerequisite
-- (add custom_lat/custom_lng columns to solo preferences table).
--
-- Changes:
--   0. Add custom_lat/custom_lng to preferences table (Fix 3 prerequisite)
--   1. Drop p_offset parameter — impression exclusion is sole pagination
--   2. Remove OFFSET p_offset from both primary and fallback paths
--   3. Remove OR cp.price_tier IS NULL from price filter in all 3 CTEs
--   4. Add cat_position window function for interleaved ordering
--   5. Change ORDER BY to cat_position ASC, popularity_score DESC (primary)
--      and cat_position ASC, last_seen_at ASC, popularity_score DESC (fallback)
--   6. Strip cat_position from JSONB output
-- ============================================================

-- 0. Add custom_lat/custom_lng to solo preferences table
-- (already exists on board_session_preferences for collab)
ALTER TABLE public.preferences
  ADD COLUMN IF NOT EXISTS custom_lat DOUBLE PRECISION,
  ADD COLUMN IF NOT EXISTS custom_lng DOUBLE PRECISION;

-- Drop old 14-param signature
...
BEGIN
  ...
  filtered AS (
    ...
    WHERE cp.is_active = true
      ...
      AND COALESCE(cp.ai_override, cp.ai_approved, true) = true
  ),
  deduped AS (
