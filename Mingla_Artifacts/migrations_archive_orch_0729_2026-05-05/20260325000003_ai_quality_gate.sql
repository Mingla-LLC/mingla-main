-- ═══════════════════════════════════════════════════════════════════════════════
-- AI Card Quality Gate — Phase 1
-- Adds AI validation columns to card_pool, editorial_summary to place_pool,
-- and AI filter to query_pool_cards serving function.
-- ═══════════════════════════════════════════════════════════════════════════════

-- ── Part A: AI columns on card_pool ─────────────────────────────────────────
...
ALTER TABLE public.card_pool
  ...
  ADD COLUMN IF NOT EXISTS original_categories TEXT[] DEFAULT '{}',
  ADD COLUMN IF NOT EXISTS ai_validated_at TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS ai_override BOOLEAN;

CREATE INDEX IF NOT EXISTS idx_card_pool_ai_approved
  ON public.card_pool (ai_approved) WHERE is_active = true;

-- ── Part B: editorial_summary on place_pool ─────────────────────────────────

ALTER TABLE public.place_pool
  ADD COLUMN IF NOT EXISTS editorial_summary TEXT;

-- Backfill from raw_google_data where available
UPDATE place_pool
SET editorial_summary = raw_google_data->'editorialSummary'->>'text'
WHERE editorial_summary IS NULL
  AND raw_google_data->'editorialSummary'->>'text' IS NOT NULL;

-- ── Part C: Update query_pool_cards with AI filter ──────────────────────────
-- AI filter line: AND COALESCE(cp.ai_override, cp.ai_approved, true) = true
-- Added to all three filtered CTEs (count, primary, fallback).
