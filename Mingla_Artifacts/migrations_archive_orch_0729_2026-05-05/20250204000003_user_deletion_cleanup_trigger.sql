...
-- ============================================
-- This migration creates a trigger to handle cleanup when a user profile is deleted.
-- It ensures that:
-- 1. Collaboration sessions are properly handled (admin reassignment, under-populated boards deleted)
-- 2. The user is removed from all session participants
-- 3. Messages are anonymized rather than deleted to preserve conversation history
-- ============================================

-- Function to handle collaboration session cleanup when a user is deleted
...
BEGIN
  ...
  -- This preserves conversation history while removing personal data
  UPDATE messages
  SET deleted_at = NOW()
...
-- Create trigger to run cleanup BEFORE profile is deleted
-- Using BEFORE DELETE ensures we can still access the user's data for cleanup
...
COMMENT ON FUNCTION public.handle_user_deletion_cleanup() IS 
  'Handles cleanup of user data when a profile is deleted. Reassigns admin roles, deletes under-populated boards, and anonymizes messages.';
