...
> Last updated: 2026-04-18 (ORCH-0493 Phase 0-C investigator complete — verdict ⚠️ PARTIAL)
> Total: 3 active | 126 completed

## Active Dispatches

| ID | Issue | Agent Role | Dispatched | Prompt File | Status |
|----|-------|-----------|------------|-------------|--------|
| AH-127 | ORCH-0485 | Investigator Phase 0-A (device trace) | — | `prompts/INV_ORCH-0485_DEVICE_TRACE.md` | **Prompt written, awaiting user dispatch** to mingla-forensics. Raises RC#1/#3 H→H+ via device timings, establishes latency baseline for spec's acceptance criteria. |
| AH-128 | ORCH-0492 | Investigator Phase 0-B (persistence baseline) | — | `prompts/INV_ORCH-0492_PERSISTENCE_BASELINE.md` | **Prompt written, awaiting user dispatch** to mingla-forensics. Establishes 7-fragment × 4-interruption baseline matrix before ORCH-0492 spec can be written. |
| AH-129 | ORCH-0489 | Implementor (admin-UI Refresh Stats button) | 2026-04-18 | prompts/IMPL_ORCH-0489_ADMIN_REFRESH_BUTTON.md | **Ready to dispatch.** User selected Alternative 1 post DEC-023 stuck-in-loop. Single-file scope in `mingla-admin/src/pages/PlacePoolManagementPage.jsx`, ~30-60 lines inline. Calls existing `admin_refresh_place_pool_mv()` RPC. Closes the last ORCH-0481 gap (auto-refresh silent failure → user-controlled visible refresh). Mingla-dev = prod per user clarification 2026-04-18; all ORCH-0480/0481 migrations already live in prod. |
