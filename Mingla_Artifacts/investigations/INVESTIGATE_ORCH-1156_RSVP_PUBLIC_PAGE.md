# INVESTIGATE — ORCH-1156 · Public RSVP Event Page (Direction C "Momentum")
...
**Date:** 2026-06-17.
**Approved design (binding):** Direction C "Momentum" — `Mingla_Artifacts/design/ORCH-1156/RSVP_DIRECTION_C_MOMENTUM.html` + `DESIGN_PHILOSOPHY_ORCH-1156_RSVP_PUBLIC_PAGE.md`. Seth chose C; the one applied tweak ("You're invited" kicker `color: inherit`, pulsing dot stays accent) is already in the mockup.
**Comms ledger:** read on entry. No OPEN BLOCK; no OPEN WARN/FYI addressed to forensics / ORCH-1156 / ALL that is actionable for a frontend-only RSVP redesign. COMMS-0002 (backend-allowlist strict-grep gate) is noted as a **conditional** factor — it only bites if a new edge function / migration is added, and this investigation proves **none is needed** (see F-3). No ledger ack required.
...
| # | File (absolute) | Why |
|---|---|---|
| 1 | `Mingla_Artifacts/design/ORCH-1156/DESIGN_PHILOSOPHY_…md` | binding design intent + real-data field inventory |
| 2 | `Mingla_Artifacts/design/ORCH-1156/RSVP_DIRECTION_C_MOMENTUM.html` | binding visual spec (phone + desktop, 5 states) |
...
- **Mechanism:** all C content is one view away from the anon caller; the page is data-complete at the source. The ONLY missing plumbing is in the JS mapper + the `PublicEventProps` type (F-8). **This means the buyer-web/business legs of ORCH-1156 are pure frontend work — no DB, no edge function, no Stripe.**
...
- **Mechanism:** the consumer detail is real and mounted; ORCH-1156 does not need to invent navigation — it needs to **replace the divergent dock with the shared C unit** (and decide how much of the momentum unit the consumer can show given F-6).
- **Severity:** CONFIRMED.
...
- **New DRAFT invariants proposed (SPEC owns the rule/enforcement; orchestrator flips ACTIVE on CLOSE):**
  - `I-PROPOSED-1156-RSVP-NO-CHECKOUT-AFFORDANCE`
  - `I-PROPOSED-1156-RSVP-DECISION-IS-HERO`
  - `I-PROPOSED-1156-RSVP-SOCIAL-PROOF-ANON-ONLY`
  - `I-PROPOSED-1156-RSVP-USES-BRAND-THEME-DIAL`
...
- **D-1 (existing drift):** the consumer RSVP dock lacks the **Maybe** option that the shared body has had since ORCH-1150 R2. ORCH-1156 will close this incidentally by unifying on the shared C unit, but flag it as a pre-existing parity defect.
...
- **Recommended next phase:** SPEC (written as the sibling file `SPEC_ORCH-1156_RSVP_PUBLIC_PAGE.md`) → then mingla-implementor → mingla-tester (per-surface live-fire) → orchestrator CLOSE.
