# INVESTIGATE — ORCH-1116 [Hub multi-select draft delete]

**Mode:** mingla-forensics INVESTIGATE
**Worktree:** `/Users/sethogieva/Desktop/mingla-orchs/ORCH-1116-[hub-multiselect-draft-delete]` (branch `ORCH-1116-hub-multiselect-draft-delete`, current with origin/main `0f9860b4a`)
**Date:** 2026-06-11
...
- The existing draft-discard RPC `public.business_discard_event_draft(p_event_id uuid)` is defined in **`supabase/migrations/20260515000006_orch_0763d_draft_discard_rpc.sql`**. **Critical:** the RPC has **NO `event_type` filter** — it discards *any* draft row (event, trip, or experience) so long as `status='draft'`. This is the single biggest enabler for ORCH-1116: a batch RPC can reuse this exact logic verbatim for all three kinds.
...
### Experiences — none
No service, hook, RPC, or RLS-mediated client UPDATE deletes an experience today. **Net-new for ORCH-1116.**
