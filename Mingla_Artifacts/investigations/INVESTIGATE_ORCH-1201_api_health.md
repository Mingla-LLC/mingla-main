# INVESTIGATE — ORCH-1201 Admin API-Health Hub + Email Alerts
...
**Authoritative inventory read:** `Mingla_Artifacts/ORCH-1201_API_INVENTORY.md` (incl. LOCKED DECISIONS block).
...
```sql
-- supabase/migrations/<ts>_orch_1201_api_health_probe_cron.sql
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname='pg_cron') THEN
    RAISE EXCEPTION 'ORCH-1201: pg_cron required'; END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname='pg_net') THEN
    RAISE NOTICE 'ORCH-1201: pg_net missing — http_post will fail at runtime'; END IF;
END$$;
DO $$ BEGIN
...
END$$;
SELECT cron.schedule('orch_1201_api_health_probe', '0 * * * *', $cron$
