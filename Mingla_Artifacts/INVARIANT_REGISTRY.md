...
### I-PROPOSED-TOPSHEET-WEB-VIEWPORT-ANCHOR (ACTIVE — ratified by ORCH-1136 CLOSE 2026-06-14)
**Rule:** On web (`Platform.OS==='web'`), `TopSheet`'s overlay root MUST anchor to the viewport (`position:'fixed'` + live window height) rather than bare `StyleSheet.absoluteFill`, so a scrolled host route (e.g. Hub) cannot offset the sheet above the viewport. Native (iOS/Android) keeps `absoluteFill` + `Dimensions.get('window')` UNCHANGED by construction. Applies to BOTH TopSheet consumers (BrandSwitcherSheet `fixed-70`, UniversalCreatorSheet `compact`).
**Enforcement:** strict-grep `.github/scripts/strict-grep/i-proposed-topsheet-web-viewport-anchor.mjs` (requires the web-gated `position:'fixed'`); CI job `orch-1136-biz-web-shell-bugs`. fails-on-revert @ `ee4c539cf`. Mechanism proven in Chromium: panel top −524px (old, scrolled) → 76px (fixed, every scroll) == Home.
