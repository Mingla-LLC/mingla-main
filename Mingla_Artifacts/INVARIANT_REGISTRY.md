...
- **Established:** DRAFT 2026-07-07 during ORCH-1321 implementation (flip DRAFT→ACTIVE at CLOSE).
...
### I-PROPOSED-1216-EXPLORER-ONLY-CTA (ACTIVE — flipped ORCH-1216 2026-06-22; NARROWED to beta/organiser-only ORCH-1319 2026-07-06)
**Rule (as narrowed by ORCH-1319):** The organiser `BetaAccessModal` mounts ONLY inside a `surface === 'organiser'` guard in `glass-nav.tsx` and must never cross onto the explorer surface, and `beta-access-modal.tsx` must never cross-import the (now-deleted) explorer get-the-app modal. **The `GetTheAppModal` clause is REMOVED: ORCH-1319 deleted that modal (the explorer CTA is now a device-aware direct store action / desktop QR, no modal).** The organiser business waitlist (`beta_access_leads`) is still live and separate — this invariant keeps it surface-scoped. **Enforcement:** strict-grep gate `.github/scripts/strict-grep/i-proposed-1216-explorer-only-cta.mjs` (`--self-test`, 4/4; the GetTheApp half stripped by ORCH-1319), wired as job `orch-1216-explorer-only-cta`. Fails-on-revert proven.
...
### I-PROPOSED-1219-FORM-NO-AUTOADVANCE-MULTISELECT (ACTIVE — flipped ORCH-1219 2026-06-22; AMENDED ORCH-1221 2026-06-22; NARROWED to organiser-only ORCH-1319 2026-07-06)
**Rule (as narrowed by ORCH-1319 — ORGANISER-ONLY):** The organiser `BetaAccessModal` brand-type first step must not auto-advance on chip tap (no `setTimeout(setStep(2))`) and is a MULTI-SELECT toggle group (`useState<string[]>`, `role="group"`, `aria-pressed`) gated on ≥1 selection — the user always presses Next. **The explorer clause is REMOVED: ORCH-1319 deleted the explorer `GetTheAppModal` (app is live on both stores), so there is no explorer interest step to protect.** The organiser business waitlist is still live (business is not yet on stores) — this invariant keeps guarding it. **Enforcement:** strict-grep gate `.github/scripts/strict-grep/i-proposed-1219-form-no-autoadvance-multiselect.mjs` (`--self-test`, 4/4 organiser cases; the explorer half stripped by ORCH-1319), wired as job `orch-1219-form-no-autoadvance-multiselect`. Fails-on-revert proven (revert the organiser form to single-select / re-introduce auto-advance → gate exit 1). Seth-directed 2026-06-22; narrowed 2026-07-06.
