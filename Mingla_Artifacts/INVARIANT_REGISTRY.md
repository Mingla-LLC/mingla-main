...
### I-PROPOSED-1219-FORM-NO-AUTOADVANCE-MULTISELECT (ACTIVE — flipped at ORCH-1219 CLOSE 2026-06-22)
**Rule:** Neither marketing lead-capture modal may auto-advance its first step on chip tap. The explorer `GetTheAppModal` interest step is MULTI-SELECT (`string[]`, toggle group, `aria-pressed`) gated on ≥1 selection; the organiser `BetaAccessModal` brand-type step stays single-select but is likewise Next-gated. The 220ms pointer `setTimeout(setStep(2))` auto-advance is removed from both `selectChip`/`toggleChip` handlers — the user always presses Next to advance. **Enforcement:** strict-grep gate `.github/scripts/strict-grep/i-proposed-1219-form-no-autoadvance-multiselect.mjs` (`--self-test`), wired as job `orch-1219-form-no-autoadvance-multiselect` in `strict-grep-mingla-business.yml`. Fails-on-revert proven (re-introduce the auto-advance / revert interest to single-select → gate exit 1). Runtime-proven by the tester in headless Chromium (chip tap stays on step 1; 3 chips hold simultaneously; organiser 2nd pick replaces 1st). Seth-directed 2026-06-22 (follow-up to ORCH-1216).
