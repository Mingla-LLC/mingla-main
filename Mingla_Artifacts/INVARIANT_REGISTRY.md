...
## DRAFT — ORCH-1273 (venue deck-readiness web flash — create lands on durable route, 2026-07-03)

> ORCH-ID COLLISION RESOLVED: the admin Identity console shipped first as **ORCH-1272** (`I-PROPOSED-1272-IDENTITY-ADMIN-READ`, merged `cf0cff156`) and KEEPS the number. This deck-readiness-web-flash work was renumbered **1272 → 1273** (shipped-first-keeps-the-number) across branch/gate/invariant/test/report tokens before merge. Registered DRAFT at ORCH-1273 IMPLEMENT (`reports/INVESTIGATION_ORCH-1273_DECK_READINESS_WEB_FLASH.md` §G). Client-only, web-facing fix (no migration/edge/native — business native OTA prohibited per COMMS-0052/0063, so it reaches native on the next native build). The orchestrator flips DRAFT → ACTIVE at CLOSE once the tester independently re-proves fails-on-revert. Enforcement = strict-grep `.github/scripts/strict-grep/i-proposed-1273-create-lands-on-durable-deck-readiness.mjs` (`--self-test` 5/5, wired as job `orch-1273-create-lands-on-durable-deck-readiness` in `strict-grep-mingla-business.yml`) + the append-only jest suite `mingla-business/src/components/venue/__tests__/venueCreateDurableDeckReadiness.orch1273.test.ts`.

### I-PROPOSED-1273-CREATE-LANDS-ON-DURABLE-DECK-READINESS (DRAFT)
...
- **Enforcement:** strict-grep `i-proposed-1273-create-lands-on-durable-deck-readiness.mjs` (comment-stripped over `VenueCreatorWizard.tsx`) — asserts (a) PRESENT `router.replace(routeForDeckReadinessFix(`, (b) ABSENT `<VenueDeckReadinessSetup` JSX, (c) ABSENT `createdVenue`/`setCreatedVenue` state. Plus the AST jest suite (durable-nav call exists with the `review_pipeline` param contract; no inline mount; no ephemeral state; the durable route reloads via `useVenueListing` + `useBrandPlaceAuthoringContext`).
- **Fails-on-revert:** deleting the `router.replace(routeForDeckReadinessFix(...))` seam (or reverting to `setCreatedVenue(...)` + the inline `<VenueDeckReadinessSetup>` mount) → gate exit 1 AND the jest core assertion fails. Proven by TRUE LINE DELETION (see `reports/IMPLEMENTATION_ORCH-1273_DECK_READINESS_WEB_FLASH.md`).
- **Established:** DRAFT at ORCH-1273 IMPLEMENT; flips ACTIVE at CLOSE.
...
### I-PROPOSED-1216-TESTFLIGHT-BEHIND-SUBMIT (ACTIVE — flipped at ORCH-1216 CLOSE 2026-06-22)
**Rule:** On the explorer marketing surface, the TestFlight invite URL (`https://testflight.apple.com/join/1gvHNqkQ`) must appear ONLY inside the iOS-success branch of `mingla-marketing/components/marketing/get-the-app-modal.tsx` — i.e. behind a successful lead submit (`status === 'success'`) AND `platform === 'ios'`. It must never appear on idle, step 1, step 2, any error state, or the non-iOS success branch. Hard-gate, no fail-open (Seth decision 1). **Enforcement:** strict-grep gate `.github/scripts/strict-grep/i-proposed-1216-testflight-behind-submit.mjs` (`--self-test`), wired as job `orch-1216-testflight-behind-submit` in `strict-grep-mingla-business.yml`. Fails-on-revert proven (implementor: moving the URL into the Step-1 body → gate exit 1). Live-fire: 50/50 browser assertions (tester) confirmed the link absent in every non-iOS-success state.

### I-PROPOSED-1216-ANDROID-NO-TESTFLIGHT-LINK (ACTIVE — flipped at ORCH-1216 CLOSE 2026-06-22)
**Rule:** The non-iOS (Android/desktop) success branch of `get-the-app-modal.tsx` must render Seth's exact "we'll let you know the moment Android drops" message and NO TestFlight link / no `href` to apple.com. The lead is still captured with `platform` recorded (Seth decision 3). **Enforcement:** strict-grep gate `.github/scripts/strict-grep/i-proposed-1216-android-no-testflight-link.mjs` (`--self-test`), wired as job `orch-1216-android-no-testflight-link`. Fails-on-revert proven. Live-fire: Android + desktop + iPadOS-13(MacIntel+touch=iOS) + touchless-Mac(=other) branches all browser-verified by the tester.

### I-PROPOSED-1216-NO-SERVICE-KEY-CLIENT (ACTIVE — flipped at ORCH-1216 CLOSE 2026-06-22)
**Rule:** No service-role key or other server secret may appear in `mingla-marketing` client code. The explorer lead submit (`lib/explorer-app-submit.ts`) uses ONLY the public anon key via raw fetch to the `explorer-app-lead-submit` edge fn; the service role lives solely in the edge fn (writes bypass RLS). Mirrors I-1045-NO-SERVICE-KEY-CLIENT. **Enforcement:** strict-grep gate `.github/scripts/strict-grep/i-proposed-1216-no-service-key-client.mjs` (`--self-test`), wired as job `orch-1216-no-service-key-client`. Fails-on-revert proven.

### I-PROPOSED-1216-EXPLORER-ONLY-CTA (ACTIVE — flipped at ORCH-1216 CLOSE 2026-06-22)
**Rule:** `GetTheAppModal` mounts ONLY on the explorer surface of `glass-nav.tsx`; the organiser surface keeps its existing `BetaAccessModal`. The two lead-capture flows (consumer `explorer_app_leads` vs organiser `beta_access_leads`) stay separate and must not cross-mount. **Enforcement:** strict-grep gate `.github/scripts/strict-grep/i-proposed-1216-explorer-only-cta.mjs` (`--self-test`), wired as job `orch-1216-explorer-only-cta`. Fails-on-revert proven.

### I-PROPOSED-1216-SUCCESS-MOUNT-GATED (ACTIVE — flipped at ORCH-1216 CLOSE 2026-06-22)
**Rule:** `<SuccessPanel` (which contains the platform-branched TestFlight reveal) must mount ONLY under `status === 'success'` and receive the detected `platform` prop — a caller-side guard preventing a fail-open where the success panel (and thus the link) could render in a non-success state. Tester-authored adversarial guard (different angle than the implementor's callee-side body scans). **Enforcement:** strict-grep gate `.github/scripts/strict-grep/i-proposed-1216-success-mount-gated.mjs` (`--self-test`, 4/4), wired as job `orch-1216-success-mount-gated` in `strict-grep-mingla-business.yml` (orchestrator-wired at CLOSE, closing tester P2-1). Fails-on-revert proven (flip the mount to `status === 'idle'` → this gate exit 1 while the implementor's primary gate stayed green — proving it catches a distinct failure mode).

### I-PROPOSED-1216-ANON-NO-SELECT (ACTIVE — flipped at ORCH-1216 CLOSE 2026-06-22)
**Rule:** `public.explorer_app_leads` must never expose an anon/authenticated SELECT (or any anon write) policy. RLS is enabled with ZERO permissive policies (deny-by-default); the service-role edge fn is the only write path; admin reads go through the SECURITY DEFINER `admin_explorer_app_leads_list()` RPC (EXECUTE granted to `authenticated` only). Lead PII must never be readable with the public anon key. **Enforcement:** structural — RLS-enabled + no policy (verified live at deploy: `relrowsecurity=true`, `pg_policies` count = 0) + live-fire (anon REST SELECT returned `[]`, no rows). A future migration adding an anon SELECT policy here is a P0 regression. Mirrors ORCH-1045's beta_access_leads deny-by-default.

### I-PROPOSED-1219-FORM-NO-AUTOADVANCE-MULTISELECT (ACTIVE — flipped at ORCH-1219 CLOSE 2026-06-22; AMENDED at ORCH-1221 CLOSE 2026-06-22 — organiser now MULTI-select too)
**Rule (as amended by ORCH-1221):** Neither marketing lead-capture modal may auto-advance its first step on chip tap, and BOTH first steps are MULTI-SELECT toggle groups (`useState<string[]>`, `role="group"`, `aria-pressed`) gated on ≥1 selection — the explorer `GetTheAppModal` interest step AND the organiser `BetaAccessModal` brand-type step. The 220ms pointer `setTimeout(setStep(2))` auto-advance is removed from both handlers — the user always presses Next to advance. **(ORCH-1219 originally kept the organiser brand-type single-select; ORCH-1221 flipped it to multi-select per Seth — "the business form should also accept multi pill select." The no-auto-advance + explorer-multi-select rules are unchanged.)** **Enforcement:** strict-grep gate `.github/scripts/strict-grep/i-proposed-1219-form-no-autoadvance-multiselect.mjs` (`--self-test`, rule B1 amended at ORCH-1221 to expect organiser `useState<string[]>` + group/aria-pressed + `.length` gate), wired as job `orch-1219-form-no-autoadvance-multiselect`. Fails-on-revert proven (re-introduce the auto-advance / revert either form to single-select → gate exit 1). Runtime-proven by the tester in headless Chromium at both ORCH-1219 (explorer) and ORCH-1221 (organiser multi-select holds ≥2 chips). Seth-directed 2026-06-22.

### I-PROPOSED-1219-ALWAYS-EMAIL-DOWNLOAD-LINK (ACTIVE — flipped at ORCH-1219 CLOSE 2026-06-22)
**Rule:** On every newly-`created` explorer lead submit, the `explorer-app-lead-submit` edge fn must send a BRANDED email to the lead's own address containing the TestFlight download link (`https://testflight.apple.com/join/1gvHNqkQ`) — for iOS AND non-iOS submitters alike (so Android/desktop users can open it on an iPhone). Built through the shared `_shared/email/shell.ts` `renderShell` (NOT a bare `<!doctype html>` outside `_shared/email/**` — that also satisfies the ORCH-0785-D shell-singleton gate). NOT re-sent on an `already_on_list` duplicate (parity with the internal seth@usemingla.com notify, which is retained). Best-effort: a send failure is logged + non-fatal (the lead is already saved). **Enforcement:** strict-grep gate `.github/scripts/strict-grep/i-proposed-1219-always-email-download-link.mjs` (`--self-test`), wired as job `orch-1219-always-email-download-link`. Fails-on-revert proven (drop the user-email build/dispatch on the created path → gate exit 1; tester runtime-proved created-sends + duplicate-does-not-resend + recipient == lead.email). NOTE: this delivers the download link to non-iOS users by EMAIL — the on-screen hard-gate (I-PROPOSED-1216-{TESTFLIGHT-BEHIND-SUBMIT,ANDROID-NO-TESTFLIGHT-LINK}) is preserved (no on-screen link for non-iOS); both invariants coexist. Seth-directed 2026-06-22.

### I-PROPOSED-1221-ALLPILL-SELECTS-ALL (ACTIVE — flipped at ORCH-1221 CLOSE 2026-06-22)
**Rule:** In the explorer `GetTheAppModal`, the "All of it" (`all`) interest chip is a SELECT-ALL control, not a plain independent toggle. Tapping it when not everything is selected → selects ALL interest values; tapping it when everything is selected → clears all; selecting every SPECIFIC pill individually makes "All of it" auto-reflect selected (and ONLY at full coverage); deselecting any specific pill removes `all`. Implemented as a pure React-free reducer `nextInterest` in `mingla-marketing/lib/explorer-interest.ts` (the modal's chip handler delegates to it; "All of it" selected-state derived from `SPECIFIC_INTERESTS.every`). **Enforcement:** strict-grep gate `.github/scripts/strict-grep/i-proposed-1221-allpill-selects-all.mjs` (`--self-test`, 6/6), wired as job `orch-1221-allpill-selects-all` in `strict-grep-mingla-business.yml`. Fails-on-revert proven (reduce "All of it" to a dumb toggle / bare `includes` → gate exit 1 + reducer test FAIL). Runtime-proven by the tester in headless Chromium (26/26 — tap-from-partial selects all; 4/4 specifics auto-highlights; deselect removes `all`). Seth-directed 2026-06-22 ("the all of it should select all pill"). **ID NOTE:** registered as ORCH-1220 but a parallel session shipped a different ORCH-1220 (App-Review reviewer-login bypass, PR #646) FIRST → per shipped-first-keeps-the-number this work renumbered to ORCH-1221 pre-merge (branch kept the `1220-` label); see COMMS-0060.
