...
### I-PROPOSED-1223-FOOTER-MOUNTED (ACTIVE — flipped at ORCH-1223 CLOSE 2026-06-22; SUPERSEDES the ORCH-1053 footer removal)
**Rule:** The marketing `Footer` must be MOUNTED on both surfaces — `mingla-marketing/app/(explorer)/layout.tsx` renders `<Footer surface="explorer" />` and `mingla-marketing/app/organisers/layout.tsx` renders `<Footer surface="organiser" />` (both import `Footer` and render it, not commented out). This guards against the ORCH-1053 failure mode where the footer was silently un-mounted ("footer removed per operator") and went unnoticed until ORCH-1223. **Enforcement:** strict-grep gate `.github/scripts/strict-grep/i-proposed-1223-footer-mounted.mjs` (`--self-test`, comment-stripped — a commented-out mount fails), wired as job `orch-1223-footer-mounted`. Fails-on-revert proven (remove the explorer mount → gate exit 1). Browser-proven: `<footer>` count = 1 on `/` and `/organisers` (was 0 pre-ORCH-1223). Seth-directed 2026-06-22 (re-mount a clean footer for the public store launch — SUPERSEDES ORCH-1053's removal).
