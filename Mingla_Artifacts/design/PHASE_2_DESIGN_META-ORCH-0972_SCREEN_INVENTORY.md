...
| `brandsService.ts` create contract | `CreateBrandInput` still accepts a brand `kind` value from creation UI | DROP `kind` from `CreateBrandInput`; unified creation no longer sends persona/kind | D1: brandsService.ts lines 93–128 (`CreateBrandInput` kind field delete) | YES | YES | YES |
| `brandMapping.ts` brand-kind plumbing | Brand TS type + insert/update mappers still carry brand `kind` through row/UI boundaries | DELETE brand-kind type fields and mapper passthroughs identified by audit | D1: brandMapping.ts lines 47–48, 91–92, 240–243, 311, 395 | YES | YES | YES |
| `brandPatch.ts` dirty patch | Dirty patch can persist `draft.kind` changes back to the brand row | DELETE the 3-line kind dirty-patch block | D1: brandPatch.ts lines 38–40 (dirty-patch delete; Codex G2) | YES | YES | YES |
| `useBrands` create mutation | `useCreateBrand` input still carries kind from the creation flow | DROP kind from mutation input to match the unified create contract | D1: useBrands.ts (`useCreateBrand` input kind delete) | YES | YES | YES |
| D1 obsolete persona/kind tests | Tests assert persona fork, trip-only wizard, and trip-planner kind create behavior | DELETE obsolete tests; replacement regression coverage belongs to Phase 3/4 unified-flow spec | D1: BrandSwitcherSheet.personaFork.test.ts, BrandSwitcherSheet.personaFork.ve1.test.ts, BrandSwitcherSheet.personaFork.ve2.test.ts, TripBrandWizard.test.ts, brandsService.tripPlannerKind.test.ts | YES | YES | YES |
