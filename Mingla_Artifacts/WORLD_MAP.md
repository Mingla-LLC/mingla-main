...
| ORCH-0434 | **Preferences Simplification Initiative** — Restructure categories (12→8), remove budget+time slot filters, bake time into Google hours, redesign preferences sheet order+toggles, redesign onboarding to match, migrate all users | Discovery / Onboarding / Admin | S0 | architecture-flaw | in-progress | D | 2026-04-15 | **Phase 1+2+3A PASS.** DB slugs migrated (P1), shared libs updated (P2), card serving pipeline compilation fixed + filterByDateTime rewritten to 3-mode date filter + budget/time removed from 5 edge functions (P3A). Phase 3B (slug updates in remaining 8 edge functions) next. |
