...
| ORCH-0480 | Admin Place Pool page 500s — all 3 RPCs (pool_overview, category_breakdown, country_overview) timeout | Admin + Backend | S1 | performance | open | F | — | User-reported 2026-04-17 shortly after ORCH-0460 deploy. **NOT caused by ORCH-0460** — pre-existing perf debt surfaced by pool growth to 63,239 active rows. EXPLAIN ANALYZE: category_breakdown takes **16.8s** scanning 40,951 rows with expression-based GROUP BY on `ai_categories[1]` (no matching index). country_overview has 7 subqueries × 7 countries per call. PostgREST statement_timeout ≈ 8s. **NARROW emergency unblock — systemic fix tracked as ORCH-0481.** Fix: (1) expression index on ai_categories[1]; (2) rewrite country_overview to single-pass. See prompts/IMPL_ORCH-0480_ADMIN_RPC_TIMEOUT.md. |
