...
| ORCH-0434 | **Preferences Simplification Initiative** — Restructure categories (12→8), remove budget+time slot filters, bake time into Google hours, redesign preferences sheet order+toggles, redesign onboarding to match, migrate all users | Discovery / Onboarding / Admin | S0 | architecture-flaw | in-progress | D | 2026-04-15 | **Phase 1+2 PASS.** DB slugs migrated (P1), shared libs updated to match (P2). DISPLAY_TO_SLUG now outputs canonical slugs. Budget/price params removed from card query pipeline. 7 phases remaining (3-9). |
