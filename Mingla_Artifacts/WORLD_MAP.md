...
| ORCH-0474 | `discover-cards` silently degrades 3 failure modes to one "pool-empty" response; collab multi-category session sees "no spots" while server holds 1,595 matching rows | Discovery + Backend | S1 | architecture-flaw | awaiting-test | F | — | Investigation + spec + implementation ALL COMPLETED 2026-04-17. Investigation (`outputs/INVESTIGATION_ORCH-0474_COLLAB_MULTICATEGORY_EMPTY.md`): root cause HIGH — silent error conflation at `discover-cards/index.ts:510-598`. RPC innocent (direct test: 1,595 rows). Spec (`outputs/SPEC_ORCH-0474_DISCOVER_CARDS_FALLTHROUGH_SPLIT.md`): 3-path split, remove redundant `auth.getUser`, kill silent catch, wire `serverPath` through client, add 2 new `DeckUIState` variants, 6 i18n keys × 29 locales, Mixpanel `server_path` dimension. Implementation (`outputs/IMPLEMENTATION_ORCH-0474_DISCOVER_CARDS_FALLTHROUGH_SPLIT.md`): APPROVED STRUCTURALLY by orchestrator — 9 mobile files + 1 edge fn + 29 locales, TS compile clean, all 10 common-mistake safeguards verified. Establishes INV-042 + INV-043 (lock in on tester PASS). Tester dispatch ready: `prompts/TESTER_ORCH-0474_DISCOVER_CARDS_FALLTHROUGH_SPLIT.md`. USER ACTION REQUIRED before tester runs: `npx supabase functions deploy discover-cards --project-ref gqnoajqerqhnvulmnyvv`. |
...
| ORCH-0485 | Deck takes several seconds to show cards after preference change — not the <1s target promised by ORCH-0340 singles-first path | Discovery | S1 | performance | awaiting-investigation | F | — | User report 2026-04-17. Target: first card visible ≤1s after pref save. Actual: multi-second blank/skeleton. Candidate causes: (a) `onSinglesReady` partial-cache write not reaching UI (ORCH-0451 `isPlaceholderData` guard may be suppressing it); (b) `discover-cards` Deno cold start 4-9s (ORCH-0342 timeout is 15s); (c) `isRefreshingAfterPrefChange` held until `!isDeckPlaceholder && recommendations.length > 0` — partial stream may not satisfy sync effect; (d) `curated-experiences` serial enrichment blocking interleave; (e) `useUserLocation` re-resolve on refreshKey churn. Investigator dispatch: `prompts/INVESTIGATOR_ORCH-0485_DECK_PREF_CHANGE_LATENCY.md`. |
...
| ORCH-0481 | Admin RPC materialized view layer — ~22 vulnerable admin RPCs all run live aggregates over place_pool/card_pool/profiles | Admin + Backend | S1 | architecture-flaw | **implemented-awaiting-verify** | F | — | Systemic fix superseding ORCH-0480 per DEC-021. Implementation delivered 2026-04-17 by implementor: `supabase/migrations/20260418000001_orch0481_admin_mv_layer.sql` (new, 1080 lines). Creates `admin_place_pool_mv` (27 cols, 5 indexes), pg_cron 10-min refresh, `admin_refresh_place_pool_mv()` RPC, rewrites **20 of 22** admin RPCs to read from MV. 2 deferred with rationale (`admin_city_picker_data` — preserves empty-cities semantic; `admin_ai_validation_preview` — GIN-indexable, not a perf risk). Report: `reports/IMPLEMENTATION_ORCH-0481_ADMIN_MV_LAYER_REPORT.md`. Implementor expanded MV beyond spec's 17-col sketch to 27 cols after full body audit (added pp_country, pp_city, seeding_category, google_place_id, photos, updated_at, etc.). Structural review PASS — signatures byte-identical, auth gates first statement in every BEGIN, CASCADE+CREATE atomic, rollback documented. **Awaiting `supabase db push` + tester verification** (prompt: `prompts/TESTER_ORCH-0481_ADMIN_MV_LAYER.md`). New discoveries: D-3 (`has_photos` bakes in `__backfill_failed__` sentinel — tightening vs legacy); D-5 (`admin_pool_stats_overview` is partial win — photo_health/refresh_health totals from MV, card_pool sections still live); D-7 (ORCH-0484 rows surface as `primary_category='uncategorized'` and are excluded from breakdown to match legacy). |
