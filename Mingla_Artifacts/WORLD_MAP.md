...
| ORCH-0469 | Brunch/Lunch/Casual pref change shows false "seen everything" in warm app — cold start fixes it | Discovery | S1 | bug | failed-test-rework-dispatched | F | — | **QA FAIL 2026-04-17** — `outputs/QA_ORCH-0469-0472_REPORT.md`. Structural cache/gate fix passes; UI split introduced P1 regression on true-exhaustion swipe-through path. Rework dispatch: `prompts/IMPLEMENTOR_REWORK_ORCH-0469-0472.md` (2-line change in SwipeableCards.tsx:616-622). Cycle #1. |
| ORCH-0472 | EMPTY and EXHAUSTED deck states render identical copy ("You've seen everything") + fire same Mixpanel event | Discovery | S2 | ux | failed-test-rework-dispatched | F | — | **QA FAIL 2026-04-17** (joint with ORCH-0469). (Originally mis-registered as ORCH-0471.) UI split regressed the pre-fix "swipe-through small pool" exhaustion — user sees EMPTY copy instead of EXHAUSTED. Cycle #1 rework scoped to `effectiveUIState` in SwipeableCards. |
