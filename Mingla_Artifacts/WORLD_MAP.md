...
| ORCH-0481 | Admin RPC materialized view layer — ~22 vulnerable admin RPCs all run live aggregates over place_pool/card_pool/profiles | Admin + Backend | S1 | architecture-flaw | **rework-cycle-2-delivered-awaiting-verify** | F | — | Cycle 2 cron fix delivered 2026-04-17 by implementor: `supabase/migrations/20260418000003_orch0481_cron_fix.sql` (128 lines). Report: `reports/IMPLEMENTATION_ORCH-0481_REWORK_V3_REPORT.md`. Creates `cron_refresh_admin_place_pool_mv()` plpgsql fn with `SET statement_timeout TO '15min'` + `REFRESH MATERIALIZED VIEW CONCURRENTLY` + `ANALYZE admin_place_pool_mv`. Re-schedules cron to call the fn. One-shot call at migration end validates fn body + catches up the stale MV. Preconditions self-verified (MV exists, broken cron active, 8/8 failed runs). Structural review APPROVED — addresses P0-1 (cron timeout) and P1-2 (category_breakdown regression via ANALYZE) in one migration. MV/indexes/20 RPCs/admin_refresh_place_pool_mv() all untouched. Cycle history: cycle 0 FAIL → cycle 1 rework/FAIL → cycle 2 rework/pending tester. Cycle count 2 (stuck-in-loop threshold is 3+). **Awaiting `supabase db push` + tester cycle 2 retest** (reuse `prompts/TESTER_ORCH-0481_ADMIN_MV_LAYER.md` with SC-28 focus). ORCH-0488 (cache-sensitivity) still deferred to prod measurement. |
