...
| ORCH-0481 | Admin RPC materialized view layer — ~22 vulnerable admin RPCs all run live aggregates over place_pool/card_pool/profiles | Admin + Backend | S1 | architecture-flaw | **rework-cycle-1-delivered-awaiting-verify** | F | — | Cycle 0 FAIL superseded. Cycle 1 rework delivered 2026-04-17 by implementor: `supabase/migrations/20260418000002_orch0481_rework_count_fix.sql` (298 lines, `CREATE OR REPLACE` of 3 functions only — MV/cron/indexes/17 other RPCs untouched). Report: `reports/IMPLEMENTATION_ORCH-0481_REWORK_V2_REPORT.md`. Self-EXPLAIN verified: `admin_place_country_overview` 53,875ms → **113ms** (475× faster — PASS <500ms target) via restructured aggregate-MV-first pattern; `admin_place_pool_overview` global 4,932ms → ~**500ms** extrapolated (PASS) via Option A branched pattern with 8 narrow Index-Only subqueries; `admin_place_city_overview` 5,118ms → **3,061ms** (PARTIAL — under 8s PostgREST timeout but over <500ms target). Principled deviation from rework prompt: COUNT(mv.*)→COUNT(mv.id) substitution alone yielded 5,060ms (10× over target); query-shape restructuring was required for true <500ms. Transparently documented. Partial residual on city_overview registered as ORCH-0487 (covering-index follow-up). Structural review APPROVED. **Awaiting `supabase db push` + tester cycle 1** (reuse `prompts/TESTER_ORCH-0481_ADMIN_MV_LAYER.md`). |
