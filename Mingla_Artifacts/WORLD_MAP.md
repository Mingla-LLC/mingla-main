...
| ORCH-0485 | Deck takes several seconds to show cards after preference change — not the <1s target promised by ORCH-0340 singles-first path | Discovery | S1 | performance | **spec pending (absorbed into ORCH-0490 program)** | F | — | User report 2026-04-17. Investigation `outputs/INVESTIGATION_ORCH-0485_DECK_PREF_CHANGE_LATENCY.md` complete — 3 root causes proven (H on #1 and #3, M on #2). #1: `useUserLocation.ts:149` includes `refreshKey` in query key → every pref change invalidates location (1–3s warm, up to 13s timeout). #2: `deckService.ts:366-393` hardcodes singles-first so curated can't deliver first even when faster. #3: when singles returns 0 cards, partial-delivery path skipped entirely. Side discoveries: ORCH-0486 (mixed-deck serverPath), ORCH-0494 (false EMPTY race), ORCH-0495 (client warm-ping). Now part of ORCH-0490 Phase 2.1 (#1 + #3) + 2.2 (#2). |
...
| ORCH-0492 | Session persistence — last-card position restored across app close + mode switches | Discovery + State & Cache | S1 | missing-feature | charter | F | — | User JTBD 2026-04-17: "switch from solo to a session, close the app, come back to the last card I was on." Current state inferred: `accumulatedCardsRef` + `sessionServedIdsRef` in-memory only, cleared on mode switch per `RecommendationsContext.tsx:895-896`. No AsyncStorage resume for card index confirmed. **Phase 0-B investigator must establish exact baseline before design.** Target: `(mode, sessionId, cardId, cardIndex, accumulatedStack, servedIds)` persist across backgrounding + cold launch. Constitutional #14 enforcement. Part of ORCH-0490 Phase 2.5. |
| ORCH-0493 | Collab multi-participant preference change pressure — verify "changes in background, shown in next card" | Collaboration | S1 | correctness | **verification required** | F | — | User assertion 2026-04-17: *"collab mode has a shared deck with multiple changing preferences ... changes are made in the background and shown in the next card. Confirm this."* Orchestrator will NOT confirm from reading alone. Phase 0-C investigator must trace: participant A changes prefs while participant B is mid-swipe — does `aggregateCollabPrefs()` re-compute? Via realtime/invalidation/polling? Does B's position + accumulated stack survive? Is B's next card computed against A's old or new prefs? Possible outcomes: works as user believes (regression tests only) / partially (scope grows) / broken (new spec). Part of ORCH-0490 Phase 0-C + possibly Phase 2.6. |
| ORCH-0494 | False EMPTY race on preference change — 20s safety timer flips completion flag before cards land; EMPTY renders with unknown `filter-outline` icon; `trackDeckEmptyFilter` analytic fires for populated server responses | Discovery | S1 | architecture-flaw | charter | F | — | Splinter of ORCH-0485 for lifecycle discipline. Root cause: one-shot 20s safety timer at `RecommendationsContext.tsx:708` + refresh-handler reset at `RecommendationsContext.tsx:635`. State machine at `RecommendationsContext.tsx:1250` falls into EMPTY when `recommendations.length===0 && hasCompletedFetchForCurrentMode===true`. EMPTY branch at `SwipeableCards.tsx:1576` uses `filter-outline` icon (ORCH-0475) and fires `trackDeckEmptyFilter` at `SwipeableCards.tsx:1585`. Reproducer log: server returned 987 cards 8312ms AFTER EMPTY rendered. Also pollutes analytics (overcount of "no matches"). Target: EMPTY is server-verdict-only. New invariant `I-DECK-EMPTY-IS-SERVER-VERDICT`. Part of ORCH-0490 Phase 2.1. |
...
| ORCH-0480 | Admin Place Pool page 500s — all 3 RPCs (pool_overview, category_breakdown, country_overview) timeout | Admin + Backend | S1 | performance | **FAIL — superseded by ORCH-0481** | F | — | User-reported 2026-04-17 shortly after ORCH-0460 deploy. Pre-existing perf debt surfaced by pool growth to 63,239 active rows. Migration `20260417300001_orch0480_admin_rpc_perf.sql` applied to **Mingla-dev** via `supabase db push` 2026-04-17 (commit 82d94aef). QA report `reports/QA_ORCH-0480_ADMIN_RPC_PERF_REPORT.md` — verdict **FAIL** (3 P0, 1 P1, 1 P2, 3 P4). All 3 RPCs still exceed PostgREST 8s timeout: category_breakdown 9s warm, country_overview 9.2s warm + 9984kB external disk sort, pool_overview DISTINCT sub-clause alone 3.9s. Root cause: partial expression index on `ai_categories[1]` only accelerates filter-narrowing; real RPCs project heap-resident columns (`stored_photo_urls`, `rating`) forcing Bitmap Heap Scan + detoast. Proven via Index-Only Scan baseline: pure `COUNT(*)` over same predicate = 53ms, but no real RPC can reach that path. Index IS used, signatures preserved, semantics correct, auth gates intact, existing indexes still work (SC-4/5/6/8/10 PASS). SC-1/2/3 FAIL. **Not applied to production per tester recommendation.** DEC-021 logs the pivot to ORCH-0481. |
| ORCH-0481 | Admin RPC materialized view layer — ~22 vulnerable admin RPCs all run live aggregates over place_pool/card_pool/profiles | Admin + Backend | S1 | architecture-flaw | **STUCK-IN-LOOP (cycle 3) — strategic pause** | F | — | Cycle 2 cron fix applied (commit 84f00a93) but tester cycle 2 retest **FAIL** per `reports/QA_ORCH-0481_ADMIN_MV_LAYER_REPORT_RETEST_2.md`. Cron jobid=13 has **0 runs in 4+ hours** despite correct config (schedule `*/10 * * * *`, command `SELECT public.cron_refresh_admin_place_pool_mv()`, active=true). Other cron jobs fire normally — jobid=13 specifically skipped. MV proven stale: 63,273 vs place_pool 63,274 (one row added post-deploy never propagated). Function attributes correct (statement_timeout=15min + lock_timeout=15min) but function never invoked by cron. Constitutional #3 still violated. **Cycle count reached 3 → stuck-in-loop escalation triggered.** DEC-023: pause implementor dispatch, strategic review instead. Query-rewrite wins from cycles 0/1 still hold (country_overview 87ms hot, pool_overview ~400ms). Working alternatives: (1) admin-UI "Refresh Stats" button calling `admin_refresh_place_pool_mv()` — zero cron dependency, ships today; (2) narrow the MV — drop TOAST'd cols so REFRESH fits in 2min naturally; (3) ship current state to prod + measure prod-cron behavior. Recommend (1)+(3) in parallel. |
...
| ORCH-0489 | Admin UI "Refresh Stats" button — replaces failed pg_cron auto-refresh with user-controlled refresh | Admin UI | S1 | missing-feature | **ready to dispatch** | F | — | Registered 2026-04-18 as user-chosen Alternative 1 following ORCH-0481 stuck-in-loop (DEC-023). Scope: 1 file modified (`mingla-admin/src/pages/PlacePoolManagementPage.jsx`, ~30-60 lines inline). Calls existing `admin_refresh_place_pool_mv()` RPC (live on Mingla-dev, admin-auth-gated, returns jsonb `{success,row_count,duration_ms}`). Loading spinner during RPC (30s-2min expected), success toast on completion with row count + duration, error toast on failure, auto-refires `fetchData()` on success. Preserves all query-rewrite wins from ORCH-0481 cycles 0/1. Zero SQL changes. No EAS OTA (admin is web). Prompt: `prompts/IMPL_ORCH-0489_ADMIN_REFRESH_BUTTON.md`. |
