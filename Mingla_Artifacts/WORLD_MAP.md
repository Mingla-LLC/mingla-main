...
| ORCH-0481 | Admin RPC materialized view layer — ~22 vulnerable admin RPCs all run live aggregates over place_pool/card_pool/profiles | Admin + Backend | S1 | architecture-flaw | **tester-fail-cycle-1, rework-cycle-2-dispatched** | F | — | Cycle 1 rework applied 2026-04-17 (commit 6f281d6b). Tester cycle 1 retest verdict **FAIL** (1 P0, 2 P1, 2 P2, 2 P4) per `reports/QA_ORCH-0481_ADMIN_MV_LAYER_REPORT_RETEST_1.md`. **NEW P0 DISCOVERY: pg_cron REFRESH has failed on every single run (6/6 runs at 2-min statement timeout).** The MV has not been auto-refreshed since migration apply — the 10-min staleness guarantee is structurally broken. Constitutional #3 violation (silent failure). Partial wins: country_overview 87ms warm (PASS <500ms), pool_overview ~400ms (PASS), city_overview 3,014ms (matches implementor's partial fix, ORCH-0487 tracks covering-index follow-up). NEW P1-1: admin RPCs are cache-sensitive on Mingla-dev — 2-5s cold, sub-100ms hot. Implementor's cycle 1 numbers were fully-cached runs. NEW P1-2 (regression): `admin_place_category_breakdown` 107ms → 4,219ms (unchanged body; plan drift + cold cache + stale stats). Cycle 2 rework prompt: `prompts/IMPL_ORCH-0481_REWORK_V3.md`. Fix: wrap refresh in plpgsql function with `SET LOCAL statement_timeout = '15min'` + `ANALYZE` step (addresses P0-1 and P1-2). P1-1 deferred to prod measurement as ORCH-0488. **NOT applied to production.** |
