...
| ORCH-0377 | Beta feedback — allow users to delete their own submitted feedback | Profile | S2 | missing-feature | speccing | F | — | User request. No delete path exists. Spec prompt written. Needs: DELETE RLS (table + storage), service method, mutation hook, trash icon + confirmation dialog in history UI. |
