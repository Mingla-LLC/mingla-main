...
| ORCH-0443 | Collab session active → blank deck — 7 competing seeding paths + race condition + silent failures | Collaboration | S0 | architecture-flaw | implemented | B | 2026-04-16 | Commit ec9bbbf4. collabPreferenceSeedService.ts created. 6 redundant write paths deleted. Race condition eliminated (seed before participant insert). Deck solo fallback added. Backfill migration applied. Edge function deployed. Needs device verification + formal test pass. |
