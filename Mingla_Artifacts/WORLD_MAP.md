...
| ORCH-0474 | Collab multi-category deck query returns 0 cards when brunch+play queried together — same RPC returns 1342 for brunch alone | Discovery + Backend | S1 | bug | awaiting-investigation | F | — | User report 2026-04-17 with live device log. Same session, same location, 2 parallel `discover-cards` calls: (a) 1 pill `brunch_lunch_casual` → 1342 cards, (b) 2 pills `brunch_lunch_casual + play` with `dateWindows: ["this_weekend"]` + `sessionId` → 0 cards with `path: "pool-empty"`. Impossible by SQL set semantics (`&&` overlap can only add matches). Hypothesis: `query_pool_cards` RPC `PARTITION BY d.category` + `v_per_category_cap` interaction when multiple slugs present, OR race between solo+collab parallel fetches. Investigator dispatch: `prompts/INVESTIGATOR_ORCH-0474_COLLAB_MULTICATEGORY_EMPTY.md`. |
...
| ORCH-0480 | Admin Place Pool page 500s — all 3 RPCs (pool_overview, category_breakdown, country_overview) timeout | Admin + Backend | S1 | performance | **implemented-awaiting-verify** | F | — | User-reported 2026-04-17 shortly after ORCH-0460 deploy. **NOT caused by ORCH-0460** — pre-existing perf debt surfaced by pool growth to 63,239 active rows. EXPLAIN ANALYZE: category_breakdown takes **16.8s** scanning 40,951 rows with expression-based GROUP BY on `ai_categories[1]` (no matching index). country_overview has 7 subqueries × 7 countries per call. PostgREST statement_timeout ≈ 8s. **NARROW emergency unblock — systemic fix tracked as ORCH-0481.** Fix: (1) expression index on ai_categories[1]; (2) rewrite country_overview to single-pass. Implementation delivered 2026-04-17: `supabase/migrations/20260417300001_orch0480_admin_rpc_perf.sql` (new), report at `reports/IMPLEMENTATION_ORCH-0480_ADMIN_RPC_PERF_REPORT.md`. **Awaiting `supabase db push` + tester verification.** Implementor left `admin_place_pool_overview` un-rewritten per spec guidance — follow-up needed if still >3s post-deploy. |
| ORCH-0481 | Admin RPC materialized view layer — ~22 vulnerable admin RPCs all run live aggregates over place_pool/card_pool/profiles; 3 already timing out, 19 waiting to | Admin + Backend | S1 | architecture-flaw | open | F | — | Systemic root cause behind ORCH-0480. Audit of 66 admin_* RPCs found 22 with same vulnerability pattern (COUNT + GROUP BY + full-table scan). Only 3 currently fail because place_pool hit ~63k first; others (admin_ai_category_health, admin_card_pool_intelligence, all 5 admin_analytics_*, etc.) will fail as their tables grow. Proper fix: materialized view layer (`admin_place_pool_mv`, `admin_profiles_mv`), refreshed via pg_cron every 10-30min; all affected RPCs rewritten to read from MVs (signatures preserved, admin UI unchanged). Expected result: admin pages <200ms regardless of pool size. See prompts/IMPL_ORCH-0481_ADMIN_MV_LAYER.md. |
...
| ORCH-0484 | 776 approved active places have NULL/empty `ai_categories` — should not exist per AI pipeline rules | Backend + Data Integrity | S2 | data-integrity | open | F | — | Discovered via ORCH-0480 implementor D-2 (2026-04-17). Count query: `SELECT COUNT(*) FROM place_pool WHERE is_active=true AND ai_approved=true AND (ai_categories IS NULL OR array_length(ai_categories,1)=0)` returns 776. These rows are correctly excluded from ORCH-0480's new partial index (predicate requires non-empty categories) so they don't affect admin perf, but their existence contradicts the AI pipeline contract (approval requires category assignment). Need investigator to trace how they bypassed the contract — legacy pre-AI rows, race condition in ai-verify-pipeline, or silent failure in validation writeback. Not blocking ORCH-0480 close. Pair well with ORCH-0481 MV migration (clean up before snapshot). |
