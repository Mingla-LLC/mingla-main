...
| ORCH-0481 | Admin RPC materialized view layer — ~22 vulnerable admin RPCs all run live aggregates over place_pool/card_pool/profiles | Admin + Backend | S1 | architecture-flaw | **tester-fail-cycle-0, rework-dispatched** | F | — | Implementation applied to Mingla-dev 2026-04-17 (commit 352fc4c7). Tester QA cycle 0 verdict **FAIL** (2 P0 / 1 P1 / 2 P2 / 3 P4) — report `reports/QA_ORCH-0481_ADMIN_MV_LAYER_REPORT.md`. Great news: category_breakdown hits **107ms** (84× faster than ORCH-0480's 9s) — MV architecture proven. **Bad news:** P0-1 `admin_place_country_overview` = **53,875ms warm (6× WORSE than ORCH-0480)** due to `COUNT(mv.*)` anti-pattern forcing 624-byte row materialization + 318MB external disk sort. P0-2 `admin_place_pool_overview` global = 4,932ms (under 8s PostgREST timeout so page loads, but 25× over <200ms target) due to full seq scan of wide MV on slow Mingla-dev I/O. Same `COUNT(mv.*)` pattern also suspected in admin_place_city_overview (P1-1, not timed). Rework prompt dispatched: `prompts/IMPL_ORCH-0481_REWORK_V2.md`. Fix is surgical — substitute `COUNT(mv.*)` → `COUNT(mv.id)` across 2 functions (P0-1), plus design decision on P0-2 (multi-subquery split vs covering index). **NOT applied to production.** Cycle 0 P2 findings: D-3 (572 rows affected by sentinel tightening), D-7 (776 orphan rows correctly surfaced). Confirmed by tester: signatures byte-identical (SC-25 PASS), all 20 read from MV (T-1 PASS), auth gates intact, cron active. |
