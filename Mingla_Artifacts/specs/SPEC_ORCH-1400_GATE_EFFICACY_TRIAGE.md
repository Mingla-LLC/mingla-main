...
ORCH-1383 proved every wired gate RUNS. This program makes gates provably CATCH. Phase 1 closes the three structural holes that let gates go dark at CLOSE (uncapped `fixture` state, unswept external gate dirs, permanently-legal `capable-unwired` self-tests) and executes the evidence-backed disposition of the 21 frozen gates + RD-1: wire 13 green gates + the orch-1369 adversarial harness + RD-1, retire 2 tombstones with Seth-visible listing, and hold 5 red gates for reconciliation after their discoveries are routed. Phase 2 backfills real two-direction self-tests onto the 43 money/release/auth token gates (the class that already failed in production). Phases 3–4 finish the remaining 135 by measured mechanical class, ratcheting the floor at every merge.

## 2. Scope & non-goals

**In scope:** MANIFEST schema + ratchets; `meta-1383-manifest-parity.mjs` new rules P10–P12; `run-batch.mjs` expected sets; disposition of the 21 + RD-1; self-test backfill for the 178-gate population (+ audit pass for 19 `node --test` suites, 2 bash); retirement mechanics + notify listing.
...
- A successor gate for the TUS upload pipeline (D-4 — candidate new ORCH).
...
- **WIRE now (13):** F-1 rows 1, 2, 6, 7, 11, 12, 13, 14, 15, 17, 18, 19, 20 → `enforcement: "batch:A"`, `invocation: "node"`, `modes: ["plain"]` (+ `"self-test"` for rows 1, 17, 18 which are `capable-unwired` — flip to `wired`); add to `run-batch.mjs` class-A expected set (P5 forces both sides); `unenforcedCap` 21 → 5.
...
- **HOLD as `unenforced` (5):** F-1 rows 3, 4, 5, 8, 16 — reasons updated to cite ORCH-1400 + the routed discovery ID (D-1…D-3, D-5). They wire in a later phase ONLY once their live run is green (code fixed or violation blessed via each gate's own allowlist-comment mechanism — that call belongs to the discovery's ORCH, not this one).
- **RETIRE (2):** F-1 rows 9, 10 (`orch-0770-event-cover-video-processing.mjs`, `orch-0776a-video-upload-progress-honesty.mjs`) — delete the files + `mingla-business/package.json` rows `test:orch-0770`, `test:orch-0776a`; remove MANIFEST rows; `expectedStrictGrepMjsFiles` 405 → 403; commit message carries `GATE-REMOVAL: ORCH-1400 — guarded Expo-upload/25MB-constant implementation replaced by TUS pipeline (INVESTIGATION F-1 #9/#10)`. **Both retirements appear in the CLOSE notify list to Seth verbatim, with D-4 (no successor coverage) beside them.**
...
- **SC-3:** After Phase 1, `count(unenforced) === 5`, and the 14 newly wired gates (13 + 1369-adv) + RD-1 each appear in the batch run's executed list on the PR (differential check against the PR's own run log).
...
- **SC-5:** The 2 retirements ship with `GATE-REMOVAL: ORCH-1400 …` in the merge commit, `expectedStrictGrepMjsFiles === 403`, both package.json rows gone, and the CLOSE note lists both deletions + D-4 to Seth.
...
| T-11 | Retirement visibility | Phase-1 merge commit | `GATE-REMOVAL: ORCH-1400` token present; CLOSE note lists both files | process |
...
- DELETE only: `orch-0770-event-cover-video-processing.mjs`, `orch-0776a-video-upload-progress-honesty.mjs`
- `mingla-business/package.json` — the two `test:orch-0770`/`test:orch-0776a` rows only
...
- **OQ-3 (orchestrator):** intake D-1…D-5 so the 5 HOLD gates can be reconciled and wired; ORCH-1400's later phases will wire whichever go green in time, else they stay counted in `unenforcedCap: 5`.

## 12. Downstream routing

IMPLEMENT (mingla-implementor) per phase, starting Phase 1, in this worktree/branch; each phase → mingla-tester (adversarial: T-1…T-14 minimum, from a bracket-free path per #958) → orchestrator CLOSE (fresh PR event per COMMS-0109; retirement notify-list at Phase-1 CLOSE; invariant flips at each CLOSE). Working tree: `~/Desktop/mingla-orchs/ORCH-1400-[gate-efficacy-triage]` on branch `ORCH-1400-gate-efficacy-triage`.
