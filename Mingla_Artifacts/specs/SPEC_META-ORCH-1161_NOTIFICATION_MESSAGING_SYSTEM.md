...
## ORCH-1227 amendment — Nigeria via Termii (2026-06-23, DEC-192)

Backend-only slice of META-ORCH-1161 that adds Nigeria SMS delivery behind the existing country/region seam. Ships text-dark (no OTA, no migration).

- **Dual-provider, country-routed.** The unified `smsAdapter.send()` now routes by `countryCode`: `NG` → Termii, every other country → Twilio (unchanged: `MessagingServiceSid`, no raw `From`, `SMS_LIVE_ENABLED_US`). This SUPERSEDES the original smsAdapter single-provider hard-guard. The dispatcher is still the sole send path (`I-PROPOSED-1161-UNIFIED-DISPATCHER-SOLE-SEND-PATH` AMENDED) — Termii is reached ONLY inside `smsAdapter`, never directly from an edge fn/cron.
- **Channel mapping.** NG transactional → Termii `dnd` channel (reaches DND-registered numbers, the NCC corporate route); NG marketing (`messageType: "marketing"`, set at the `marketing-send` call site) → Termii `generic` channel. Send = `POST {TERMII_BASE_URL}/api/sms/send` with `{ api_key, to, from: TERMII_SENDER_ID, sms: composeSmsBody(...), type:"plain", channel }`; success = HTTP 200 with a JSON `message_id` → `providerMessageId`. The shared `composeSmsBody` (STOP footer + GSM-7 sanitize) + `computeSegments` apply to the Termii path too.
- **Kill-switch unchanged.** NG is still gated by `SMS_LIVE_ENABLED_NG` (default OFF) via `resolveMarketKillSwitch` — skipped with ZERO HTTP when off. FAIL-CLOSED: if any of `TERMII_API_KEY` / `TERMII_BASE_URL` / `TERMII_SENDER_ID` is missing the adapter returns `failed:termii_env_missing` with no HTTP.
- **Delivery webhook.** New `supabase/functions/termii-delivery-status` (verify_jwt=false) mirrors `twilio-message-status`: FAIL-CLOSED verifies an `X-Termii-Signature` HMAC-SHA512 over the RAW body using `TERMII_WEBHOOK_SECRET ?? TERMII_API_KEY` (403 on missing secret / absent header / mismatch), then reconciles `notification_deliveries` by `provider_message_id` + `channel='sms'`, and on a DND/opt-out/rejected status writes a `channel_suppressions(channel='sms', scope='all', reason='stop_keyword')` row (same convention as `twilio-inbound-sms` STOP).
- **DEFERRED.** Twilio→NG failover is deferred (low value — same foreign-origin carrier filtering). Webhook signature scheme (HMAC-SHA512 / `X-Termii-Signature` header / API-key-as-secret) is the authoritative build assumption; the orchestrator must confirm it against Termii's live dashboard config before flipping `SMS_LIVE_ENABLED_NG` on.
- **Enforcement.** Strict-grep gate `i-proposed-1227-ng-sms-via-termii.mjs` (wired into `strict-grep-mingla-business.yml` next to the 1161 gate) + deno `smsAdapter.termii.test.ts`. Invariant: `I-PROPOSED-1227-NG-SMS-VIA-TERMII`.
- **Required secrets (orchestrator sets at the gate).** `TERMII_API_KEY`, `TERMII_BASE_URL` (`https://v3.api.termii.com`), `TERMII_SENDER_ID` (`Mingla`), optional `TERMII_WEBHOOK_SECRET`, and `SMS_LIVE_ENABLED_NG` (flip to `true` to go live).
