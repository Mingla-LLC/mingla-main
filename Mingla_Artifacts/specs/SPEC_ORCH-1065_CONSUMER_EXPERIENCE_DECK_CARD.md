...
> **AMENDMENT (operator-approved, Seth, 2026-06-03) — EXPERIENCES FRONT-LOAD, NOT INTERLEAVE.** The round-robin even-spread above was superseded by operator directive: eligible brand experiences now lead the deck — they are placed at the FRONT (index 0..n-1, ahead of the AI curated cards and singles) in stable RPC order (`pg_eligible_experiences_for_deck` returns `ORDER BY next_start_at ASC NULLS LAST, published_at DESC` → soonest upcoming first, then most recently published). The normal place/curated/singles deck follows. The helper now returns `[...expToPlace, ...placeCards]`. Every other guard is unchanged (dedupe, exclude-self/already-seen, additive — no place card dropped, empty-pool experiences-only path, COMMS-0018 bypass, solo-deck-only). Rationale: make the feature easy to spot and test. This is an INTENTIONAL ordering change — do NOT treat experiences-at-front as a regression. The two position-dependent adversarial tests were updated under `[TEST-MOD-APPROVED ORCH-1065]`; fails-on-revert re-proven for the front-load contract.
