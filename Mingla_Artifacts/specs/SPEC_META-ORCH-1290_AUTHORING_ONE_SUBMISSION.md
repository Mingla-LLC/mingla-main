...
- **I-PROPOSED-1290-PITCH-WRITES-VIA-PIPELINE-ACTION (DRAFT — B2 addendum).** The listing/deck-readiness pitch edit is persisted ONLY through the `update_pitch` pipeline action (a USER action on `run-business-place-authoring-pipeline`, owner-authed via requireUser → loadOwnedBrand → loadOwnedVenue), NEVER a direct client `place_pool` RLS UPDATE. The write is column-scoped to the pitch: apply mode (verified/create-owned) writes `place_pool.generative_summary`; stage mode (pre-approval claim) writes ONLY `business_authoring_inputs.tier1.description` — never a serving/scoring column (chains I-1263-NO-LIVE-PLACE-MUTATION-PRE-APPROVE). apply-vs-stage is decided SERVER-SIDE via `placeWriteMode`, so a client can never force a live write. Resolves blocker B-2 (Leg B's RLS-gated owner UPDATE let an owner set any place_pool column via PostgREST). Gate = `.github/scripts/strict-grep/i-proposed-1290-pitch-via-pipeline.mjs` (in the `meta-orch-1290-venue-authoring-one-submission` job) + the deno suite `meta_orch_1290_b2_update_pitch.test.ts` + jest `updateVenuePitch.b2.test.ts`.
