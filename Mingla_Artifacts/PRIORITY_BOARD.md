...
| ID | Title | Score | Severity | Status |
|----|-------|-------|----------|--------|
| ORCH-0481 | Admin RPC materialized view layer — 22 vulnerable admin RPCs systemic fix | **92** | S1 | **Tester cycle 1 FAIL** — P0 cron refresh broken (6/6 runs timing out, MV never auto-refreshed since 2026-04-17 18:45). Cycle 2 rework dispatched (prompts/IMPL_ORCH-0481_REWORK_V3.md). Surgical fix: wrap refresh in plpgsql fn with `SET LOCAL statement_timeout='15min'` + `ANALYZE`. |
...
**Immediate (ORCH-0481 cycle 2 — cron fix):**
1. User dispatches rework prompt: `prompts/IMPL_ORCH-0481_REWORK_V3.md` to implementor.
2. Implementor writes a new migration: creates `cron_refresh_admin_place_pool_mv()` plpgsql fn with `SET LOCAL statement_timeout='15min'` + `REFRESH MATERIALIZED VIEW CONCURRENTLY` + `ANALYZE admin_place_pool_mv`. Re-schedules cron to call that function. ~15 lines of SQL.
3. User applies cycle 2 migration: `supabase db push`. Wait 11 minutes for cron tick.
4. User re-dispatches tester (reuse prompt) — cycle 2 retest focus: verify cron succeeds (SC-28); re-time the 4 SCs from cycle 1 (expect improvements from fresh stats via ANALYZE); spot-check 2-3 previously-unverified SCs.
5. On cycle 2 PASS → Post-PASS protocol → apply to **production** → admin dashboard unblocked. ORCH-0488 (cache-sensitivity) evaluated only after prod measurement.

Rework cycle count so far: 2 (not yet stuck-in-loop; cycle 3+ would trigger escalation).
