...
> Last updated: 2026-04-18 (env-clarification: Mingla-dev IS prod. ORCH-0480 closed A, ORCH-0481 partially shipped D, ORCH-0489 queued for final polish)
...
| ID | Title | Score | Severity | Status |
|----|-------|-------|----------|--------|
| ORCH-0489 | Admin-UI "Refresh Stats" button — replaces failed pg_cron with user-controlled refresh | **65** | S1 | **Ready to dispatch** (prompts/IMPL_ORCH-0489_ADMIN_REFRESH_BUTTON.md). Single-file scope in PlacePoolManagementPage.jsx. Closes the last ORCH-0481 gap; converts silent cron failure into visible user-controlled refresh. |
...
**Dispatch ORCH-0489 implementor** — `prompts/IMPL_ORCH-0489_ADMIN_REFRESH_BUTTON.md`. Single-file change in `mingla-admin/src/pages/PlacePoolManagementPage.jsx`, ~30-60 lines inline. Calls existing `admin_refresh_place_pool_mv()` RPC. Button replaces the silently-failing pg_cron with a user-controlled, visible refresh. Closes the last ORCH-0481 gap.

**Post-clarification context:** Mingla-dev is production (single-env setup). All ORCH-0480 + ORCH-0481 migrations already live in prod. Admin page loads and is fast. Only remaining gap is auto-refresh, which ORCH-0489 solves.

On ORCH-0489 close: ORCH-0481 promotes D→A, full admin dashboard unblock complete.
