...
> Last updated: 2026-04-17 (post ORCH-0481 tester FAIL cycle 0 — rework dispatched)
...
| ID | Title | Score | Severity | Status |
|----|-------|-------|----------|--------|
| ORCH-0481 | Admin RPC materialized view layer — 22 vulnerable admin RPCs systemic fix | **90** | S1 | **Tester FAIL cycle 0** — rework dispatched (prompts/IMPL_ORCH-0481_REWORK_V2.md). Surgical: COUNT(mv.*)→COUNT(mv.id) fixes P0-1 (54s→<500ms expected); P0-2 needs design call. Category_breakdown already 107ms (84× win). |
...
**Immediate (ORCH-0481 rework cycle 1 — tester cycle 0 FAIL):**
1. User dispatches rework prompt: `prompts/IMPL_ORCH-0481_REWORK_V2.md` to implementor.
2. Implementor fixes P0-1 (surgical `COUNT(mv.*)` → `COUNT(mv.id)` in 2 functions — expected ~10 lines edited) and makes design call on P0-2 (multi-subquery split vs covering index for `admin_place_pool_overview` global path).
3. User applies rework migration (via `supabase db push` — expect tiny migration on top of existing MV).
4. User re-dispatches tester (reuse `prompts/TESTER_ORCH-0481_ADMIN_MV_LAYER.md`) — focus on P0-1 / P0-2 retimings + the SCs left UNVERIFIED in cycle 0.
5. On retest PASS → Post-PASS protocol → apply to production → ORCH-0480 closes as "superseded", ORCH-0482 closes (subsumed).

The MV + indexes + cron from cycle 0 stay in place — only RPC bodies need rework.
