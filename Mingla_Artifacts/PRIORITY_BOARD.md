...
> Last updated: 2026-04-17 (post ORCH-0481 implementor rework cycle 1 — awaiting tester retest)
...
| ID | Title | Score | Severity | Status |
|----|-------|-------|----------|--------|
| ORCH-0481 | Admin RPC materialized view layer — 22 vulnerable admin RPCs systemic fix | **90** | S1 | **Rework cycle 1 delivered 2026-04-17.** Self-EXPLAIN: country_overview 113ms (475× win), pool_overview global ~500ms (10× win), city_overview 3,061ms (partial, under 8s timeout). Awaiting `supabase db push` + tester cycle 1. |
...
**Immediate (ORCH-0481 rework verification — cycle 1 in flight):**
1. User applies rework migration: `supabase db push` — one pending file (`20260418000002_orch0481_rework_count_fix.sql`).
2. User re-dispatches tester (reuse `prompts/TESTER_ORCH-0481_ADMIN_MV_LAYER.md`) — focus on retiming country_overview / city_overview / pool_overview (expect 113ms / ~3s / ~500ms) + verifying SCs left UNVERIFIED in cycle 0.
3. On PASS (or CONDITIONAL PASS accepting 3s city_overview) → Post-PASS protocol → apply to **production** via `supabase db push` → admin dashboard unblocked on prod.
4. After close: ORCH-0480 marked superseded; ORCH-0482 closes (subsumed); ORCH-0487 queued for city_overview <500ms follow-up (covering-index migration).

No MV/index/cron changes in cycle 1 migration — only 3 function bodies rewritten.
