...
> Last updated: 2026-04-17 (post ORCH-0481 tester FAIL cycle 0 — rework cycle 1 dispatched)
> Total: 347 | Open: 149 | Closed: 122 | Verified (B grade): 24 | Partial: 1 | Deferred: 1 | FAIL-superseded: 1 | Tester-fail-cycle-0: 1
>
> In flight (2026-04-17 late): **ORCH-0481** — admin RPC materialized view layer. **Cycle 0 FAIL** per `reports/QA_ORCH-0481_ADMIN_MV_LAYER_REPORT.md`. Migration applied to Mingla-dev (commit 352fc4c7) but tester surfaced 2 P0: (P0-1) `admin_place_country_overview` = 53,875ms warm — **6× WORSE** than ORCH-0480 due to `COUNT(mv.*)` anti-pattern forcing 624-byte row materialization + 318MB disk sort. (P0-2) `admin_place_pool_overview` global = 4,932ms (under 8s timeout so page loads but 25× over <200ms target) due to full seq scan of wide MV on slow Mingla-dev I/O. Big win: `admin_place_category_breakdown` at **107ms** (84× faster than ORCH-0480) — **proves MV architecture works**; the 2 failures are implementation defects not architectural. Rework cycle 1 dispatched: `prompts/IMPL_ORCH-0481_REWORK_V2.md` — surgical `COUNT(mv.*)` → `COUNT(mv.id)` substitution fixes P0-1 (expected <500ms). P0-2 needs design call (multi-subquery split vs covering index). **NOT applied to production.**
