...
> Last updated: 2026-04-17 (post ORCH-0481 tester cycle 1 FAIL — cycle 2 cron-fix rework dispatched)
> Total: 349 | Open: 151 | Closed: 122 | Verified (B grade): 24 | Partial: 1 | Deferred: 1 | FAIL-superseded: 1 | Tester-fail-cycle-1: 1
>
> In flight (2026-04-17 late): **ORCH-0481** — admin RPC materialized view layer. **Tester cycle 1 retest FAIL** per `reports/QA_ORCH-0481_ADMIN_MV_LAYER_REPORT_RETEST_1.md` (1 P0 / 2 P1 / 2 P2 / 2 P4). **NEW P0:** pg_cron refresh fails on every run (6/6 at 2-min statement timeout since 2026-04-17 18:45). MV never auto-refreshed. Constitutional #3 violation. **NEW P1-1:** cache-sensitive perf on Mingla-dev (country_overview 87ms hot / 5s cold) — registered as **ORCH-0488** for prod-side measurement. **NEW P1-2 regression:** category_breakdown 107ms→4,219ms (unchanged body; plan drift + cold cache). Wins confirmed: country_overview restructure works; pool_overview Option A works; city_overview 3s (ORCH-0487 still tracks). Cycle 2 surgical rework dispatched: `prompts/IMPL_ORCH-0481_REWORK_V3.md` — cron fn wrapper with `SET LOCAL statement_timeout='15min'` + `ANALYZE` (addresses P0-1 AND P1-2). Cycle count 2 (not yet stuck-in-loop). **NOT applied to production.**
