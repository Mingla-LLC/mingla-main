...
> Last updated: 2026-04-17 (post ORCH-0481 implementor APPROVED structurally; tester + deploy pending)
> Total: 347 | Open: 150 | Closed: 122 | Verified (B grade): 23 | Partial: 1 | Deferred: 1 | FAIL-superseded: 1 | Implemented-awaiting-verify: 1
>
> In flight (2026-04-17 late): **ORCH-0481** — admin RPC materialized view layer. **IMPLEMENTATION DELIVERED.** Migration `supabase/migrations/20260418000001_orch0481_admin_mv_layer.sql` (1080 lines): `admin_place_pool_mv` (27 cols, 5 indexes) + pg_cron 10-min refresh + `admin_refresh_place_pool_mv()` + 20 of 22 admin RPCs rewritten (2 deferred with rationale). Report: `reports/IMPLEMENTATION_ORCH-0481_ADMIN_MV_LAYER_REPORT.md`. Structural review APPROVED — signatures byte-identical, auth gates preserved, rollback documented. Awaiting user `supabase db push` + tester verify (`prompts/TESTER_ORCH-0481_ADMIN_MV_LAYER.md`). Expected: admin pages <200ms regardless of pool size. ORCH-0480 migration stays on Mingla-dev only (commit 82d94aef) as no-harm stepping stone per DEC-021.
> Recently closed (2026-04-17): **ORCH-0469** (brunch/lunch/casual cache poisoning + EMPTY vs EXHAUSTED split — QA retest cycle #1 PASS 6/6, Grade B, I-EMPTY-CACHE-NONPERSIST invariant established), **ORCH-0472** (jointly closed with ORCH-0469), **ORCH-0460** (place pipeline accuracy overhaul — QA retest PASS 11/11 SC), ORCH-0461 (casual_eats 50-type split), ORCH-0462 (Upscale & Fine Dining expansion), ORCH-0463 (garden store flowers leak — FLOWERS_BLOCKED_TYPES split primary/secondary, live SQL: 180→11 strips, 139 supermarkets preserved), ORCH-0464 (venue name keyword sync), ORCH-0465 (brunch-restaurants-only), ORCH-0471 (mutual exclusivity upscale↔casual REMOVED — supersedes ORCH-0428), ORCH-0477 (invariant #13 drift — constitutional #13 restored FAIL→PASS)
...
> Recently added: **ORCH-0485** (deck cards take >1s after pref change — S1 perceived-latency perf, investigator queued), **ORCH-0480** (admin Place Pool RPC timeouts — S1, implementation delivered), **ORCH-0481** (admin RPC MV layer — S1, systemic fix ready to dispatch), **ORCH-0482** (admin analytics RPCs will fail at scale — S2, pre-emptive), **ORCH-0483** (admin RPC perf gate — S2 process rule), **ORCH-0484** (776 approved places with NULL ai_categories — S2 data integrity, discovered via ORCH-0480 D-2), **ORCH-0474** (discover-cards silently degrades auth/pipeline failures to "pool-empty" — S1 architecture-flaw; investigation + spec + implementation COMPLETED 2026-04-17; structurally APPROVED; awaiting edge-fn deploy + tester dispatch; establishes INV-042 + INV-043), **ORCH-0475** (filter-outline icon name unknown to app — S3, side effect of ORCH-0472 UI split), ORCH-0476 (category-mapping.md stale — doc debt), ORCH-0478 (pre-flight pool-impact dry-run — S2, high-value guardrail), ORCH-0479 (TopGolf non-idempotent classification — S3), ORCH-0470 (generate-single-cards seeding-ID vs app-slug mismatch — awaiting spec dispatch), ORCH-0473 (dead-code TS errors in RecommendationsContext — implementor discovery, not urgent), ORCH-0466 (admin-seed-places create_run 500 regression — fixed, pending smoke), ORCH-0467 (edge-function deploy pipeline lacks type-check gate), ORCH-0468 (admin-seed-places duplicated batch logic — tech debt)
