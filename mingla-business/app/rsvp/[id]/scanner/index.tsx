/**
 * /rsvp/[id]/scanner — Issue #1447 RSVP guest camera scanner.
 *
 * Operator-side route. Camera permission gate → CameraView with QR-only
 * barcode scanner → server-authoritative typed RSVP result overlay → session
 * activity log. Offline/network failure is fail-closed and never queued.
 *
...
import {
  ScanRsvpError,
  fetchRsvpCheckinSummary,
  scanRsvp,
  type RsvpScanResult,
} from "../../../../src/services/scanRsvpService";
...
const overlaySpec = (kind: RsvpScanResult): ResultOverlaySpec => {
  switch (kind) {
    case "success":
      return {
        iconName: "check",
    ...
    // Warning tone for recoverable door-window outcomes.
    case "not_yet_open":
    case "event_ended":
      return {
        iconName: "flag",
...
export default function ScannerCameraRoute(): React.ReactElement {
  ...
  const summary = useQuery({
    queryKey: ["rsvpCheckinSummary", event?.id ?? eventId],
    queryFn: () => fetchRsvpCheckinSummary(event?.id ?? String(eventId)),
    enabled: event !== null,
    staleTime: 10_000,
  });
  useEffect(() => {
    if (event !== null) postHogService.capture("rsvp_scanner_opened", { event_id: event.id });
  }, [event]);
  useEffect(() => {
  ...
  const sessionScanTotal = sessionScans.length;
  ...
  }, [dismissOverlay, summary.refetch]);
  ...
  const showResult = useCallback(
    (next: ResultOverlayState): void => {
      ...
      AccessibilityInfo.announceForAccessibility(
        [next.message, next.detail].filter(Boolean).join(". "),
      );
      if (reduceMotionRef.current) {
  ...
  const handleBarcodeScanned = useCallback(
    (scan: BarcodeScanningResult): void => {
      ...
      AccessibilityInfo.announceForAccessibility("Checking pass");
      const scanGeneration = ++scanGenerationRef.current;
      void (async () => {
        const startedAt = Date.now();
        postHogService.capture("rsvp_scan_attempted", {
          event_id: event.id,
          platform: Platform.OS,
        });
        try {
          const result = await scanRsvp(event.id, scan.data);
          if (
            scanGeneration !== scanGenerationRef.current ||
            !isAppActiveRef.current
          ) return;
          const kind = result.result;
          postHogService.capture("rsvp_scan_result", {
            event_id: event.id, outcome: kind, platform: Platform.OS,
            latency_ms: Date.now() - startedAt,
          });
          setSessionScans((current) => [{
            id: result.scanId ?? `${Date.now()}`,
            result: kind,
            guestName: result.attendeeDisplayName ?? "Guest",
            role: result.entityType ?? "primary",
          ...
          if (kind === "success") {
            ...
            showResult({
              kind,
              message: `${result.attendeeDisplayName ?? "Guest"} checked in`,
              detail: result.entityType === "guest" ? "Plus-one" : "Primary guest",
            });
          ...
          let message: string;
          let detail: string | undefined = result.attendeeDisplayName ?? undefined;
          if (kind === "duplicate") {
            message = "Already checked in";
          } else if (kind === "wrong_event") {
            message = "Different RSVP event";
          } else if (kind === "not_eligible") {
            message = "RSVP not approved";
          } else if (kind === "revoked") {
            message = "RSVP no longer valid";
            ...
            message = result.lastEndAt
              ...
              : "Event has ended";
            detail = "RSVP passes can't be used after the event";
          } else {
            message = "RSVP invite not found";
          }
          ...
          postHogService.capture("rsvp_scan_result", {
            event_id: event.id,
            outcome: error instanceof ScanRsvpError ? error.code : "network_error",
            platform: Platform.OS,
            latency_ms: Date.now() - startedAt,
          });
          // ORCH-0795: surface the real edge-function failure reason.
          // The typed edge error distinguishes authorization from a generic failure.
          let message = "Can't verify this pass";
          let detail: string | undefined =
            "You're offline. Reconnect and scan again.";
          if (error instanceof ScanRsvpError) {
            if (error.code === "scanner_not_authorized") {
              message = "You're not authorized to scan this RSVP";
              detail = "Ask the organiser to add you as a scanner.";
      ...
      <View style={[styles.host, { paddingTop: insets.top }]}>
        ...
        <View style={styles.permWrap}>
          ...
          <Text style={styles.permCopy}>
            {askable
              ? "Camera access needed to scan RSVP invites at the door."
    ...
    <View style={[styles.host, { paddingTop: insets.top }]}>
      <View style={styles.chromeRow}>
        ...
        <View style={styles.chromeTitleBlock}>
          <Text style={[styles.chromeTitle, styles.chromeTitleNested]}>Scan RSVP guests</Text>
          <Text style={styles.chromeCounter} numberOfLines={1}>
            {event.name} · Checked in {summary.data?.checkedIn ?? "—"} / {summary.data?.going ?? "—"}
          </Text>
        </View>
        <View style={styles.chromeRightSlot} />
      </View>
      ...
      <View style={styles.cameraWrap}>
        <CameraView
          ...
          onBarcodeScanned={isAppActive && !isProcessing
        ...
        <View style={styles.reticleWrap} pointerEvents="none">
          ...
          <Text style={styles.reticleHelper}>
            Point camera at RSVP invite QR code
          </Text>
        </View>

        {isProcessing ? (
          <View
            style={styles.processingHost}
            pointerEvents="none"
            accessibilityLiveRegion="assertive"
          >
            <ActivityIndicator color="#ffffff" />
            <Text style={styles.processingText}>Checking pass…</Text>
          </View>
        ) : null}
          ...
          sessionScans.length === 0 ? (
            <View style={styles.logEmpty}>
              <Text style={styles.logEmptyText}>
                No scans yet. Point the camera at an RSVP invite QR code.
              </Text>
            </View>
          ) : (
            <ScrollView
...
const SESSION_RESULT_LABEL: Record<RsvpScanResult, string> = {
  success: "IN",
  duplicate: "DUPE",
  wrong_event: "WRONG",
  not_found: "404",
...
const SESSION_RESULT_ICON: Record<RsvpScanResult, "check" | "flag" | "close"> = {
  success: "check",
  duplicate: "flag",
  wrong_event: "close",
  not_found: "close",
...
const SessionLogRow: React.FC<SessionLogRowProps> = ({ scan }) => {
  const spec = overlaySpec(scan.result);
  const iconName = SESSION_RESULT_ICON[scan.result];
  return (
    <View style={styles.logRow}>
      ...
      <View style={styles.logRowCol}>
        ...
        <Text style={styles.logRowSubline} numberOfLines={1}>
          {scan.role === "guest" ? "Plus-one" : "Primary guest"}
        </Text>
      </View>
      <View style={styles.logRowRight}>
        ...
        <View style={styles.logRowBadge}>
          <Text style={styles.logRowBadgeText}>
            {SESSION_RESULT_LABEL[scan.result]}
          </Text>
        </View>
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  chromeTitleNested: { flex: 0 },
  chromeTitleBlock: { flex: 1, alignItems: "center" },
  chromeCounter: { color: textTokens.secondary, fontSize: 11, marginTop: 2 },
  chromeRightSlot: {
    width: 36,
  },
  emptyHost: {
