// ORCH-1335 RsvpStep5Setup chip-in bank-banner render-proof — worktree-local
// jest config.
//
// Stands up a REAL @testing-library/react-native mount of the production
// RsvpStep5Setup so the payout-aware callout SWAP (neutral "Connect your bank"
// nudge ↔ positive "Payouts are on") is proven to RENDER at runtime (not just
// source-grepped), including the adversarial `chipInPayoutReady === undefined`
// (loading) case that must fall to the nudge with no false-positive flash.
//
// Mirrors jest.orch1143.render.cjs (RN preset + babel-jest of the RN tree) and
// resolves RTL + react-test-renderer from the worktree-local .orch1118-testdeps
// overlay (gitignored; provisioned per worktree), with react/react-native pinned
// to the business install (single copy).
//
// Run:
//   npx jest --config jest.orch1335.render.cjs --runInBand
...
module.exports = {
  ...
  testMatch: [
    "**/__tests__/RsvpStep5Setup.orch1335.render.test.tsx",
  ],
