// #426 G1 — experience-checkout-bundle cache contract (mirrors issue #2879 tests).
const test = require("node:test");
const assert = require("node:assert/strict");
const Module = require("node:module");

const HANDLER = "../experience-checkout-bundle.js";
...
test("exp B-01 — valid slugs return the RPC payload", async () => {
  const handler = load(async () => ({ id: "exp-1", experienceSlug: "wine-crawl" }));
  const res = mockRes();
  await handler(
    { query: { brandSlug: "lantern-vine", experienceSlug: "wine-crawl" } },
    res,
  );
...
test("exp B-02 — hit is cached s-maxage=5 without SWR", async () => {
  const handler = load(async () => ({ id: "exp-1" }));
  const res = mockRes();
  await handler(
    { query: { brandSlug: "lantern-vine", experienceSlug: "wine-crawl" } },
    res,
...
test("exp B-03 — not-found cached; failure never cached", async () => {
...
test("exp B-04 — RPC args use p_experience_slug", async () => {
  let seen = null;
...
test("exp B-05 — malformed input never reaches the RPC", async () => {
  let called = 0;
  ...
  const cases = [
    ...
    { brandSlug: "a/b", experienceSlug: "c" },
  ];
