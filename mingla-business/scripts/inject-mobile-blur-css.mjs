...
function buildRouteDeferralLoader(scripts) {
  return `<script id="${SCRIPT_DEFERRAL_MARKER}">(function(){var scripts=${JSON.stringify(scripts)};function isPhone(){try{return matchMedia("(max-width: 767px), (pointer: coarse)").matches||/Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent)}catch(_e){return false}}var sbUrl=${JSON.stringify(SUPABASE_URL)};var sbAnon=${JSON.stringify(SUPABASE_ANON_KEY)};function routeStatus(path){var map={"/":"interactive","/auth":"interactive","/auth/callback":"interactive","/hub/events":"interactive","/marketing":"interactive","/marketing/campaigns/compose":"interactive","/account":"interactive","/hub/trips":"interactive","/event/create":"interactive","/hub/experiences":"blocked","/ari":"blocked","/connect-account-management":"blocked"};return map[path]||"static-section"}function isLightRoute(path){return path==="/hub/events"||path==="/hub/trips"||path==="/marketing"||path==="/marketing/campaigns/compose"||path==="/account"}function esc(value){return String(value==null?"":value).replace(/[&<>"']/g,function(ch){return ch==="&"?"&amp;":ch==="<"?"&lt;":ch===">"?"&gt;":ch==='"'?"&quot;":"&#39;"})}function page(title,body){document.documentElement.style.background="#090b0f";document.body.style.margin="0";document.body.style.minHeight="100vh";document.body.style.background="#090b0f";document.body.innerHTML='<main data-orch-1095-light-route-entry="true" style="box-sizing:border-box;min-height:100vh;padding:18px;background:#090b0f;color:#fff;font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif"><section style="max-width:520px;margin:0 auto"><header style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin:2px 0 18px"><a href="/home" style="color:#eb7825;font-size:13px;font-weight:800;text-decoration:none">Mingla Business</a><a href="/home" style="color:rgba(255,255,255,.72);font-size:13px;text-decoration:none">Home</a></header><h1 style="margin:0 0 14px;font-size:30px;line-height:1.08;letter-spacing:0">'+esc(title)+'</h1>'+body+'</section></main>'}function card(html){return '<section style="border:1px solid rgba(255,255,255,.14);border-radius:16px;background:rgba(255,255,255,.075);padding:18px;margin:12px 0">'+html+'</section>'}function btn(href,label,primary){return '<a href="'+href+'" style="display:flex;min-height:46px;align-items:center;justify-content:center;border-radius:12px;margin-top:12px;background:'+(primary?'#eb7825':'rgba(255,255,255,.1)')+';border:1px solid '+(primary?'#eb7825':'rgba(255,255,255,.14)')+';color:'+(primary?'#111':'#fff')+';text-decoration:none;font-weight:800">'+esc(label)+'</a>'}function getSession(){try{for(var i=0;i<localStorage.length;i++){var key=localStorage.key(i);if(!/^sb-.+-auth-token$/.test(key||""))continue;var raw=localStorage.getItem(key);if(!raw)continue;var parsed=JSON.parse(raw);var session=parsed&&parsed.access_token?parsed:parsed&&parsed.currentSession&&parsed.currentSession.access_token?parsed.currentSession:null;if(session&&session.access_token)return session}}catch(_e){}return null}function currentBrandId(){try{var raw=localStorage.getItem("mingla-business.currentBrand.v14");var parsed=raw?JSON.parse(raw):null;return parsed&&parsed.state&&typeof parsed.state.currentBrandId==="string"?parsed.state.currentBrandId:null}catch(_e){return null}}function rest(path,params,token){var url=sbUrl+"/rest/v1/"+path+(params?"?"+params:"");return fetch(url,{headers:{apikey:sbAnon,Authorization:"Bearer "+token,Accept:"application/json"}}).then(function(r){if(!r.ok)throw new Error("HTTP "+r.status);return r.json()})}function fmtDate(value){if(!value)return "";try{return new Intl.DateTimeFormat(undefined,{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"}).format(new Date(value))}catch(_e){return ""}}function item(title,meta,href){return card('<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px"><div><h2 style="margin:0 0 6px;font-size:17px;line-height:1.2">'+esc(title)+'</h2><p style="margin:0;color:rgba(255,255,255,.68);font-size:13px;line-height:1.35">'+esc(meta)+'</p></div><span style="color:#eb7825;font-size:22px">›</span></div>').replace("<section",'<a href="'+href+'" style="display:block;color:inherit;text-decoration:none"').replace("</section>","</a>")}function signedOut(path){var labels={"/hub/events":"Hub Events","/hub/trips":"Hub Trips","/marketing":"Marketing overview","/marketing/campaigns/compose":"Compose blast","/account":"Account settings"};page("Sign in to open "+(labels[path]||"this route")+".",card('<p style="margin:0;color:rgba(255,255,255,.72);line-height:1.45">This phone-browser route is ready, but it needs a business session before it can load your brand data.</p>'+btn("/auth","Sign in",true)+btn("/home","Return to Home",false)))}function renderRecovery(status){page("This route is staying protected.",card('<p style="margin:0;color:rgba(255,255,255,.72);line-height:1.45">'+(status==="static-section"?"This phone-browser route has not been promoted to direct interactive entry yet, so Mingla is keeping you on the stable Home launcher.":"This phone-browser route is not ready for direct entry yet, so Mingla is sending you back to the stable Home launcher.")+'</p>'+btn("/home","Return to Home",true)))}function renderRoute(path,session){var email=session.user&&session.user.email?session.user.email:"signed-in";page("Loading "+path.replace(/^\\//,"")+"...",card('<p style="margin:0;color:rgba(255,255,255,.72)">Signed in as '+esc(email)+'. Loading your brands without the full app bundle.</p>'));var uid=session.user&&session.user.id;var brandParams="select=id,name,slug,default_currency,created_at&account_id=eq."+encodeURIComponent(uid||"")+"&deleted_at=is.null&order=created_at.desc";rest("brands",brandParams,session.access_token).then(function(brands){brands=Array.isArray(brands)?brands:[];var chosen=brands.find(function(b){return b.id===currentBrandId()})||brands[0]||null;if(!chosen){page("Create a brand to continue.",card('<p style="margin:0;color:rgba(255,255,255,.72);line-height:1.45">Your account is signed in, but there is no business brand to show on this route yet.</p>'+btn("/home","Open Home",true)));return}if(path==="/account"){var rows=brands.map(function(b){return item(b.name||"Untitled brand",(b.slug?"/b/"+b.slug:"Brand")+" · "+(b.default_currency||"USD"),"/home#account")}).join("");page("Account settings",card('<p style="margin:0;color:rgba(255,255,255,.72)">Signed in as '+esc(email)+'</p><p style="margin:8px 0 0;color:rgba(255,255,255,.72)">Current brand: <strong style="color:#fff">'+esc(chosen.name)+'</strong></p>'+btn("/home#account","Edit profile",false)+btn("/home#account","More account tools",true))+rows);return}if(path==="/marketing"){var since=new Date(Date.now()-30*86400000).toISOString();Promise.all([rest("marketing_campaigns","select=id,status,sent_at,created_at&account_id=eq."+encodeURIComponent(uid||"")+"&created_at=gte."+encodeURIComponent(since),session.access_token),rest("marketing_campaigns","select=id,name,status,sent_at,scheduled_for,recipient_count,created_at&account_id=eq."+encodeURIComponent(uid||"")+"&order=sent_at.desc.nullslast&order=created_at.desc&limit=3",session.access_token)]).then(function(results){var windowRows=Array.isArray(results[0])?results[0]:[];var recent=Array.isArray(results[1])?results[1]:[];var sent=windowRows.filter(function(c){return c.status==="sent"&&c.sent_at}).length;var recentRows=recent.map(function(c){return item(c.name||"Untitled campaign",(c.status||"draft")+" · "+(c.sent_at?fmtDate(c.sent_at):c.scheduled_for?fmtDate(c.scheduled_for):"not sent"),"/marketing")}).join("");page("Marketing",card('<p style="margin:0 0 6px;color:rgba(255,255,255,.72)">Campaigns sent in the last 30 days</p><strong style="font-size:38px;line-height:1">'+sent+'</strong>'+btn("/marketing/campaigns/compose","New campaign",true))+(recentRows||card('<h2 style="margin:0 0 6px;font-size:17px">No campaigns yet</h2><p style="margin:0;color:rgba(255,255,255,.72)">Your first blast is one tap away.</p>')))}).catch(function(){page("Marketing",card('<p style="margin:0;color:rgba(255,255,255,.72)">Could not load metrics right now.</p>'+btn("/marketing/campaigns/compose","New campaign",true)))});return}if(path==="/marketing/campaigns/compose"){page("Compose blast",card('<label style="display:block;margin:0 0 6px;color:rgba(255,255,255,.72);font-size:13px">Subject</label><input aria-label="Subject" placeholder="What should buyers know?" style="box-sizing:border-box;width:100%;min-height:46px;border-radius:12px;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.08);color:#fff;padding:0 12px;font:inherit"><label style="display:block;margin:14px 0 6px;color:rgba(255,255,255,.72);font-size:13px">Message</label><textarea aria-label="Message" placeholder="Write your blast..." style="box-sizing:border-box;width:100%;min-height:150px;border-radius:12px;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.08);color:#fff;padding:12px;font:inherit"></textarea>'+btn("/marketing","Return to marketing",true)));return}if(path==="/hub/trips"){rest("events","select=id,title,status,slug,created_at,published_at,event_type&brand_id=eq."+encodeURIComponent(chosen.id)+"&event_type=eq.trip&deleted_at=is.null&order=created_at.desc&limit=12",session.access_token).then(function(rows){rows=Array.isArray(rows)?rows:[];var list=rows.map(function(t){return item(t.title||"Untitled trip",(t.status||"draft")+" · "+(t.published_at?fmtDate(t.published_at):fmtDate(t.created_at)),"/home#hub-trips")}).join("");page("Hub Trips",card('<p style="margin:0;color:rgba(255,255,255,.72)">Brand: <strong style="color:#fff">'+esc(chosen.name)+'</strong></p>'+btn("/home#hub-trips","Open trip tools",true))+(list||card('<h2 style="margin:0 0 6px;font-size:17px">No trips yet</h2><p style="margin:0;color:rgba(255,255,255,.72)">Select a brand to see its trips.</p>')))}).catch(function(){page("Hub Trips",card('<p style="margin:0;color:rgba(255,255,255,.72)">Could not load trips right now.</p>'+btn("/home#hub-trips","Return to Hub",true)))});return}rest("business_management_events_view","select=id,title,status,slug,published_at,created_at,master_start_at&brand_id=eq."+encodeURIComponent(chosen.id)+"&order=published_at.desc.nullslast&limit=12",session.access_token).then(function(rows){rows=Array.isArray(rows)?rows:[];var ids=rows.map(function(r){return r.id});var finish=function(events){var list=events.map(function(e){return item(e.title||"Untitled event",(e.status||"scheduled")+" · "+(e.master_start_at?fmtDate(e.master_start_at):e.published_at?fmtDate(e.published_at):fmtDate(e.created_at)),"/home#hub-events")}).join("");page("Hub Events",card('<p style="margin:0;color:rgba(255,255,255,.72)">Brand: <strong style="color:#fff">'+esc(chosen.name)+'</strong></p>'+btn("/event/create","Build a new event",true))+(list||card('<h2 style="margin:0 0 6px;font-size:17px">Nothing created yet</h2><p style="margin:0;color:rgba(255,255,255,.72)">Build a new event when you are ready.</p>')))};if(ids.length===0){finish([]);return}rest("events","select=id,event_type&id=in.("+ids.map(encodeURIComponent).join(",")+")",session.access_token).then(function(types){var tripIds=new Set((Array.isArray(types)?types:[]).filter(function(t){return t.event_type==="trip"}).map(function(t){return t.id}));finish(rows.filter(function(r){return !tripIds.has(r.id)}))}).catch(function(){finish(rows)})}).catch(function(){page("Hub Events",card('<p style="margin:0;color:rgba(255,255,255,.72)">Could not load events right now.</p>'+btn("/event/create","Build a new event",true)))});}).catch(function(){page("Could not load this route.",card('<p style="margin:0;color:rgba(255,255,255,.72)">Your session is present, but Mingla could not load your business data in this lightweight route.</p>'+btn("/home","Return to Home",true)))})}function loadAt(i){if(i>=scripts.length)return;var s=document.createElement("script");s.defer=true;s.src=scripts[i];s.setAttribute("data-${JS_CACHE_BUST_MARKER}","true");s.onload=function(){loadAt(i+1)};s.onerror=function(){try{sessionStorage.setItem("mingla-mobile-web-chunk-recovery",String(Date.now()))}catch(_e){} location.replace("/home?recovered=chunk")};document.body.appendChild(s)}var path=location.pathname.replace(/\\/$/,"")||"/";var status=routeStatus(path);if(isPhone()&&status!=="interactive"){renderRecovery(status);return}if(isPhone()&&status==="interactive"&&isLightRoute(path)){var session=getSession();if(!session){signedOut(path);return}renderRoute(path,session);return}loadAt(0);})();</script>`;
}
