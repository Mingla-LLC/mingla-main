...
// ─── issue #2009 — published ticketed-event visibility ────────────────
 ...
 *
 * There is deliberately NO `.from("events").update({ visibility })` path here
 * or anywhere else: the RPC owns authorization (an authorization-bearing first
 * row lock), value/type/status validation, the same-value no-op, optimistic
 * concurrency, the Private fail-closed refusal, the discovery-generation
 * increment, share revocation cardinality and the single audit row. A direct
 * table UPDATE from an `authenticated` client is refused by the database.
 *
