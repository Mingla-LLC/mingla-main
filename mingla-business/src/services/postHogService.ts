...
//
// PostHog native (posthog-react-native) singleton facade for the business app.
...
//
// KEY READ (COMMS-0028 env reachability): the project key + host come from
// `Constants.expoConfig?.extra` (mirrors supabase.ts / giphyEventCoverService.ts)
// — NEVER a dynamic `process.env[name]` bracket read, which babel-preset-expo
// does NOT inline and is undefined in Hermes standalone/OTA builds. A static
// `process.env.EXPO_PUBLIC_POSTHOG_KEY` is the secondary fallback. No-ops
// gracefully when the key is absent (T-10).
//
// SECRET HYGIENE (I-PROPOSED-1187-NO-PHX-IN-CLIENT): only the public `phc_*`
// PROJECT key ships here. The `phx_*` personal/MCP key MUST NEVER appear here.
//
...
// recordings/mo tier; Seth also sets a $0 PostHog billing cap (SA-1).
//
// WEB SAFETY: NATIVE-only. On the buyer-web export this stays a pure no-op —
// `posthog-react-native` is imported lazily and ONLY when `Platform.OS !== 'web'`.
// confirm.tsx (a shared web+native checkout route) calls this facade; on web it
// is a no-op and the separate web-analytics leg (webAnalytics.web.ts) owns web
// capture. Preserves I-PROPOSED-1187-ANALYTICS-WEB-ONLY-VIA-WEB-TS.
...
function resolveHost(): string {
  const fromEnv =
    readExtra("EXPO_PUBLIC_POSTHOG_HOST") ?? process.env.EXPO_PUBLIC_POSTHOG_HOST;
  return fromEnv && fromEnv.startsWith("http") ? fromEnv : US_HOST;
}
...
class PostHogService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      if (useAnalyticsPrefsStore.getState().analyticsOptOut) {
        client.optOut();
      }
    } catch (error) {
