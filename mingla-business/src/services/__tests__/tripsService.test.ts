...
describe("ORCH-0859 — tripsService.publishTrip", () => {
  test("calls business_publish_trip_draft RPC (NOT business_publish_event_draft)", async () => {
    ...
    // Mock the revision read followed by getTrip's subsequent refresh.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    // [TEST-MOD-APPROVED #1971] The command loads persisted state and delegates
    // to the atomic poster wrapper inside its database transaction.
  ...
  test("raises TripPublishValidationError on RPC error with real Postgrest shape", async () => {
    ...
    fromMock.mockReturnValue(
      createChainableQuery({ data: { updated_at: "2026-05-17T00:00:00Z" } }),
    );
    rpcMock.mockResolvedValueOnce({
      data: null,
