...
describe("visibleTabsForRank", () => {
  ...
  // ORCH-1112 — SANCTIONED behavior change: a brand-less rank-0 user now sees
  ...
  it("surfaces ari for a brand-less rank-0 user (ORCH-1112)", () => {
  ...
  // ORCH-1112 — the carve-out must NOT re-grant Ari to a rank-10 scanner; the
  ...
  it("rank-10 scanner still does NOT see ari after the ORCH-1112 carve-out", () => {
    const visible = visibleTabsForRank(FULL_TABS, BRAND_ROLE_RANK.scanner);
    expect(visible.map((t) => t.id)).toEqual(["home", "account"]);
  });

  // ORCH-1112 — a rank-20 marketing manager sees Home + Blast + Account, NOT
  // ari. The non-monotonic carve-out is scoped to rank 0 and rank>=30 only.
  it("rank-20 marketing manager does NOT see ari (ORCH-1112)", () => {
    const visible = visibleTabsForRank(
      FULL_TABS,
      BRAND_ROLE_RANK.marketing_manager,
    );
