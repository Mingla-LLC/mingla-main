...
type TripMutationWithRevision = TripMutationWithOperation & {
  eventId: string;
  expectedUpdatedAt?: string;
};

/** Mutates the React Query variables object once so automatic retries replay. */
export function ensureTripOperationId<T extends TripMutationWithOperation>(
  input: T,
): string {
...
export async function ensureTripMutationRevision<
  T extends TripMutationWithRevision,
...
export const tripKeys = {
  all: ["trips"] as const,
  lists: (): readonly ["trips", "list"] => [...tripKeys.all, "list"] as const,
  ...
  return useMutation<Trip, Error, UpdateTripBasicsInput>({
    mutationFn: async (input) =>
      updateTripBasics(
        ...
        input.patch,
        ensureTripOperationId(input),
        await ensureTripMutationRevision(input),
      ),
