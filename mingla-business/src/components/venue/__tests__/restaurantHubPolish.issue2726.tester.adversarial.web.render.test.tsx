...
describe("#2726 tester adversarial web rail", () => {
  beforeEach(() => {
    ...
    jest
      ...
      .mockReturnValue(emitter.addListener("reduceMotionChanged", jest.fn()));
  });
  ...
  it("uses one tablist, non-accessible group labels, and one roving tab stop", async () => {
    ...
    expect(
      tree.root.findAll((node) => node.props.accessibilityRole === "tablist"),
    ).toHaveLength(1);
    const tabs = modules.map((module) =>
      byTestId(tree.root, `test-rail-${module.key}`),
    );
    expect(tabs).toHaveLength(4);
    expect(tabs.map((tab) => tab.props.tabIndex)).toEqual([-1, -1, 0, -1]);
    const headings = tree.root
      .findAll((node) => node.props.accessible === false)
      .map((node) => node.props.children)
      .filter((child) => typeof child === "string");
    expect(new Set(headings)).toEqual(
      new Set(["Venue", "Bookings", "Operations"]),
    );
  ...
  it.each([
    ...
    "%s selects the expected tab and supplies a focus-return callback",
    async (key, from, expected) => {
      ...
      TestRenderer.act(() => {
        const handler = byTestId(tree.root, `test-rail-${from}`).props
          .onKeyDown;
        if (typeof handler !== "function")
          throw new Error("missing web keyboard handler");
        handler(event);
      });
