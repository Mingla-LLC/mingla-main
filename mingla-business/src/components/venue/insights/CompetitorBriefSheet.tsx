  ...
  return <Sheet visible={visible} onClose={onClose} snapPoint="full" testID="competitor-brief-sheet">
    <ScrollView contentContainerStyle={styles.body}>
      ...
      {data?.brief ? <>
        ...
        <Section title="EVIDENCE" testID="competitor-brief-sheet-section-evidence">{data.brief.evidence.map((evidence) => <GlassCard key={evidence.id} variant="base" testID={`competitor-brief-sheet-evidence-${evidence.id}`}><Text style={styles.provenance}>Observed fact</Text><Text style={styles.copy}>{evidence.observation}</Text>{evidence.observedAt ? <Text style={styles.meta}>{`Observed ${new Date(evidence.observedAt).toLocaleString()}`}</Text> : null}<Text style={styles.meta}>{`Checked ${new Date(evidence.checkedAt).toLocaleString()}`}</Text><Button label="Open source evidence" variant="secondary" size="md" accessibilityLabel={`Open source evidence, checked ${new Date(evidence.checkedAt).toLocaleDateString()}, opens outside Mingla`} onPress={() => { if (!/^https?:\/\//i.test(evidence.publicUrl)) return; captureCompetitorIntelligenceEvent("competitor_evidence_opened", { watch_id: row?.id, schema_version: 2 }); openExternal(evidence.publicUrl); }} testID={`competitor-brief-sheet-evidence-${evidence.id}-open`} /></GlassCard>)}</Section>
