...
export function CircleReachSheet({visible,ring,query,onClose}:{visible:boolean;ring:BrandCircleRing;query:CircleQuery;onClose:()=>void}):React.ReactElement|null{
  ...
  const sheetRef=React.useRef<View|null>(null),titleRef=React.useRef<Text|null>(null),closeRef=React.useRef<React.ElementRef<typeof Button>|null>(null);
  React.useEffect(()=>{
    if(!visible)return;
