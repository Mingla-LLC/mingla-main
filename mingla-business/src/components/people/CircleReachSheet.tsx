...
export function CircleReachSheet({visible,ring,query,onClose}:{visible:boolean;ring:BrandCircleRing;query:CircleQuery;onClose:()=>void}):React.ReactElement|null{
  ...
  const loadMore=React.useCallback(async():Promise<void>=>{if(loadMoreInFlight.current||!query.hasNextPage||query.isFetchingNextPage)return;loadMoreInFlight.current=true;try{await query.fetchNextPage({cancelRefetch:false})}finally{loadMoreInFlight.current=false}},[query]);
