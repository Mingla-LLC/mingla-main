/**
 * Issue #2735 — Buyer public venue Menu survives the real lazy transition.
 *
 * The route-shaped harness holds a real React.lazy promise pending, observes
 * PublicMenuSections as Suspense's fallback, then resolves to the ACTUAL
 * BuyerVenueOrderingSurface with ordering off. The same uniquely named items
 * must remain in the committed tree exactly once after resolution.
 */
...
const mockBuyerOrdering: { current: BuyerVenueOrdering } = {
  current: {
    config: ORDERING_OFF,
    scanned: false,
    modifiersByItemId: {},
...
jest.mock("../useBuyerVenueOrdering", () => ({
  __esModule: true,
  useBuyerVenueOrdering: () => mockBuyerOrdering.current,
}));
...
const renderBuyerSurface = async (): Promise<RenderTree> => {
  let tree!: RenderTree;
  await TestRenderer.act(async () => {
    tree = TestRenderer.create(
      <BuyerVenueOrderingSurface
        palette={palette}
        surface={surface}
        theme={theme}
        brandSlug="gogilagos"
        venueSlug="gogi"
        spotCode={mockBuyerOrdering.current.scanned ? "table-12" : null}
        entrySource={mockBuyerOrdering.current.scanned ? "qr" : null}
        menu={MENU}
        timezone="Africa/Lagos"
      />,
    );
...
test("Buyer ordering-off Menu remains populated after the actual lazy surface resolves", async () => {
  ...
  const LazyBuyerSurface = React.lazy(() => lazyModule);
  let tree!: RenderTree;
  ...
  expect(
    mounted.root.findAll((node) => node.type === BuyerVenueOrderingSurface),
  ).toHaveLength(0);
...
test("Buyer preserves one honest notice above one display menu when ordering is unavailable", async () => {
  mockBuyerOrdering.current = {
    ...mockBuyerOrdering.current,
    config: ORDERING_OFF,
    scanned: true,
  };
  const tree = await renderBuyerSurface();
...
test("Buyer keeps the existing orderable menu without a duplicate display list", async () => {
  mockBuyerOrdering.current = {
    ...mockBuyerOrdering.current,
    config: {
      ...ORDERING_OFF,
