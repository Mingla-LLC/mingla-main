...
const served = (id = "night-a") => ({ entityType: "primary", entityId: "same-id", displayName: "Ada", qrCode: "served-qr", pdfFetchRef: "old-pdf", eventId: id });
...
test("current denial of an unconfirmed restore never exposed a popup, clears the reply, and announces once without stealing focus", async () => {
  const check = pending<unknown>(); mockMetadata.mockReturnValue(check.promise); storage.set(key(), JSON.stringify(snapshot()));
  const tree = await mount();
  // #3416 D1: no pass or popup can be opened while the service has not answered.
  expect(state.guestStatus).toBe("going"); expect(state.passAction).toBeNull();
  ...
  await update(tree); expect(focus).not.toHaveBeenCalled();
  expect(nodes(tree, "rsvp-decision-validation-hint").filter((node) => node.props.accessibilityRole === "alert")).toHaveLength(1);
});

test("offline feedback keeps the reply but never the unconfirmed pass, does not focus, and validation takes precedence", async () => {
  ...
  // #3416 D1: offline is not confirmation — the reply stays, the stored QR does not.
  expect(state.validationHint).toBe(offline); expect(state.guestStatus).toBe("going"); expect(state.passAction).toBeNull();
  expect(storage.has(key())).toBe(true); expect(focus).not.toHaveBeenCalled();
...
test("same anonymous tab remount restores its own pass; account change purges every RSVP key but preserves other tab data", async () => {
  mockMetadata.mockResolvedValue(served());
...
test("returning A to B to A is a new generation: old A denial cannot delete the current restored A pass", async () => {
  const old = pending<unknown>(); mockMetadata.mockReturnValueOnce(old.promise).mockResolvedValue(served()); storage.set(key(), JSON.stringify(snapshot()));
...
test("a pending PDF presentation receives a predicate that loses authority when its open pass is invalidated", async () => {
  const check = pending<unknown>(); mockMetadata.mockReturnValue(check.promise); storage.set(key(), JSON.stringify(snapshot()));
  const tree = await mount();
  // #3416 D1: nothing to present until the service confirms the pass.
  expect(state.passAction).toBeNull();
  ...
  // The confirmed pass is invalidated for this page by leaving its event.
  await update(tree, { id: "night-b" });
...
test("queued callbacks from a departed account cannot reopen its pass or strand the new account in Saving", async () => {
  ...
  const tree = await mount({ identity: "account-a" });
  // #3416 D2: a signed-in account never restores from tab storage; its pass comes from its own reply.
  expect(state.guestStatus).toBeNull(); expect(state.passAction).toBeNull(); expect(mockMetadata).not.toHaveBeenCalled();
  await act(async () => { state.onGoingTap(); }); await act(async () => { confirm(); });
  ...
  expect(popup().visible).toBe(false); expect(state.submitting).toBe(false); expect(state.guestStatus).toBeNull(); expect(onSubmit).toHaveBeenCalledTimes(1);
});
