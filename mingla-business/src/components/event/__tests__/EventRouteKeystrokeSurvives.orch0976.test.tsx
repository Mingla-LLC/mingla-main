/**
 * EventRouteKeystrokeSurvives.orch0976 — issue #976 [event-name-focus]
 * happy-path route-level regression suite (SPEC §8.3, ticketed event wizard —
 * twin of RsvpRouteKeystrokeSurvives.orch0976).
 *
 * Reproduces the EXACT armed configuration that dropped focus on the physical
 * Samsung: a fresh d_* draft open in the wizard at /event/[id]/edit, a SECOND
...
// The wizard mock mirrors the REAL EventCreatorWizard's draft-edit lifecycle
// (beginDraftEdit on mount / endDraftEdit on unmount, keyed on the draft id —
// EventCreatorWizard.tsx:261-265): that lifecycle is what arms the #976
...
describe("issue #976 — event route: first-keystroke promotion never remounts the wizard nor loses text", () => {
  ...
  test("armed second list-hook instance + burst typing across the in-flight insert: 1 mount, 0 replace, 1 row, full live text", async () => {
    const draft = useDraftEventStore.getState().createDraft(BRAND_ID);
    const dId = draft.id;
