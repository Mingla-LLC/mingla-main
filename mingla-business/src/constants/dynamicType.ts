 ...
 * at 50/71.4 pt — inside a pill whose height is 44. Before #2211 the shared
 * `Button` primitive exposed no ceiling at all, so all 161 of its call sites
 * cropped their label at accessibility text sizes, and the two routes that
 * accept a team invitation pushed their only button off the bottom of the
 * screen entirely.
 *
 ...
 *
 * NOTE — `app/+not-found.tsx` keeps its own `CTA_MAX_FONT_SCALE` /
 * `ctaLabelStyle` exports from #2180. They are imported BY PATH from the
 * immutable `issue_2180_not_found_structure` suite, so they are deliberately
 * NOT moved here. Same number, same intent, different owner.
 */
