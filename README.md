...
- **Optional stops (hardened 2026-03-22):** Compact swipe card shows ONLY main (non-optional) stops. Expanded modal shows full plan with optional stops labeled "Suggested" and dismissible. Shopping list (picnic-dates) renders as top-level section above stops. AI descriptions render per stop from `stop.aiDescription`. Must never happen: optional stops appearing on the compact card face, or shopping list buried inside a stop.
