...
export function useAppHandlers(state: any) {
  ...
  const handleCollabPreferencesSave = (preferences: any) => {
    if (state.activeSessionData) {
      ...
      console.log('Collaboration preferences saved for session:', state.activeSessionData.id, preferences);
    }
  ...
  const handlePromoteToAdmin = (boardId: string, participantId: string) => {
    const updatedBoards = boardsSessions.map((board: any) => {
      if (board.id === boardId) {
        ...
        const participant = board.participants.find((p: any) => p.id === participantId);
        const notification = {
    ...
    console.log('Promoted member to admin:', participantId, 'on board:', boardId);
  };
  ...
  const handleDemoteFromAdmin = (boardId: string, participantId: string) => {
    const updatedBoards = boardsSessions.map((board: any) => {
      if (board.id === boardId) {
        const updatedBoard = {
          ...board,
          admins: board.admins.filter((adminId: string) => adminId !== participantId)
        };
  ...
  const handleRemoveMember = (boardId: string, participantId: string) => {
    ...
    const participant = board.participants.find((p: any) => p.id === participantId);
    const updatedBoards = boardsSessions.map((b: any) => {
      if (b.id === boardId) {
        const updatedParticipants = b.participants.filter((p: any) => p.id !== participantId);
        const updatedAdmins = b.admins.filter((adminId: string) => adminId !== participantId);
    ...
    const notification = {
      ...
      type: 'success' as const,
      title: updatedBoards.find((b: any) => b.id === boardId) ? 'Member Removed' : 'Board Deleted',
  ...
  const handleLeaveBoard = (boardId: string) => {
    ...
    const updatedBoards = boardsSessions.map((b: any) => {
      if (b.id === boardId) {
        const updatedParticipants = b.participants.filter((p: any) => p.id !== currentUserId);
        let updatedAdmins = b.admins.filter((adminId: string) => adminId !== currentUserId);
        
        if (updatedParticipants.length < 2) {
          return null;
        }
        
        if (isCurrentUserAdmin && updatedAdmins.length === 0 && updatedParticipants.length >= 2) {
          const nonAdminParticipants = updatedParticipants.filter((p: any) => !updatedAdmins.includes(p.id));
          if (nonAdminParticipants.length > 0) {
            const randomIndex = Math.floor(Math.random() * nonAdminParticipants.length);
    ...
    const notification = {
      ...
      type: 'success' as const,
      title: updatedBoards.find((b: any) => b.id === boardId) ? 'Left Board' : 'Board Deleted',
      message: updatedBoards.find((b: any) => b.id === boardId) 
  ...
  const handleAddToBoard = (sessionIds: string[], friend: any) => {
    sessionIds.forEach((sessionId, index) => {
      const notification = {
        ...
        type: 'success' as const,
        title: sessionIds.length === 1 ? 'Added to Board!' : `Added to ${sessionIds.length} Boards!`,
    ...
    console.log('Friend added to boards:', friend.name, 'Sessions:', sessionIds);
  };

  const handleShareSavedCard = (friend: any, suppressNotification?: boolean) => {
  ...
  const handleUnblockUser = (blockedUser: any, suppressNotification?: boolean) => {
    const updatedBlockedUsers = blockedUsers.filter((u: any) => u.id !== blockedUser.id);
    setBlockedUsers(updatedBlockedUsers);
  ...
  const handleReportUser = (friend: any, suppressNotification?: boolean, reason?: string, details?: string) => {
    if (!suppressNotification) {
      ...
      const reasonText = reason && reasonMessages[reason] 
    ...
    console.log('Reported user:', friend.name, 'Reason:', reason, 'Details:', details);
  };
  ...
  const handleSavePreferences = (preferences: any) => {
    // Map PreferencesSheet data to userPreferences format
    const mappedPreferences = {
      experienceTypes: preferences.selectedExperiences || [],
      categories: preferences.selectedCategories || [],
      budgetMin: preferences.budgetMin || '',
      budgetMax: preferences.budgetMax || '',
      dateOption: preferences.dateOption || '',
      selectedDate: preferences.selectedDate || '',
      timeOfDay: preferences.selectedTimeSlot || '',
      exactTime: preferences.exactTime || '',
      transportation: preferences.travelMode || '',
      actualDateTime: preferences.actualDateTime || null,
      // Keep other existing preferences
      location: userPreferences?.location || 'San Francisco, CA',
      groupSize: userPreferences?.groupSize || 'Small group (2-4)',
      accessibility: userPreferences?.accessibility || 'No specific needs',
      duration: userPreferences?.duration || '2-3 hours',
      weatherPreference: userPreferences?.weatherPreference || 'Any weather'
    };
    
    setUserPreferences(mappedPreferences);
    safeLocalStorageSet('mingla_user_preferences', mappedPreferences);
    
    // Reset removed cards to allow new card generation
    setRemovedCardIds([]);
    safeLocalStorageSet('mingla_removed_cards', []);
    
    // Notify user that preferences were saved and new cards will be generated
    const notification = {
      id: `preferences-saved-${Date.now()}`,
      type: 'success' as const,
      title: '✨ Preferences Updated!',
      message: 'Generating new personalized recommendations for you...',
      autoHide: true,
      duration: 3000
    };
    setNotifications((prev: any) => [...prev, notification]);
    
    console.log('User preferences saved and cards reset:', mappedPreferences);
  };
  ...
  const handleNavigateToActivityBoard = (board: any, discussionTab: string = 'discussion') => {
    setCurrentPage('activity');
    setActivityNavigation({ 
      selectedBoard: board, 
      activeTab: 'boards',
    ...
    console.log('Navigating to Activity page, board:', board.name, 'discussion tab:', discussionTab);
  };
  ...
  const handleShareCard = (experienceData: any) => {
    ...
    console.log('Sharing card:', experienceData.title, 'with prefs:', dateTimePrefs);
  };
