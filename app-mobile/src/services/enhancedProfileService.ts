...
export class EnhancedProfileService {
  ...
  async getGamifiedProfileData(
    userId: string
  ): Promise<ProfileGamifiedData | null> {
    try {
      const now = new Date();
  ...
  private async updateUserVibes(userId: string): Promise<void> {
    try {
      ...
      if (vibesData && vibesData.length > 0) {
        ...
        await supabase.from("user_vibes").insert(vibesToInsert);
      }
    } catch (error) {
  ...
  async addTimelineEvent(
    ...
    try {
      ...
      const { error } = await supabase.from("user_timeline").insert({
        user_id: user.id,
