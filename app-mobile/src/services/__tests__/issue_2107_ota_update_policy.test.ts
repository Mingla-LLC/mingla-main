// #2107 implementor happy-path proof — Consumer.
//
...
describe("#2107 Consumer OTA policy parsing", () => {
  it("accepts the exact server key set and nothing else", () => {
    ...
    expect(parseOtaPolicy(policy({ appId: "business" as never }), "ios", "1.1.4")).toBeNull();
  });
  ...
  it("scopes the acknowledgement cache to app, platform, runtime and schema", () => {
    expect(getAcknowledgementCacheKey("ios", "1.1.4")).toBe(
      "mingla.otaAcknowledgement.explorer.ios.1.1.4.schema1",
    );
...
describe("#2107 Consumer acknowledgement gate", () => {
