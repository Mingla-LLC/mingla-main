...
import { AccessibilityInfo, AppState, Clipboard, Image, Platform, Pressable, StyleSheet, Text, View, useColorScheme, useWindowDimensions, ActivityIndicator } from 'react-native';
...
export function UnifiedShareProvider({ children }: { children: React.ReactNode }): React.ReactElement {
  ...
  const openContentShare = useCallback((nextInput: OpenContentShareInput): void => {
    if (activePresentationAttempt.current !== null || visible) {
      ...
      if (pending !== null) {
        pending.input = nextInput;
        pendingExpandedAttempt.current = null;
        emitPresentationFailure(pending, 'presentation_rejected');
        pending.presented.reject(new Error('presentation_rejected'));
        pending.dismissalRequested.resolve();
        pending.dismissed.resolve();
      }
      throw new Error('share_presentation_busy');
    }
    ...
    const attempt = pendingExpandedAttempt.current ?? {
  ...
  }, [emitPresentationFailure, loadRecipients, loadShare, visible]);
  ...
  const handleNativeShow = useCallback((): void => {
    const attempt = activePresentationAttempt.current;
    if (attempt === null || attempt.input === null || attempt.presented.settled) return;
  ...
  const handleNativeDismiss = useCallback((): void => {
    const attempt = activePresentationAttempt.current;
  ...
  const close = useCallback(() => {
    if (sending) return;
    activePresentationAttempt.current?.dismissalRequested.resolve();
    generation.current += 1;
    setVisible(false);
  }, [sending]);
  ...
  return <UnifiedShareContext.Provider value={value}>
    {children}
    <BaseBottomSheet accessibilityLabel={`Share ${prepared?.title ?? input?.kind ?? ''}`} visible={visible} onClose={close} onNativeShow={handleNativeShow} onNativeDismiss={handleNativeDismiss} theme={dark ? 'dark' : 'light'} snapPoints={['90%']} enablePanDownToClose={!sending} scrollMode="scroll" wrapInRNModal keyboardBehavior="interactive" keyboardBlurBehavior="restore" android_keyboardInputMode="adjustResize" header={header} stickyFooter={footer} scrollProps={{ keyboardShouldPersistTaps: 'handled', contentContainerStyle: styles.body }}>
