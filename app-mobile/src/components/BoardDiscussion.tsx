...
const mockBoardCards = [
  ...
  },
  {
    ...
    fullDescription: 'Join us for an immersive farm-to-table cooking experience where you\'ll learn to create delicious dishes using the freshest local ingredients. Our expert chefs will guide you through techniques and recipes you can recreate at home.',
            ...
            <DropdownMenuTrigger asChild>
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48">
                  ...
                  >
                    <Ionicons name="people" size={16} color="#6b7280" style={{ marginRight: 8 }} />
                    Manage Members
                  </DropdownMenuItem>
                  <DropdownMenuItem 
                    onPress={() => {
                  ...
                  >
                    <Ionicons name="log-out" size={16} color="#6b7280" style={{ marginRight: 8 }} />
                    Leave Board
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            ...
            <View className="flex-1 overflow-y-auto bg-white">
              {messages.length > 0 ? (
                      ...
                      <View className="flex gap-3">
                        ...
                        <View className="flex-1 min-w-0">
                          <View className="flex items-center gap-2 mb-1">
                            <Text className="font-medium text-gray-900 text-sm">{message.user.name}</Text>
                            <Text className="text-xs text-gray-500">{message.timestamp}</Text>
                          </View>
                          ...
                          <View className="text-sm text-gray-800 mb-2">
                            {message.content.split(/(@\w+\s*\w*|#[\w\s]+)/g).map((part, index) => {
                              if (part.startsWith('@')) {
                                return (
                                  <Text key={index} className="text-[#eb7825] font-medium">
                                    {part}
                                  </Text>
                                );
                              } else if (part.startsWith('#')) {
                                return (
                                  <Text key={index} className="text-blue-600 font-medium">
                                    {part}
                                  </Text>
                                );
                              }
                          ...
                          {(message.mentions || message.cardTags) && (
                            <View className="flex flex-wrap gap-1 mb-2">
                              {message.mentions?.map(mention => (
                                <Text key={mention} className="inline-flex items-center gap-1 bg-orange-50 text-[#eb7825] px-2 py-0.5 rounded-full text-xs">
                                  <Ionicons name="at" size={12} color="#6b7280" />
                                  {mention}
                                </Text>
                              ))}
                              {message.cardTags?.map(tag => (
                                <Text key={tag} className="inline-flex items-center gap-1 bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full text-xs">
                                  <Ionicons name="hash" size={12} color="#6b7280" />
                                  {tag}
                                </Text>
                              ))}
                            </View>
                          ...
                          <View className="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                            <TouchableOpacity className="flex items-center gap-1 text-xs text-gray-500 hover:text-red-500 transition-colors">
                              <Ionicons name={message.isLiked ? 'heart' : 'heart-outline'} size={12} color={message.isLiked ? '#ef4444' : '#6b7280'} />
                              {message.likes}
                            </TouchableOpacity>
                            <TouchableOpacity className="text-xs text-gray-500 hover:text-gray-700 transition-colors">
                              <Ionicons name="arrow-undo" size={12} color="#6b7280" />
                            </TouchableOpacity>
                            <TouchableOpacity className="text-xs text-gray-500 hover:text-gray-700 transition-colors">
                              <Ionicons name="ellipsis-horizontal" size={12} color="#6b7280" />
                            </TouchableOpacity>
                          </View>
                        </View>
                      </View>
                ...
                <View className="text-center py-8 text-gray-500">
                  <Ionicons name="chatbubble-outline" size={32} color="#d1d5db" style={{ alignSelf: 'center', marginBottom: 8 }} />
                  <Text className="text-sm">No messages yet. Start the conversation!</Text>
                </View>
              )}
            </View>

            {/* Input */}
            <View className="relative p-4 border-t border-gray-100 bg-white flex-shrink-0 z-10" style={{ paddingBottom: 'env(safe-area-inset-bottom, 16px)' }}>
              {/* Suggestions */}
              {showMentions && (
