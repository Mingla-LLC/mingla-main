...
export function curatedToRecommendation(card: any): Recommendation {
  ...
  return {
    ...
    distance: firstStop && (firstStop.distanceFromUserKm ?? 0) > 0 ? `${firstStop.distanceFromUserKm} km` : '',
    travelTime: firstStop && (firstStop.travelTimeFromUserMin ?? 0) > 0 ? `${firstStop.travelTimeFromUserMin} min` : '',
